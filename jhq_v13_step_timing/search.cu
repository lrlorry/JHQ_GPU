#include "jhq_v13_step_timing/search.cuh"
...
{
    if (ws.batch_cap <= 0)
        throw std::runtime_error("v13: workspace not initialised — call add() first");
    if (!ws.stream || !ws.h_q_pinned)
        throw std::runtime_error("v13: CUDA stream / pinned buffer not created");
    ...
    for (int qoff = 0; qoff < nq; qoff += batch_size) {
        ...
        CUDA_CHECK(cudaMemcpyAsync(h_out_dists + (long long)qoff * k,
                                   ...
                                   cudaMemcpyDeviceToHost, ws.stream));
        CUDA_CHECK(cudaEventRecord(ws.ev_d2h_done, ws.stream));

        if (ws.timing_count < LOG_BATCHES) {
            CUDA_CHECK(cudaStreamSynchronize(ws.stream));

            float h2d_ms = 0.f, d2h_ms = 0.f;
            float step_ms[N_STEP_EVENTS - 1] = {};
            float graph_ms = 0.f;
            CUDA_CHECK(cudaEventElapsedTime(&h2d_ms, ws.ev_h2d_start, ws.ev_h2d_done));
            CUDA_CHECK(cudaEventElapsedTime(&d2h_ms, ws.ev_step[N_STEP_EVENTS - 1], ws.ev_d2h_done));
            for (int s = 0; s < N_STEP_EVENTS - 1; ++s) {
                CUDA_CHECK(cudaEventElapsedTime(&step_ms[s], ws.ev_step[s], ws.ev_step[s + 1]));
                graph_ms += step_ms[s];
            }

            std::fprintf(stderr,
                "[V13 STEP-TIMING batch %d  B=%d  nprobe=%d  ck=%d]\n"
                "  H2D             = %7.3f ms\n"
                "  gemm_rotate     = %7.3f ms\n"
                "  gemm_centroid   = %7.3f ms\n"
                "  select_probes   = %7.3f ms\n"
                "  build_byte_lut  = %7.3f ms\n"
                "  scan_ivf_trans  = %7.3f ms  ***\n"
                "  build_lut_r     = %7.3f ms\n"
                "  residual_refine = %7.3f ms  ***\n"
                "  topk_final      = %7.3f ms\n"
                "  D2H             = %7.3f ms\n"
                "  GRAPH kernel    = %7.3f ms\n"
                "  TOTAL gpu       = %7.3f ms\n",
                ws.timing_count, B, nprobe, ck,
                h2d_ms,
                step_ms[0], step_ms[1], step_ms[2], step_ms[3],
                step_ms[4], step_ms[5], step_ms[6], step_ms[7],
                d2h_ms, graph_ms, h2d_ms + graph_ms + d2h_ms);
            ++ws.timing_count;
        }
    }
