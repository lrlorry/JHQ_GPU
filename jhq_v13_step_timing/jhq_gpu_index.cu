#include "jhq_v13_step_timing/jhq_gpu_index.cuh"
#include "jhq_v13_step_timing/encode.cuh"
...
JHQGpuIndex::~JHQGpuIndex() {
    ...
    for (cudaEvent_t ev : ws_.ev_step)
...
void JHQGpuIndex::alloc_workspace(int batch_size) {
    ...
    for (cudaEvent_t& ev : ws_.ev_step)
        if (!ev) CUDA_CHECK(cudaEventCreate(&ev));
...
{
    if (n_train < nlist_)
        throw std::invalid_argument("n_train must be >= nlist for v13_step_timing");
...
void JHQGpuIndex::add(const float* h_x, int n) {
    ...
    if (ntotal_ != 0)
        throw std::runtime_error("v13_step_timing currently supports one add() call");
