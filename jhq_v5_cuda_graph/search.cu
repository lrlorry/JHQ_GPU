#include "jhq_v5_cuda_graph/search.cuh"
    ...
    int*   h_out_ids)
{
    ...
    if (!ws.stream)
        throw std::runtime_error("v5: CUDA stream not created");
    ...
    // Enqueue all batches on ws.stream — no per-batch CPU block.
    for (int qoff = 0; qoff < nq; qoff += batch_size) {
        int B = std::min(batch_size, nq - qoff);

        // Stage this batch's queries at the fixed workspace pointer.
        CUDA_CHECK(cudaMemcpyAsync(
            ws.d_q_batch,
            d_queries + (long long)qoff * d,
            (long long)B * d * sizeof(float),
            cudaMemcpyDeviceToDevice, ws.stream));

        // Zero-pad trailing slots for partial last batch.
        if (B < B_full) {
            CUDA_CHECK(cudaMemsetAsync(
                ws.d_q_batch + (long long)B * d, 0,
                (long long)(B_full - B) * d * sizeof(float), ws.stream));
        }

        // One graph launch replaces the entire multi-kernel pipeline.
        CUDA_CHECK(cudaGraphLaunch(ws.graph_exec, ws.stream));

        // Async D→H — overlaps with next batch's compute.
        CUDA_CHECK(cudaMemcpyAsync(
