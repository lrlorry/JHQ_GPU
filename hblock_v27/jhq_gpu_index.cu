...
HBlockIndex::HBlockIndex(int d, Params p)
    : d_(d), d_proj_(p.d_proj), Kr_(p.Kr), Br_(p.Br),
      ...
      beam_size_(p.beam_size), block_topk_(p.block_topk),
...
void HBlockIndex::train(const float* h_x, int n_train)
{
    ...
    printf("[v26 train] d=%d d_proj=%d K1=%d K2=%d K3=%d ck1=%d ck2=%d ck3=%d"
    ...
    const std::vector<int>& h_pair_blk_count)
{
    ...
    gpu_build_block_adj_v27(d_blk_proj_gpu_, d_blk_norm_gpu_,
...
#define FD(p) do{if(ws_.p){cudaFree(ws_.p);ws_.p=nullptr;}}while(0)
    ...
    CUDA_CHECK(cudaMalloc(&ws_.d_out_dists,(long long)max_pairs*block_topk_*sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ws_.d_out_ids,  (long long)max_pairs*block_topk_*sizeof(int)));
    ...
    ws_.beam_size=beam_size_;
    ws_.block_topk=block_topk_;
#undef FH
#undef FD
}
                          ...
                          float* h_dists, int* h_ids) const
{
    ...
    for(int qstart=0;qstart<nq;qstart+=batch_size_){
        ...
        auto t3=Clock::now();
        gpu_build_and_sort_pairs_v27(nb,n_pairs,n_leaf_blocks_,ws_.max_leaf_sel,ws_);
        ms_pairs+=Ms(Clock::now()-t3).count();

        auto t4=Clock::now();
        launch_leaf_flat_v27(ws_.d_pair_leaf_b,ws_.d_pair_qid_b,
                             d_leaf_codes_,d_leaf_ids_,d_leaf_sizes_,ws_.d_lut_fine,
                             ws_.d_out_dists,ws_.d_out_ids,
                             n_pairs,d_,Kr_,Br_,bpv_,leaf_size_,block_topk_,s);
        ms_pq+=Ms(Clock::now()-t4).count();

        auto t5=Clock::now();
        gpu_merge_pq_topk_v27(nb,n_pairs,rerank_r_,block_topk_,ws_);
        ms_merge+=Ms(Clock::now()-t5).count();

        auto t6=Clock::now();
        launch_gather_vecs_v27(d_base_vecs_,ws_.d_pq_ids,ws_.d_cand_vecs,nb,rerank_r_,d_,s);
        ms_gather+=Ms(Clock::now()-t6).count();

        auto t7=Clock::now();
        launch_exact_rerank_v27(ws_.d_q_batch,ws_.d_cand_vecs,ws_.d_pq_ids,
