// hblock_v25/search.cu  — GPU kernels identical to v22, namespace hblock_v25.
...
void route_gpu_v25(
    ...
    SearchWorkspace& ws)
{
    if (nq > ws.batch_cap) throw std::runtime_error("hblock_v25: nq > batch_cap");
...
__global__ void build_pairs_kernel_v25(
...
void gpu_build_and_sort_pairs_v25(
    ...
    int max_leaf_sel, SearchWorkspace& ws)
{
    ...
    build_pairs_kernel_v25<<<(nq+255)/256, 256, 0, s>>>(
...
__global__ void leaf_flat_kernel_v25(
    ...
    cudaStream_t stream)
{
    ...
    leaf_flat_kernel_v25<<<n_pairs, leaf_size, smem, stream>>>(
...
__global__ void fill_iota_kernel_v25(int* arr, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) arr[i] = i;
}

__global__ void segmented_topk_kernel_v25(
...
void gpu_merge_pq_topk_v25(int nq, int n_pairs, int rerank_r, int top_p, SearchWorkspace& ws)
{
    cudaStream_t s = ws.stream;
    int k = std::min(rerank_r, (int)K_MAX);
    fill_iota_kernel_v25<<<(n_pairs+255)/256, 256, 0, s>>>(ws.d_pair_leaf_a, n_pairs);
    CUDA_CHECK(cub::DeviceRadixSort::SortPairs(
        ws.d_cub_tmp, ws.cub_bytes,
    ...
    segmented_topk_kernel_v25<<<nq, BLOCK, smem, s>>>(
...
__global__ void gather_vecs_kernel_v25(
...
void launch_gather_vecs_v25(
    const float* d_base_vecs, const int* d_cand_ids, float* d_cand_vecs,
    int B, int R, int d, cudaStream_t s)
{
    dim3 grid(R, B);
    gather_vecs_kernel_v25<<<grid, 128, 0, s>>>(d_base_vecs, d_cand_ids, d_cand_vecs, R, d);
    CUDA_CHECK(cudaGetLastError());
}

__global__ void exact_rerank_kernel_v25(
    ...
    int B, int R, int d, int k, cudaStream_t s)
{
    ...
    exact_rerank_kernel_v25<<<B, BLOCK, smem, s>>>(
...
blk_qdist_v25(const float* __restrict__ s_q, float q_norm,
...
try_visit_v25(int* __restrict__ vis, int b, int n_blks)
{
...
// ─────────────────────────────────────────────────────────────────────────────
__global__ void compute_adj_kernel_v25(
    ...
    int n_blks, int d_proj, int degree, cudaStream_t s)
{
    ...
    compute_adj_kernel_v25<<<grid, 128, 0, s>>>(
...
// ─────────────────────────────────────────────────────────────────────────────
__global__ void block_search_fused_v25(
    // Routing outputs (already on GPU after route_gpu_v25)
    ...
    int entry_per_cell, int bitmap_words)
{
        ...
        for(int i2 = 0; i2 < ck2; i2++){
            ...
            for(int i3 = 0; i3 < ck3; i3++){
                ...
                for(int start = 0; start < bc; start += 32){
                    ...
                    if(bi < bc){
                        ck_d  = blk_qdist_v25(s_q, q_norm, d_blk_proj, d_blk_norm,
                                               bs + bi, d_proj);
                        ck_id = bs + bi;
                    }
                    ...
                    for(int ep = 0; ep < entry_per_cell; ep++){
                        ...
                        bool fv = false;
                        if(tid == 0 && ins_id >= 0)
                            fv = try_visit_v25(vis, ins_id, n_blks);
    ...
    for(int iter = 0; iter < budget; iter++){
        ...
        if(tid < degree){
            ...
            if(try_visit_v25(vis, nb, n_blks)){
                new_d  = blk_qdist_v25(s_q, q_norm, d_blk_proj, d_blk_norm, nb, d_proj);
                new_id = nb;
            }
        }
    ...
    SearchWorkspace& ws)
{
    ...
    block_search_fused_v25<<<B, 32, smem, ws.stream>>>(
        ws.d_top1_ids, ws.d_top2_beam, ws.d_top3_beam,
