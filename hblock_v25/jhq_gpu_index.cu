#include "hblock_v25/jhq_gpu_index.cuh"
...
void HBlockIndex::train(const float* h_x, int n_train)
{
    ...
    printf("[v25 train] d=%d d_proj=%d K1=%d K2=%d K3=%d ck1=%d ck2=%d ck3=%d"
           " graph_degree=%d graph_budget=%d entry_per_cell=%d top_p=%d\n",
           d_, d_proj_, K1_, K2_, K3_, ck1_, ck2_, ck3_,
           graph_degree_, graph_budget_, entry_per_cell_, top_p_);
    ...
    printf("[v25 train] total=%.1f ms  sigma_r3=%.4f\n", Ms(Clock::now()-T0).count(), sigma);
}
...
void HBlockIndex::add(const float* h_x, int n)
{
    ...
    printf("[v25 add] n=%d  d=%d  batch=%d  mini_km_iters=%d\n", n, d_, 8192, mini_km_iters_);
    ...
    std::vector<float>   h_proj1_all((long long)n*d_proj_);  // Pi1*x, saved for mini k-means
    ...
    for (int s=0;s<n;s+=BATCH) {
        ...
        // save Pi1*x for mini k-means block reordering
    ...
    // ── Mini k-means within each L3 cell — reorder for semantic blocks ────────
    if (mini_km_iters_ > 0) {
        T0 = Clock::now();
        ...
        for (int i = 0, j; i < n; i = j) {
            ...
            int N_cell = j - i;
            if (N_cell <= leaf_size_) continue;  // single block, skip

            int K = (N_cell + leaf_size_ - 1) / leaf_size_;

            // Extract Pi1 projections for this cell's vectors
            ...
            // Mini k-means: evenly-spaced initialization
            ...
            // Reorder within cell by cluster id (stable to keep intra-cluster order)
        ...
        printf("  [mini-kmeans reorder] %d cells reordered  %.1f ms\n",
    ...
    printf("[v25 add total] %.1f ms  blocks=%d\n",
    ...
    const std::vector<int>& h_pair_blk_count)
{
    ...
    gpu_build_block_adj_v25(d_blk_proj_gpu_, d_blk_norm_gpu_,
...
#define FD(p) do{if(ws_.p){cudaFree(ws_.p);ws_.p=nullptr;}}while(0)
    ...
    CUDA_CHECK(cudaMalloc(&ws_.d_out_dists,(long long)max_pairs*top_p_*sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ws_.d_out_ids,  (long long)max_pairs*top_p_*sizeof(int)));
                          ...
                          float* h_dists, int* h_ids) const
{
    ...
    for(int qstart=0;qstart<nq;qstart+=batch_size_){
        ...
        auto t1=Clock::now();
        gpu_block_search_v25(nb, n_leaf_blocks_, d_proj_,
        ...
        auto t3=Clock::now();
        gpu_build_and_sort_pairs_v25(nb,n_pairs,n_leaf_blocks_,ws_.max_leaf_sel,ws_);
        ms_pairs+=Ms(Clock::now()-t3).count();

        auto t4=Clock::now();
        launch_leaf_flat_v25(ws_.d_pair_leaf_b,ws_.d_pair_qid_b,
                             d_leaf_codes_,d_leaf_ids_,d_leaf_sizes_,ws_.d_lut_fine,
                             ws_.d_out_dists,ws_.d_out_ids,
                             n_pairs,d_,Kr_,Br_,bpv_,leaf_size_,top_p_,s);
        ms_pq+=Ms(Clock::now()-t4).count();

        auto t5=Clock::now();
        gpu_merge_pq_topk_v25(nb,n_pairs,rerank_r_,top_p_,ws_);
        ms_merge+=Ms(Clock::now()-t5).count();

        auto t6=Clock::now();
        launch_gather_vecs_v25(d_base_vecs_,ws_.d_pq_ids,ws_.d_cand_vecs,nb,rerank_r_,d_,s);
        ms_gather+=Ms(Clock::now()-t6).count();

        auto t7=Clock::now();
        launch_exact_rerank_v25(ws_.d_q_batch,ws_.d_cand_vecs,ws_.d_pq_ids,
                                   ...
                                   const float* h_base, const int* h_gt, int gt_k) const
{
    ...
    // Allocate temporary search outputs (routing still uses GPU)
    std::vector<float> dummy_dists((long long)nq * k, 1e30f);
    std::vector<int>   dummy_ids  ((long long)nq * k, -1);
    ...
    for (int qstart = 0; qstart < nq; qstart += batch_size_) {
        ...
        // GPU routing + beam search to collect visited blocks
        route_gpu_v25(cublas_,
                      d_Pi1_, d_Pi2_, d_Pi3_,
                      d_route1_cents_proj_, d_route1_cents_full_, d_route1_norms_,
                      d_route2_cents_proj_, d_route2_cents_full_, d_route2_norms_,
        ...
        // D2H: leaf_sel [nb × max_leaf_sel] and leaf_cnt [nb]
        std::vector<int> h_sel((long long)nb * ws_.max_leaf_sel);
        std::vector<int> h_cnt(nb);
        CUDA_CHECK(cudaMemcpyAsync(h_cnt.data(), ws_.d_leaf_cnt,
                                   (long long)nb * sizeof(int),
                                   cudaMemcpyDeviceToHost, s));
        CUDA_CHECK(cudaMemcpyAsync(h_sel.data(), ws_.d_leaf_sel,
                                   (long long)nb * ws_.max_leaf_sel * sizeof(int),
                                   cudaMemcpyDeviceToHost, s));
        CUDA_CHECK(cudaStreamSynchronize(s));

        // CPU exact scan over visited blocks for each query in batch
        for (int qi = 0; qi < nb; qi++) {
            int global_qi = qstart + qi;
            const float* qv = h_qb + (long long)qi * d_;
            int n_visited = h_cnt[qi];

            // Collect all vectors in visited blocks
            std::vector<std::pair<float,int>> cands;
            cands.reserve((size_t)n_visited * leaf_size_);

            for (int vi = 0; vi < n_visited; vi++) {
                int blk = h_sel[(long long)qi * ws_.max_leaf_sel + vi];
                if (blk < 0 || blk >= n_leaf_blocks_) continue;
                int sz = h_leaf_sizes_cpu_[blk];
                const int* ids = h_leaf_ids_cpu_.data() + (long long)blk * leaf_size_;
                for (int pos = 0; pos < sz; pos++) {
                    ...
                    for (int dim = 0; dim < d_; dim++) {
                        float diff = qv[dim] - xv[dim];
                        dist += diff * diff;
                    }
                    cands.push_back({dist, vid});
                }
            }

            // partial sort to get top-k
            int actual_k = std::min(k, (int)cands.size());
            if (actual_k > 0) {
                std::partial_sort(cands.begin(), cands.begin() + actual_k, cands.end());
            }

            // count hits against ground truth
            const int* gt_row = h_gt + (long long)global_qi * gt_k;
            for (int j = 0; j < actual_k; j++) {
                int found = cands[j].second;
