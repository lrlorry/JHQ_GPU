#include "v3_ivf/jhq_gpu_index.cuh"
#include "v3_ivf/encode.cuh"
#include "v3_ivf/search.cuh"
...
__global__ void assign_from_dots_kernel(
    const float* __restrict__ dots,       // [nlist * nb], column-major
...
JHQGpuIndex::~JHQGpuIndex() {
    ...
    cudaFree(ws_.d_cent_dists);
    cudaFree(ws_.d_cent_ids);
...
void JHQGpuIndex::alloc_workspace(long long N) {
    ...
    cudaFree(ws_.d_cent_dists);   ws_.d_cent_dists = nullptr;
    cudaFree(ws_.d_cent_ids);     ws_.d_cent_ids = nullptr;
    ...
    CUDA_CHECK(cudaMalloc(&ws_.d_cent_dists,   (long long)nlist_ * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ws_.d_cent_ids,     (long long)nlist_ * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&ws_.d_probe_ids,    (long long)nprobe_ * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&ws_.d_probe_offsets, (long long)(nprobe_ + 1) * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&ws_.d_final_ids,    N * sizeof(int)));
}
...
{
    if (n_train < nlist_)
        throw std::invalid_argument("n_train must be >= nlist for v3_ivf");
...
void JHQGpuIndex::add(const float* h_x, int n) {
    ...
    if (ntotal_ != 0)
        throw std::runtime_error("v3_ivf currently supports one add() call");
