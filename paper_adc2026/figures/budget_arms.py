...
def table(dst):
    ...
    with open(os.path.join(dst, "budget.tex"), "w") as fh:
        ...
        # Six columns, not nine.  The rule's pick barely moves with S -- it is
        # 32/64/64 on vogue and 4/4/4 on openai3 -- so two of those three
        # columns were a constant taking up width the failure rates needed,
        # and the full-pool choice is one sentence in the text.
        fh.write("\\begin{tabular}{@{}l@{\\qquad}c@{\\quad}c@{\\qquad}"
                 "c@{\\quad}c@{\\quad}c@{}}\n\\toprule\n")
        fh.write("& & & \\multicolumn{3}{c}{draws losing $>10^{-3}$ (\\%)}\\\\\n")
        fh.write("\\cmidrule(l){4-6}\n")
        fh.write("dataset\\,/\\,\\textit{np} & ref.\\,$\\alpha$ & "
                 "rule\\,$\\alpha$ & $S{=}32$ & $64$ & $128$\\\\\n\\midrule\n")
        fh.write("\n".join(rows) + "\n\\bottomrule\n\\end{tabular}\n")
