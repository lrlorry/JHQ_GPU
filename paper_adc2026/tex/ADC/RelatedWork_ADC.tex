...
budget. We do not report it as a baseline: we could not configure the GPU
implementation available to us so that its search reproduced the two-stage
behaviour its paper describes, and we prefer no measurement to one of a
configuration we cannot vouch for. CAGRA~\cite{cagra} and cuVS
IVF-PQ~\cite{cuvs} supply the GPU comparisons.
