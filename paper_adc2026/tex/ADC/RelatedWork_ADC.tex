...
budget. We do not report it as a baseline. Its two-stage behaviour is not what the
search call of the implementation available to us performs on its own; it has
to be assembled from that call and a separate re-ranking primitive, and we
have not validated that assembly against the published system. We leave the
comparison open rather than report a configuration we cannot vouch for. CAGRA~\cite{cagra} and cuVS
