...
// on the refine stage, so it is not a second-best.
//
// Two independent switches make a 2x2. JHQ_SCAN_NO_LUT keeps the strided read
// of list_primary_t and adds the raw byte instead of looking it up;
// JHQ_SCAN_NO_COMPACT keeps the arithmetic and drops the threshold compaction.
// With f, l, c and b for full, no-lut, no-compact and neither:
//
//     lookup     = f - l = c - b
//     compaction = f - c = l - b
//     gather     = b, minus launch overhead
//
// Both halves of each equality are measured, so a mismatch means one of the
// removals changed something else and the split does not hold.
//
    ...
    int nprobe, int M, int N, int ck, int cap, bool lut_in_smem, int tile_m)
{
    ...
    // every candidate.
    float sink = 0.0f; int sink_pos = 0; (void)sink; (void)sink_pos;
    int p = 0;
