...
#include "hblock_v41/search.cuh"   // for align4() and the region-indirected leaf kernel
      ...
      gpu_region_cap_(p.gpu_region_cap)
{
    ...
    if (p.leaf_size != 128)      throw std::invalid_argument("v41 currently requires leaf_size=128");
...
void HBlockIndex::train(const float* h_x, int n_train)
{
    ...
    printf("[v41 train] d=%d d_proj=%d K1=%d K2=%d K3=%d ck1=%d ck2=%d ck3=%d"
    ...
    printf("[v41 train] total=%.1f ms  sigma_r3=%.4f\n", Ms(Clock::now()-T0).count(), sigma);
}
...
void HBlockIndex::add_impl(Reader& reader, int n)
{
    ...
    printf("[v41 add] n=%d  d=%d  add_batch_size=%d  mini_km_iters=%d\n",
    ...
    // cells a cheap locality-preserving block order without quadratic mini-kmeans.
    ...
    for(int i=0,j;i<n;i=j){
        ...
        auto pack_one = [&](int blk, int pos, int oid) {
            ...
            // v41: block centroid accumulated from the int8 region source
    ...
    // Leaf records and block metadata are now self-contained. Release the
    // O(N*d_proj), O(N*bpv), and routing-code build arrays before allocating
    // the host region stores.
    ...
    std::vector<int8_t>().swap(h_base_i8_all);
...
std::vector<RegionWave> build_region_waves(
    const int* sorted_blocks, int n_pairs,
    const std::vector<int>& block_to_region, int pool_capacity)
{
    std::vector<RegionWave> waves;
    if (n_pairs == 0) return waves;
    if (pool_capacity <= 0) throw std::invalid_argument("region pool capacity must be positive");

    RegionWave wave;
    wave.pair_begin = 0;
    int last_region = -1;
    for (int i = 0; i < n_pairs; ++i) {
        const int block = sorted_blocks[i];
        if (block < 0 || block >= (int)block_to_region.size())
            throw std::runtime_error("invalid block id in region wave planner");
        const int region = block_to_region[block];
        if (region < 0) throw std::runtime_error("block has no region assignment");
        if (last_region >= 0 && region < last_region)
            throw std::runtime_error("block-to-region layout is not monotonic");
        if (region != last_region) {
            if ((int)wave.regions.size() == pool_capacity) {
                wave.pair_end = i;
                waves.push_back(std::move(wave));
                wave = RegionWave{};
                wave.pair_begin = i;
            }
            wave.regions.push_back(region);
            last_region = region;
        }
    }
    wave.pair_end = n_pairs;
    waves.push_back(std::move(wave));
    return waves;
}
                         ...
                         float* h_dists, int* h_ids, int ef) const
{
    ...
    if(nq<=0 || k<=0 || ef<=0) throw std::invalid_argument("nq, k and ef must be positive");
    ...
    for(int qstart=0;qstart<nq;qstart+=batch_size_){
        ...
        // Raw int8 query batch, for the staged exact-rerank kernel.
    ...
    bool verbose) const
{
    ...
    // Deliberately deferred: v41's scope is add()/search() payload
    ...
    throw std::runtime_error(
        "HBlockIndex::diagnose_missed_gt: not implemented in v41 (out of scope -- "
