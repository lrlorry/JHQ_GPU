...
namespace hblock_v33 {

// ══════════════════════════════════════════════════════════════════════════════
// v33 block beam search: W=4 batched expansion + HNSW early termination
// ══════════════════════════════════════════════════════════════════════════════

__device__ __forceinline__ float
blk_qdist_v33(const float* __restrict__ s_q, float q_norm,
...
try_visit_v33(int* __restrict__ vis, int b, int n_blks)
{
...
// SPT = slots per thread; beam_size = 32 × SPT (no cap at 128: SPT up to 8 for ef=256)
// W = 4: expand 4 blocks per round, check early termination once per round.
// Output: d_leaf_sel[qi*max_ls + 0..cnt-1] = final beam contents (valid block ids).
template <int SPT>
__global__ void block_search_v33_kernel(
    ...
    int entry_per_cell, int bitmap_words)
{
    ...
    // Beam: SPT slots per thread (beam_size = 32 * SPT)
        ...
        for (int i2 = 0; i2 < ck2; i2++) {
            ...
            for (int i3 = 0; i3 < ck3; i3++) {
                ...
                for (int start = 0; start < bc; start += 32) {
                    ...
                    if (bi < bc) {
                        ck_d  = blk_qdist_v33(s_q, q_norm, d_blk_proj, d_blk_norm, bs + bi, d_proj);
                        ck_id = bs + bi;
                    }
                    for (int ep = 0; ep < entry_per_cell; ep++) {
                        ...
                        bool fv = false;
                        if (tid == 0 && ins_id >= 0)
                            fv = try_visit_v33(vis, ins_id, n_blks);
    ...
    // ── Phase 2: Batched beam search with early termination ───────────────────
    for (int round = 0; round < max_rounds; round++) {

        // Termination check: best unexpanded vs worst in beam
        ...
        float worst_beam = 0.f;
        for (int s = 0; s < SPT; s++)
            if (my_id[s] >= 0) worst_beam = fmaxf(worst_beam, my_dist[s]);
        for (int o = 16; o >= 1; o >>= 1)
            worst_beam = fmaxf(worst_beam, __shfl_xor_sync(FULL, worst_beam, o));

        if (best_unexp >= worst_beam) break;  // HNSW early termination
        ...
        for (int w = 0; w < W; w++) {
            ...
            if (nb >= 0 && try_visit_v33(vis, nb, n_blks)) {
                nd = blk_qdist_v33(s_q, q_norm, d_blk_proj, d_blk_norm, nb, d_proj);
                ni = nb;
            }
    ...
    // ── Phase 3: Dump final beam to d_leaf_sel (compact, valid entries only) ──
    // All threads iterate same slots via shuffle; thread 0 writes.
    int out_cnt = 0;
    for (int s = 0; s < SPT; s++) {
        for (int src = 0; src < 32; src++) {
            int bid = __shfl_sync(FULL, my_id[s], src);
            if (bid >= 0) {
                if (tid == 0 && out_cnt < max_ls)
                    d_leaf_sel[(long long)qi * max_ls + out_cnt] = bid;
                out_cnt++;
            }
        }
    }
    if (tid == 0) d_leaf_cnt[qi] = min(out_cnt, max_ls);
}

#define LAUNCH_V33(SPT_VAL) \
    block_search_v33_kernel<SPT_VAL><<<B, 32, smem, ws.stream>>>( \
...
void gpu_block_search_v33(
    ...
    SearchWorkspace& ws)
{
    if (degree > 32) throw std::runtime_error("gpu_block_search_v33: degree must be <= 32");
    const int smem = d_proj * (int)sizeof(float);
    // max_rounds: same total expansion budget as v32 (ef expansions), divided into rounds of W=4
    // Early termination fires much sooner on easy queries
    const int max_rounds = ef / 4 + 1;
    if      (ef <= 32)  { LAUNCH_V33(1); }
    else if (ef <= 64)  { LAUNCH_V33(2); }
    else if (ef <= 128) { LAUNCH_V33(4); }
    else                { LAUNCH_V33(8); }
...
void route_gpu_v29(
    ...
    SearchWorkspace& ws)
{
    if (nq > ws.batch_cap) throw std::runtime_error("hblock_v33: nq > batch_cap");
