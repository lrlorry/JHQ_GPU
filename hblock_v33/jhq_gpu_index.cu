#include "hblock_v33/jhq_gpu_index.cuh"
                          ...
                          float* h_dists, int* h_ids, int ef) const
{
    ...
    ws_.beam_size    = ef;   // v33: no cap, full ef beam
    ws_.max_leaf_sel = ef;   // output is final beam (≤ ef entries per query)
    ...
    for(int qstart=0;qstart<nq;qstart+=batch_size_){
        ...
        auto t1=Clock::now();
        gpu_block_search_v33(nb, n_leaf_blocks_, d_proj_,
    ...
    const int* h_gt, int gt_k) const
{
    ...
    for (int qstart = 0; qstart < nq; qstart += batch_size_) {
        ...
        gpu_block_search_v33(nb, n_blks, d_proj_,
                             K2_, K3_, ck1_, ck2_, ck3_,
