#include "jhq_v14_streaming_add/jhq_gpu_index.cuh"
#include "jhq_v14_streaming_add/encode.cuh"
#include "jhq_v14_streaming_add/search.cuh"
...
// jhq_v14_streaming_add/jhq_gpu_index.cuh's Params::add_batch comment for
...
void JHQGpuIndex::add(const float* h_x, int n) {
    ...
    for (long long start = 0; start < n; start += AB) {
        ...
        launch_primary_encode(d_y_batch, d_pc + start * M_, d_c1d_,
                              nb, d_, M_, Ds_, K1D_, bits_per_dim_);
        launch_residual_encode(d_y_batch, d_pc + start * M_,
                               d_rc + start * bpv_, d_co + start,
                               d_c1d_, d_res_c1d_,
                               nb, d_, M_, Ds_, K1D_, Kr_, Br_, bpv_, bits_per_dim_);
        CUDA_CHECK(cudaDeviceSynchronize());
