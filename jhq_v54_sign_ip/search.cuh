...
// ── JHQ_SIGN_IP — the primary distance without a table ──────────────────────
//
// Equation 4 makes the K codewords a Cartesian product of the Ds per-dimension
// levels of equation 3, and L = 2^(B/Ds). Every configuration here runs at
// Ds = B = 8, so L = 2 and the two levels are +/- sigma*sqrt(2)*erfinv(0.5):
// symmetric, because equation 3 places them at the quantiles of a zero-mean
// Gaussian. Write them +/-a and the primary distance collapses:
//
//     D_P(q,y) = sum_j (q'_j - a*s_j)^2  with s_j in {-1,+1}
//              = ||q'||^2 + d*a^2 - 2a<q', s_y>
//
// which is an identity, not an approximation -- the same set, the same
// distances, only a different way to sum them. What it buys is shared memory.
// The factorised table needs M*32 = 4d floats a query resident; the query
// itself needs d. On openai3-3072 that is 49,152 B against 12,288, and
// results/front6/NEGATIVES.md prices the difference: 16 KB a block is worth
// 17-26% wherever occupancy is not already pinned at one -- and M=384's table
// is exactly what pins it there.
//
// What it costs is instructions: 32 predicated adds a word against the
// table's 8 loads and 8 adds. The scan runs at about 7% of this card's
// arithmetic throughput, so there is headroom on paper. Whether there is
// headroom in practice is what the two builds measure.
//
// Requires the word layout (the 32 signs of four subspaces are one uint32)
// and Ds = 8. verify_sign_levels() checks the codebook really is +/-a at
// train time and refuses to run otherwise.
#ifndef JHQ_SIGN_IP
#define JHQ_SIGN_IP 0
#endif
...
struct SearchWorkspace {
    ...
    // JHQ_SIGN_IP only. d_q_bit is the rotated query permuted so that
    // component p of a code byte sits at index p -- the encoder's bit order is
    // read off the centroids, not assumed. d_q_const carries the whole
    // query-only part of the distance, ||q'||^2 + d*a^2 + 2a*sum(q'), so the
    // scan's tail is one FMA: D_P = d_q_const[b] - 4a*acc.
    float*    d_q_bit         = nullptr;   // [batch_cap, d]
    float*    d_q_const       = nullptr;   // [batch_cap]
    float     sign_a          = 0.0f;      // the level magnitude a
    unsigned  sign_bitmap     = 0;         // dim_of_bit[p] in 3 bits each
