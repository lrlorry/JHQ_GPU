#include "jhq_v54_sign_ip/jhq_gpu_index.cuh"
...
#endif
#include "jhq_v54_sign_ip/encode.cuh"
#include "jhq_v54_sign_ip/train_pq_gpu.cuh"
#include "jhq_v54_sign_ip/train_res_gpu.cuh"
...
// Rearrange the primary codes from [M][N] bytes into [M/4][N] words, each
// word holding four consecutive subspaces -- 32 dimensions at 1 bit each.
//
// Nothing about the codes changes. What changes is which addresses a warp
// touches: reading a byte per thread makes 32 threads ask for 32 consecutive
// bytes, and the memory system still moves a 128-byte line for them. Reading a
// word per thread fills that line. The gather is 53-66% of the scan and runs
// at about half this card's peak bandwidth, which is what a quarter-used
// transaction looks like.
//
// Done after the fact rather than in the two encoders, so neither the
// two-pass build nor the streaming one has to know about the layout.
__global__ void repack_primary_words_kernel(
    const uint8_t* __restrict__ src,   // [M][n]
    uint32_t*      __restrict__ dst,   // [M/4][n]
    long long n, int M)
{
    const long long total = (long long)(M / 4) * n;
    for (long long i = (long long)blockIdx.x * blockDim.x + threadIdx.x;
         i < total; i += (long long)gridDim.x * blockDim.x) {
        const long long w   = i / n;      // which group of four subspaces
        const long long pos = i - w * n;  // which vector
        const int m0 = (int)w * 4;
        dst[i] = (uint32_t)src[(long long)(m0 + 0) * n + pos]
               | ((uint32_t)src[(long long)(m0 + 1) * n + pos] << 8)
               | ((uint32_t)src[(long long)(m0 + 2) * n + pos] << 16)
               | ((uint32_t)src[(long long)(m0 + 3) * n + pos] << 24);
    }
}
...
JHQGpuIndex::~JHQGpuIndex() {
    ...
    cudaFree(ws_.d_q_bit);
    cudaFree(ws_.d_q_const);
...
void JHQGpuIndex::alloc_workspace(int batch_size) {
    ...
    cudaFree(ws_.d_q_bit);         ws_.d_q_bit          = nullptr;
    cudaFree(ws_.d_q_const);       ws_.d_q_const        = nullptr;
    ...
    CUDA_CHECK(cudaMalloc(&ws_.d_byte_lut,
                          (size_t)B * M_ * JHQ_SPLIT_LUT * sizeof(jhq_gpu::jhq_lut_t)));
#if JHQ_SIGN_IP
    CUDA_CHECK(cudaMalloc(&ws_.d_q_bit,   (size_t)B * d_ * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ws_.d_q_const, (size_t)B      * sizeof(float)));
#endif
...
float* JHQGpuIndex::rotate_on_gpu(const float* h_x, int n, double* sum_sq) const {
    ...
    if (sum_sq) {
        // n * d_ is int * int. At d=1024 it passes 2^31 above 2,097,152 rows
        // and wraps negative; BLAS returns without touching the result for a
        // non-positive length, so ss stays 0, sigma comes out 0, and equation
        // 4 builds every centroid at the origin -- with no error anywhere.
        //
        // That is what the nlist>=32768 sweep hit: at JHQ_N_TRAIN=2,600,000
        // recall fell to 0.64-0.73 while ivf_recall stayed at 0.99, because
        // routing uses the IVF centroids and only the primary level depends
        // on sigma. results/v45_compact/ has both halves of the A/B.
        //
        // Chunked at 2^28 floats, and accumulated in double: 2.6e9 squares
        // summed into one float would lose the tail even if the length fitted.
        const long long total = (long long)n * (long long)d_;
        const long long CH    = 1LL << 28;
        double acc = 0.0;
        for (long long off = 0; off < total; off += CH) {
            const long long rem = total - off;
            const int len = (int)((rem < CH) ? rem : CH);
            float part = 0.f;
            CUBLAS_CHECK(cublasSdot(cublas_, len, d_x + off, 1, d_x + off, 1, &part));
            acc += (double)part;
        }
        *sum_sq = acc;
    }
...
// The residual of a batch, on the device where its inputs already are.
//
// y, the primary codes and the codebook are all on the GPU when this runs, and
// r = y - yhat is one subtraction per value with no dependence between values.
// v25 copied all three back and did it in a host loop -- 4.1 billion
// subtractions single-threaded at n_res=4M, which took 388 s of a 388 s
// "training" phase. That loop was inherited from the host path, where the data
// genuinely was on the host; here it never was.
__global__ void residual_values_kernel(
    const float*   __restrict__ y,     // [nb][gd]
    const uint8_t* __restrict__ code,  // [nb][gm]
    const float*   __restrict__ cent,  // this group's [gm][K][Ds]
    float*                      out,   // [gm][nb][Ds] -- subspace-major
    long long nb, int gd, int gm, int Ds, int K)
{
    const long long total = nb * gd;
    for (long long idx = (long long)blockIdx.x * blockDim.x + threadIdx.x;
         idx < total; idx += (long long)gridDim.x * blockDim.x) {
        const long long i = idx / gd;
        const int       j = (int)(idx - i * gd);
        const int       g = j / Ds, k = j - g * Ds;
        // The transpose to subspace-major happens here, in the store, because
        // the consumer wants each subspace's values contiguous and the host
        // was doing it instead: nb*gm memcpys of Ds floats a batch, single
        // threaded -- 25.6 million 32-byte copies at stella. Same bytes, same
        // count, but the index arithmetic is free and the store coalesces
        // within a subspace.
        out[((size_t)g * nb + i) * Ds + k] =
            y[idx] - cent[((size_t)g * K + code[i * gm + g]) * Ds + k];
    }
}
...
#pragma omp parallel for schedule(dynamic)
    ...
    for (int m = 0; m < M_; ++m) {
        ...
        // _hold is the convergent one: an empty cell keeps its centroid rather
        // than being reseeded into dense data, which is the same rule the
        // device estimator uses. train_1d_kmeans keeps the old rule for the
        // frozen v23, so this calls the new symbol instead of changing it.
        std::vector<float> cb = train_1d_kmeans_hold(resid_m.data(),
                                                     (int)resid_m.size(), Kr_, 2000);
...
{
    // Equation 5 over the whole dataset: one pass, and the device estimator.
    //
    // Two pieces existed and were not joined. launch_residual_codebook already
    // sorts each subspace's values once, prefix-sums them, and then runs Lloyd
    // in O(Kr log n) an iteration instead of O(n log Kr) -- 5120 operations
    // against 6.4 million, with identical results, since sums and counts do not
    // depend on visit order. But it derives the residuals from the rotated set,
    // which has to be resident. The streamed path computed residuals a batch at
    // a time on the device and then handed them to the host estimator, which
    // sorts on the CPU.
    //
    // Joining them needs the residuals somewhere between the pass that produces
    // them and the chunk that consumes them. Holding a chunk means re-reading
    // the base once per chunk -- sixteen passes over 67.8 GB at stella. v36
    // staged all of them to disk instead, which is one base read plus a write
    // and a read of the residuals: 203 GiB of traffic for 10 s of GPU work,
    // and the measured 172 s was almost all of it host-side.
    //
    // So they live in host memory up to a budget and only the excess spills.
    // At stella 67.8 GiB of residuals against a 90 GB cgroup is affordable
    // whole; on a smaller machine the tail goes to disk and the traffic
    // degrades smoothly rather than the run failing.
    // The budget comes from the cgroup this process is in, not a constant: the
    // 90 GB box and a 16 GB one are the same code path, and the difference has
    // to appear as more spilling rather than as an OOM. Three quarters of it,
    // because the base is mapped and its page cache -- reclaimable, but counted
    // -- is what the remaining quarter absorbs.
    const size_t per_sub_b = (size_t)n * Ds_ * sizeof(float);
    size_t budget = 0;
    for (const char* cg : { "/sys/fs/cgroup/memory.max",
                            "/sys/fs/cgroup/memory/memory.limit_in_bytes" }) {
        if (FILE* g = std::fopen(cg, "r")) {
            char buf[64] = {0};
            if (std::fgets(buf, sizeof buf, g) && buf[0] >= '0' && buf[0] <= '9') {
                const unsigned long long lim = std::strtoull(buf, nullptr, 10);
                // cgroup v1 writes a sentinel near 2^63 when it is unlimited.
                if (lim > 0 && lim < (1ULL << 62)) budget = (size_t)(lim / 4 * 3);
            }
            std::fclose(g);
            if (budget) break;
        }
    }
    if (budget == 0) budget = 32ULL << 30;
    if (const char* hb = std::getenv("JHQ_RES_HOST_GB"))
        budget = (size_t)(std::atof(hb) * 1073741824.0);
    const int Mh = (int)std::min((size_t)M_, budget / std::max<size_t>(1, per_sub_b));
    std::vector<float> h_all((size_t)Mh * n * Ds_);

    const char* dir = std::getenv("JHQ_RES_SPILL");
    const std::string path = std::string(dir ? dir : "/root/autodl-tmp")
                           + "/jhq_resid_spill.bin";
    ...
    if (Mh < M_) {
        ...
        if (!f) throw std::runtime_error("cannot open residual spill file: " + path);
    }
    std::fprintf(stderr, "[res-train] %d of %d subspaces in host memory "
                 "(%.1f GiB), %d spilled (%.1f GiB)\n",
                 Mh, M_, (double)Mh * per_sub_b / 1073741824.0,
                 M_ - Mh, (double)(M_ - Mh) * per_sub_b / 1073741824.0);
    ...
    CUDA_CHECK(cudaMalloc(&d_y,    (size_t)batch * d_ * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_code, (size_t)batch * M_));
    CUDA_CHECK(cudaMalloc(&d_res,  (size_t)batch * d_ * sizeof(float)));
    // Pinned, so the copy out of the device runs at PCIe speed instead of
    // through CUDA's own single-threaded staging of pageable memory.
    float* h_stage = nullptr;
    CUDA_CHECK(cudaHostAlloc(&h_stage, (size_t)batch * d_ * sizeof(float),
    ...
    for (int off = 0; off < n; off += batch) {
        ...
        launch_primary_encode(d_y, d_code, d_cent_, nb, d_, M_, Ds_, K_);
        {
            const long long tot = (long long)nb * d_;
            const int blk = 256;
            const int grid = (int)std::min((tot + blk - 1) / blk, 65535LL);
            residual_values_kernel<<<grid, blk>>>(d_y, d_code, d_cent_, d_res,
                                                  nb, d_, M_, Ds_, K_);
            CUDA_CHECK(cudaGetLastError());
        }
        CUDA_CHECK(cudaMemcpy(h_stage, d_res,
                              (size_t)nb * d_ * sizeof(float), cudaMemcpyDeviceToHost));
        // The kernel already wrote subspace-major, so a subspace's nb*Ds values
        // are contiguous at both ends: M copies of nb*Ds floats, in parallel,
        // where v36 did nb*M copies of Ds floats on one thread.
#ifdef _OPENMP
#pragma omp parallel for schedule(static)
#endif
        for (int m = 0; m < Mh; ++m)
            std::memcpy(h_all.data() + ((size_t)m * n + off) * Ds_,
                        h_stage + (size_t)m * nb * Ds_,
                        (size_t)nb * Ds_ * sizeof(float));
        // The tail, if any, goes to disk in the same subspace-major layout,
        // renumbered from Mh so the file holds only what spilled.
        for (int m = Mh; m < M_; ++m) {
            const long long pos =
                ((long long)(m - Mh) * n + off) * Ds_ * (long long)sizeof(float);
            if (std::fseek(f, pos, SEEK_SET) ||
                std::fwrite(h_stage + (size_t)m * nb * Ds_, sizeof(float),
                            (size_t)nb * Ds_, f) != (size_t)nb * Ds_) {
                std::fclose(f); std::remove(path.c_str());
                throw std::runtime_error("residual spill write failed; is there room on "
                                         + path + "?");
            }
        }
    }
    cudaFree(d_x); cudaFree(d_y); cudaFree(d_code); cudaFree(d_res);
    cudaFreeHost(h_stage);
    std::fprintf(stderr, "[res-train] one pass over %d vectors\n", n);

    // Hand the estimator a chunk of subspaces at a time. A chunk that lives in
    // host memory is copied straight to the device; one past Mh is read from
    // the spill file first. Chunks never straddle Mh, so there is one source
    // per chunk and no seam to get wrong.
    // C is derived, not fixed at 8. A chunk costs C*n*Ds*20 bytes on the
    // device -- the values, the sorted copy, the double prefix sum, and cub's
    // temporary -- and Ds = d/M, so a fixed C grows as M falls. stella at
    // M=64, which the paper's 1024-d sweep includes, wants 33.9 GiB against
    // 31.4 of card, and its C*n*Ds = 2.28e9 overruns the 32-bit segment
    // offsets cub sorts with. Smaller chunks cost kernel launches and nothing
    // else: each reads its own contiguous slice of the spill file, so the
    // total read is M*n*Ds = n*d whatever C is.
    const long long per_sub = (long long)n * Ds_;
    long long C64 = std::min<long long>((16LL << 30) / (per_sub * 20), M_);
    C64 = std::min<long long>(C64, 2147483647LL / per_sub);
    int C = (int)std::max(1LL, C64);
    if (const char* ce = std::getenv("JHQ_RES_CHUNK")) {  // reproduces a
        C = std::atoi(ce);                                // measured chunking
        if (C < 1) C = 1;
        if (C > M_) C = M_;
    }
    std::fprintf(stderr, "[res-train] %d of %d subspaces per chunk, "
                 "%.1f GiB on device\n", C, M_,
                 (double)C * per_sub * 20.0 / 1073741824.0);
    res_c1d_.assign((size_t)M_ * Kr_, 0.f);
    if (!d_res_c1d_)
        CUDA_CHECK(cudaMalloc(&d_res_c1d_, (size_t)M_ * Kr_ * sizeof(float)));

    float* d_chunk = nullptr;
    CUDA_CHECK(cudaMalloc(&d_chunk, (size_t)C * n * Ds_ * sizeof(float)));
    std::vector<float> h_chunk;
    if (Mh < M_) h_chunk.resize((size_t)C * n * Ds_);
    for (int m0 = 0; m0 < M_; ) {
        int cm = std::min(C, M_ - m0);
        if (m0 < Mh && m0 + cm > Mh) cm = Mh - m0;      // never straddle Mh
        const float* src;
        if (m0 < Mh) {
            src = h_all.data() + (size_t)m0 * n * Ds_;
        } else {
            const long long pos =
                (long long)(m0 - Mh) * n * Ds_ * (long long)sizeof(float);
            std::fseek(f, pos, SEEK_SET);
            if (std::fread(h_chunk.data(), sizeof(float), (size_t)cm * n * Ds_, f)
                    != (size_t)cm * n * Ds_) {
                std::fclose(f); std::remove(path.c_str());
                throw std::runtime_error("residual spill read failed");
            }
            src = h_chunk.data();
        }
        staged_h2d(d_chunk, src, (size_t)cm * n * Ds_ * sizeof(float));
        launch_residual_codebook_from_values(d_chunk, (long long)n * Ds_, cm, Kr_, 2000,
                                             d_res_c1d_ + (size_t)m0 * Kr_);
        CUDA_CHECK(cudaDeviceSynchronize());
        m0 += cm;
    }
    cudaFree(d_chunk);
    if (f) { std::fclose(f); std::remove(path.c_str()); }
...
std::string JHQGpuIndex::cache_path(const char* dir, const float* h_x,
    ...
    // n_res_train belongs here. It changes the residual codebook and nothing
    // else in the key moves with it, so a 100K run and an ALL run hash the
    // same and the second silently loads the first's codebook -- a sensitivity
    // sweep over exactly that parameter would be comparing a result against
    // itself. The sweep already run happened not to use the cache, which was
    // luck rather than design.
    //
    // So do the residual Lloyd's own parameters, for exactly the same reason
    // and with a sharper edge: results/v34_lloyd_iterations/ is a sweep over
    // the iteration cap that moves recall by 7e-3, and run with a cache
    // directory set it would have loaded the first cap's codebook nine times
    // and reported it as ten measurements. The seed likewise -- 91% of the
    // residual codes change with it (results/v31_init_spread/).
    const int res_iters = std::getenv("JHQ_RES_MAX_ITER")
                        ? std::atoi(std::getenv("JHQ_RES_MAX_ITER")) : 2000;
    const int res_seed  = std::getenv("JHQ_RES_SEED")
                        ? std::atoi(std::getenv("JHQ_RES_SEED")) : 0;
    const int params[] = { d_, M_, B_, Br_, K_, Kr_, nlist_,
                           ivf_iters_, kmeans_iters_, seed_, n_train,
                           codebook_path, res_train_n_, res_iters, res_seed };
    mix(params, sizeof params);
    // A float, so it cannot ride in the int array above.
    const float res_tol = std::getenv("JHQ_RES_TOL")
                        ? (float)std::atof(std::getenv("JHQ_RES_TOL")) : 1e-6f;
    mix(&res_tol, sizeof res_tol);
...
void JHQGpuIndex::train(const float* h_x, int n_train,
    ...
    // Set before cache_path() runs, so the key sees it.
    res_train_n_ = (h_res_x && n_res_train > 0) ? n_res_train : 0;
    ...
    if (cache_dir) {
        ...
        if (load_trained(cpath)) {
            ...
            upload_trained();
#if JHQ_SIGN_IP
            // A cache hit skips the fresh-train call, and the two constants
            // live only in memory. Without this every distance is computed
            // from a=0 and recall reads as a search bug.
            verify_sign_levels();
#endif
    ...
    if (gpu_codebook) {
        ...
        if (!d_res_c1d_)
            CUDA_CHECK(cudaMalloc(&d_res_c1d_, (size_t)M_ * Kr_ * sizeof(float)));
        // 2000 iterations, not a converged one: exact convergence is not
        // reachable here. Sparse tail bins two-cycle -- a bin of tens of values
        // shifts its mean by 6e-5 when one value crosses its boundary, which
        // moves the boundary back -- so v32's exact test and v33's relative one
        // both ran to their caps. 2000 is where recall plateaus: +7.6e-3,
        // +7.1e-3 and +6.3e-3 over the 25 used through v31, for +22, +30 and
        // +20 ms, and 2000 through 40000 all fall inside repeat-run noise.
        // results/v34_lloyd_iterations/ has the sweep.
        //
        // One estimator: exact 1-D Lloyd off a segmented sort. The histogram
        // estimator this replaces trades the sort for one O(n) pass and places
        // values by bucket; it was measured at 2048 and 8192 buckets and keeps
        // its own directory, jhq_v29_hist_wide.
        if (streamed) {
            // Residuals are computed in one pass and spilled to disk, then read
            // back a chunk of subspaces at a time and handed to the same device
            // estimator -- so the rotated set never has to be resident.
            ...
            launch_residual_codebook(d_y_res, d_codes_res, d_cent_,
                                     n_res, d_, M_, Ds_, K_, Kr_, 2000, d_res_c1d_);
            CUDA_CHECK(cudaMemcpy(res_c1d_.data(), d_res_c1d_,
                                  (size_t)M_ * Kr_ * sizeof(float), cudaMemcpyDeviceToHost));
...
// ── The factorised LUT's precondition, checked rather than assumed ───────────
//
// T_m[c] = Thi_m[c>>4] + Tlo_m[c&15] holds only because equation 4 builds the
// K codewords as a Cartesian product of per-dimension levels, so the high
// nibble determines the first Ds/2 coordinates of the centroid and the low
// nibble the rest. That is a property of how the codebook was constructed, and
// a Lloyd-refined product quantiser (JHQ_PAPER_CODEBOOK=0) does not have it --
// its centroids move independently and the identity silently stops holding.
//
// So it is verified against the centroids themselves, not inferred from which
// construction was requested: for every subspace and code, the first half of
// the centroid must equal that of code (c & ~15) << 0 -- i.e. (c>>4)<<4 -- and
// the second half must equal that of code (c & 15). O(M*K*Ds) once at train
// time, about 25 M comparisons at M=384, against a wrong answer on every
// query if it does not hold.
void JHQGpuIndex::verify_split_lut() const {
    if (K_ != 256)
        throw std::runtime_error(
            "v47's factorised LUT needs K=256 (B=8); got K=" + std::to_string(K_));
    if ((Ds_ & 1) != 0)
        throw std::runtime_error(
            "v47's factorised LUT needs an even Ds so the nibbles split the "
            "dimensions; got Ds=" + std::to_string(Ds_));
    const int H = Ds_ >> 1;
    for (int m = 0; m < M_; ++m) {
        const float* base = cb_->centroids(m);           // [K][Ds]
        for (int c = 0; c < K_; ++c) {
            const float* cc = base + (size_t)c * Ds_;
            const float* hi = base + (size_t)((c >> 4) << 4) * Ds_;
            const float* lo = base + (size_t)(c & 15) * Ds_;
            for (int j = 0; j < H; ++j)
                if (cc[j] != hi[j])
                    throw std::runtime_error(
                        "v47: the primary codebook is not a Cartesian product "
                        "-- subspace " + std::to_string(m) + ", code " +
                        std::to_string(c) + ", dim " + std::to_string(j) +
                        ". The factorised LUT would be wrong. Use v46 for a "
                        "Lloyd-refined product quantiser.");
            for (int j = H; j < Ds_; ++j)
                if (cc[j] != lo[j])
                    throw std::runtime_error(
                        "v47: the primary codebook is not a Cartesian product "
...
// ── JHQ_SIGN_IP's precondition, checked rather than assumed ─────────────────
//
// D_P = ||q'||^2 + d*a^2 - 2a<q', s_y> holds only if every centroid coordinate
// is exactly +a or -a. That is what equation 3 gives at L = 2, and L is
// 2^(B/Ds), so it needs Ds = B = 8. A Lloyd-refined product quantiser
// (JHQ_PAPER_CODEBOOK=0) moves its centroids off the levels and the identity
// stops holding silently, so the centroids themselves are what is checked.
//
// The same pass reads off the encoder's bit order. Code 1<<p differs from code
// 0 in exactly one coordinate, and that coordinate is the dimension bit p
// carries; nothing here assumes it is p itself.
void JHQGpuIndex::verify_sign_levels() {
    if (K_ != 256)
        throw std::runtime_error(
            "v54's sign inner product needs K=256 (B=8); got K=" + std::to_string(K_));
    if (Ds_ != 8)
        throw std::runtime_error(
            "v54's sign inner product needs L=2, i.e. Ds=B=8; got Ds=" +
            std::to_string(Ds_) + ". At Ds != 8 equation 4 has more than two "
            "levels a dimension and the primary distance is not an inner "
            "product. Use v53.");

    const float* base0 = cb_->centroids(0);          // [K][Ds]
    // Code 0 has every bit clear, so every coordinate is the low level.
    const float a = -base0[0];
    if (!(a > 0.0f))
        throw std::runtime_error(
            "v54: centroid 0 of subspace 0 is " + std::to_string(base0[0]) +
            "; the low level must be negative for symmetric +/-a levels.");
    for (int j = 1; j < 8; ++j)
        if (base0[j] != -a)
            throw std::runtime_error(
                "v54: code 0 of subspace 0 is not uniform -- dim " +
                std::to_string(j) + " is " + std::to_string(base0[j]) +
                ", expected " + std::to_string(-a) + ".");

    int dim_of_bit[8];
    for (int p = 0; p < 8; ++p) {
        dim_of_bit[p] = -1;
        const float* cc = base0 + (size_t)(1u << p) * 8;
        for (int j = 0; j < 8; ++j) {
            if (cc[j] == a) {
                if (dim_of_bit[p] >= 0)
                    throw std::runtime_error(
                        "v54: code " + std::to_string(1u << p) + " raises more "
                        "than one dimension; the code is not one bit a dimension.");
                dim_of_bit[p] = j;
            } else if (cc[j] != -a) {
                throw std::runtime_error(
                    "v54: centroid coordinate " + std::to_string(cc[j]) +
                    " is neither +a nor -a (a=" + std::to_string(a) + ").");
            }
        }
        if (dim_of_bit[p] < 0)
            throw std::runtime_error(
                "v54: code " + std::to_string(1u << p) + " equals code 0; the "
                "codebook has fewer than 8 distinct bits.");
    }

    // The full check: O(M*K*Ds), about 786 K comparisons at M=384, once at
    // train time, against a wrong distance on every candidate if it does not
    // hold. One shared a across every subspace is part of the claim -- the
    // levels are a closed form in one global sigma, so they must agree.
    for (int m = 0; m < M_; ++m) {
        const float* bm = cb_->centroids(m);
        for (int c = 0; c < 256; ++c) {
            const float* cc = bm + (size_t)c * 8;
            for (int p = 0; p < 8; ++p) {
                const float want = ((c >> p) & 1) ? a : -a;
                if (cc[dim_of_bit[p]] != want)
                    throw std::runtime_error(
                        "v54: subspace " + std::to_string(m) + ", code " +
                        std::to_string(c) + ", bit " + std::to_string(p) +
                        " gives " + std::to_string(cc[dim_of_bit[p]]) +
                        " where the +/-a form needs " + std::to_string(want) +
                        ". The primary codebook is not two symmetric levels a "
                        "dimension; use v53.");
            }
        }
    }
    ...
    sign_bitmap_ = 0;
    for (int p = 0; p < 8; ++p)
        sign_bitmap_ |= (unsigned)dim_of_bit[p] << (3 * p);
}
...
// jhq_v54_sign_ip/jhq_gpu_index.cuh's Params::add_batch comment for
...
#if JHQ_WORD_LAYOUT
    // Both build paths have written [M][n] bytes by now. Rearrange once, into
    // the same number of bytes, and drop the old buffer. M is a multiple of 4
    // in every configuration here; anything else keeps the byte layout, since
    // the scan would read past the end of a partial word.
    if ((M_ & 3) == 0 && n > 0) {
        uint32_t* d_words = nullptr;
        const size_t bytes = (size_t)M_ * (size_t)n;   // same size, different shape
        CUDA_CHECK(cudaMalloc(&d_words, bytes));
        const long long total = (long long)(M_ / 4) * n;
        const int grid = (int)std::min((total + 255) / 256, (long long)65535);
        repack_primary_words_kernel<<<grid, 256>>>(d_list_primary_t_, d_words, (long long)n, M_);
        CUDA_CHECK(cudaGetLastError());
        CUDA_CHECK(cudaDeviceSynchronize());
        cudaFree(d_list_primary_t_);
        d_list_primary_t_ = reinterpret_cast<uint8_t*>(d_words);
    } else if ((M_ & 3) != 0) {
        throw std::runtime_error(
            "JHQ_WORD_LAYOUT packs four subspaces to a 32-bit word and needs M "
            "divisible by 4; got M=" + std::to_string(M_));
    }
#endif
...
// ── Search ────────────────────────────────────────────────────────────────────
void JHQGpuIndex::vector_lists(std::vector<int>& out) const {
    out.assign((size_t)ntotal_, -1);
    if (ntotal_ == 0 || !d_list_offsets_ || !d_list_ids_) return;
    std::vector<int> off((size_t)nlist_ + 1), ids((size_t)ntotal_);
    CUDA_CHECK(cudaMemcpy(off.data(), d_list_offsets_,
                          off.size() * sizeof(int), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(ids.data(), d_list_ids_,
                          ids.size() * sizeof(int), cudaMemcpyDeviceToHost));
    for (int l = 0; l < nlist_; ++l)
        for (int p = off[l]; p < off[l + 1]; ++p)
            if (ids[p] >= 0 && ids[p] < ntotal_) out[ids[p]] = l;
}
