#include "hblock_v43/jhq_gpu_index.cuh"
#include "hblock_v17/encode.cuh"
#include "hblock_v27/search.cuh"   // for gpu_build_block_adj_v27
#include "hblock_v43/search.cuh"   // for align4() and the region-indirected leaf kernel
...
// mmap-backed region files (POSIX; this project targets Linux GPU boxes).
#include <fcntl.h>
#include <sys/mman.h>
#include <sys/stat.h>
#include <unistd.h>
...
HBlockIndex::HBlockIndex(int d, Params p)
    : d_(d), d_proj_(p.d_proj), Kr_(p.Kr), Br_(p.Br),
      ...
      region_file_dir_(p.region_file_dir)
{
    ...
    if (p.region_file_dir.empty()) throw std::invalid_argument("region_file_dir must be non-empty");
    code_region_path_ = region_file_dir_ + "/hblock_v43_" + std::to_string((long long)getpid()) + "_codes.region";
    raw_region_path_  = region_file_dir_ + "/hblock_v43_" + std::to_string((long long)getpid()) + "_raw.region";
    CUBLAS_CHECK(cublasCreate(&cublas_));
}

HBlockIndex::~HBlockIndex()
{
    if (code_region_map_) munmap((void*)code_region_map_, code_region_map_bytes_);
    if (raw_region_map_)  munmap((void*)raw_region_map_,  raw_region_map_bytes_);
    if (code_region_fd_ >= 0) close(code_region_fd_);
    if (raw_region_fd_  >= 0) close(raw_region_fd_);
...
void HBlockIndex::build_region_layout(
    ...
    int total_blocks)
{
    // Writes `buf` to `path`, then mmaps it PROT_READ so fetch_code_regions
    // ()/fetch_raw_regions() can read region bytes straight out of the OS
    // page cache instead of a resident std::vector. Any previous mapping
    // for this fd/path (from an earlier add() on the same index -- not
    // expected given "one add() call" below, but defensive) is torn down
    // first.
    auto write_and_map = [](const std::vector<uint8_t>& buf, const std::string& path,
                             int& fd, const uint8_t*& map, size_t& map_bytes) {
        if (map) { munmap((void*)map, map_bytes); map = nullptr; }
        if (fd >= 0) { close(fd); fd = -1; }
        map_bytes = 0;

        FILE* f = std::fopen(path.c_str(), "wb");
        if (!f) throw std::runtime_error("build_region_layout: cannot create " + path);
        size_t written = buf.empty() ? 0 : std::fwrite(buf.data(), 1, buf.size(), f);
        std::fclose(f);
        if (written != buf.size())
            throw std::runtime_error("build_region_layout: short write to " + path);
        if (buf.empty()) return;

        fd = open(path.c_str(), O_RDONLY);
        if (fd < 0) throw std::runtime_error("build_region_layout: cannot open " + path + " for mmap");
        void* m = mmap(nullptr, buf.size(), PROT_READ, MAP_SHARED, fd, 0);
        if (m == MAP_FAILED) {
            close(fd); fd = -1;
            throw std::runtime_error("build_region_layout: mmap failed for " + path);
        }
        map = (const uint8_t*)m;
        map_bytes = buf.size();
    };

    // ── Graph-aware pack order: BFS over the block adjacency graph
    // (h_block_adj_, already built by build_block_graph() before this
    // function runs -- see add()) instead of physical block-id order.
    // Blocks discovered close together in this BFS are graph-neighbors --
    // likely to be visited together within the same query's beam search --
    // and now tend to land in the same or nearby regions. Every block
    // appears exactly once (either as some BFS's start, or discovered as
    // someone else's neighbor), so pack_order is a permutation of
    // [0, total_blocks).
    std::vector<int> pack_order;
    pack_order.reserve(total_blocks);
    {
        std::vector<char> visited(total_blocks, 0);
        std::vector<int> frontier;
        frontier.reserve(total_blocks);
        int n_components = 0;
        for (int start = 0; start < total_blocks; start++) {
            ...
            while (head < frontier.size()) {
                ...
                pack_order.push_back(cur);
                for (int d = 0; d < graph_degree_; d++) {
                    int nb = h_block_adj_[(long long)cur * graph_degree_ + d];
                    if (nb >= 0 && nb < total_blocks && !visited[nb]) {
                        visited[nb] = 1;
        ...
        // Many components means the block graph is fragmented -- BFS order
        // only gives locality *within* a component; different components
        // still end up in arbitrary relative order (start-block-id order).
        // A single (or a few) large component(s) is the case this packing
        // actually helps.
        printf("  [region layout] graph BFS: %d connected component(s) over %d blocks\n",
               n_components, total_blocks);
    }

    // ── Code regions: pack each block's (codes + vector ids) contiguously,
    // filling regions up to region_bytes_ before starting a new one, in
    // pack_order (not physical block-id order -- see above).
    ...
    h_block_code_offset_.assign(total_blocks, -1);
    {
        ...
        int cur_region = -1;
        for (int pi = 0; pi < total_blocks; pi++) {
            int b = pack_order[pi];
        ...
        write_and_map(code_store, code_region_path_, code_region_fd_,
                      code_region_map_, code_region_map_bytes_);
    }
    ...
    h_block_raw_offset_.assign(total_blocks, -1);
    {
        ...
        write_and_map(raw_store, raw_region_path_, raw_region_fd_,
                      raw_region_map_, raw_region_map_bytes_);
    }

    printf("  [region layout] %d code regions (%.2f GB) -> %s, %d raw regions (%.2f GB) -> %s, region_bytes=%d\n",
           n_code_regions_, (double)code_region_map_bytes_/1e9, code_region_path_.c_str(),
           n_raw_regions_,  (double)raw_region_map_bytes_/1e9,  raw_region_path_.c_str(),
           region_bytes_);
...
long long HBlockIndex::fetch_code_regions(const std::vector<int>& needed_region_ids) const
{
    ...
    for (int region : uniq) {
        ...
        // Reads straight out of the mmap'd file (OS page cache brings the
        // page in from disk on first touch if it isn't already resident --
        // this is the actual out-of-core behavior v42 adds over v39).
        CUDA_CHECK(cudaMemcpyAsync(
            d_code_region_pool_ + (long long)slot * region_bytes_,
            code_region_map_ + (long long)region * region_bytes_,
...
long long HBlockIndex::fetch_raw_regions(const std::vector<int>& needed_region_ids) const
{
    ...
    for (int region : uniq) {
        ...
        CUDA_CHECK(cudaMemcpyAsync(
            ...
            raw_region_map_ + (long long)region * region_bytes_,
            region_bytes_, cudaMemcpyHostToDevice, s));
        bytes_moved += region_bytes_;
