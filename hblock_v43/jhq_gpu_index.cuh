...
// HBlock v43: v42 (mmap'd region files) + graph-aware region packing.
//
// v39/v42 packed blocks into regions in physical/(c1,c2,c3)-cell-
// lexicographic order. That measurably failed to capture locality: a
// query's beam search follows the block graph's cross-cell edges (added
// specifically so routing isn't limited to lexicographically-nearby
// cells), so graph-adjacent blocks a query actually visits together were
// frequently numbered far apart and landed in different regions. Measured
// on SPACEV: 57-91% of all regions touched per batch at batch=64, only
// dropping to 11-33% at batch=8 -- see results/hblock_v39_findings.md.
//
// v43 fixes the actual cause instead of the batch-size workaround: blocks
// are assigned to regions in **block-adjacency-graph BFS order** (using
// h_block_adj_, already built by build_block_graph() before
// build_region_layout() runs) instead of physical block-id order. Blocks
// that are graph-neighbors -- and therefore likely to be visited together
// by the same query's beam search -- now tend to land in the same or
// nearby regions. This changes region *packing* only: the region_id ->
// pool-slot indirection, GPU pool, fetch planning, and mmap'd file
// storage are all unchanged from v42.
//
// Same tree routing + block graph + per-block fixed top-16 exact rerank as
// v36/v38/v39/v42 -- this version changes region packing order only, not
// recall semantics (every block still lands in exactly one region; the
// kernel's region_id -> pool-slot addressing doesn't care what order
// blocks were assigned in).
...
public:
    struct Params {
        ...
        // during add() (hblock_v43_<pid>_codes.region / _raw.region).
