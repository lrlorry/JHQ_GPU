...
void route_gpu_v29(
    ...
    SearchWorkspace& ws)
{
    if (nq > ws.batch_cap) throw std::runtime_error("hblock_v29: nq > batch_cap");
    ...
    int entry_per_cell, int bitmap_words)
{
            ...
            for (int i3 = 0; i3 < ck3; i3++) {
                ...
                for (int start = 0; start < bc; start += 32) {
                    ...
                    for (int ep = 0; ep < entry_per_cell; ep++) {
                        ...
                        bool fv = false;
                        if (tid == 0 && ins_id >= 0) fv = try_visit_v29(vis, ins_id, n_blks);
    ...
    for (int iter = 0; iter < depth; iter++) {
        ...
        if (tid < degree) {
            ...
            if (try_visit_v29(vis, nb, n_blks)) {
