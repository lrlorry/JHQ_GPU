#include "jhq_v10_bytelut/jhq_gpu_index.cuh"
#include "jhq_v10_bytelut/encode.cuh"
...
void JHQGpuIndex::alloc_workspace(int batch_size) {
    ...
    // Byte LUT: B × M × 256 entries — replaces the [M][Ds][K1D] primary LUT.
    // 1000 × 96 × 256 × 4 = ~94 MB; eliminates the 16-way bank conflict in scan.
    ...
    // Pinned host staging buffer: avoids per-call cudaMalloc + blocking H2D.
...
{
    if (n_train < nlist_)
        throw std::invalid_argument("n_train must be >= nlist for v10_bytelut");
...
void JHQGpuIndex::add(const float* h_x, int n) {
    ...
    if (ntotal_ != 0)
        throw std::runtime_error("v10_bytelut currently supports one add() call");
...
void JHQGpuIndex::search(const float* h_q, int nq, int k,
    ...
    // No cudaMalloc/cudaFree — h_q is staged via pinned buffer inside search_gpu.
    search_gpu(cublas_,
