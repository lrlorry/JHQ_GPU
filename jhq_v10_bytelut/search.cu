#include "jhq_v10_bytelut/search.cuh"
...
// ── select_probes_kernel (same as v7) ─────────────────────────────────────────
...
// ── build_byte_lut_kernel (NEW) ───────────────────────────────────────────────
// Replaces build_primary_lut_batched_kernel.
// Precomputes byte_lut[bqi][m][byte_val] = sum_{k=0}^{Ds-1}
//   (q_rot[bqi, m*Ds+k] - c1d[(byte_val >> (k*bpd)) & (K1D-1)])^2
//
// Eliminates the M*Ds=768 inner-loop iterations + 16-way bank conflicts
// in scan, replacing them with M=96 direct byte-indexed lookups.
__global__ void build_byte_lut_kernel(
    const float* __restrict__ d_q_rot,    // [B, d]
    const float* __restrict__ d_c1d,      // [K1D]
    float*                    d_byte_lut, // [B, M, 256]
    int B, int d, int M, int Ds, int K1D, int bits_per_dim)
{
    // Each thread fills one (bqi, m, byte_val) entry.
...
// ── scan_ivf_bytelut_kernel (NEW) ─────────────────────────────────────────────
// Replaces scan_ivf_batched_topk_kernel.
// Inner loop: M=96 lookups into d_byte_lut (L2-cached, no bank conflicts).
// list_primary read: 4-byte aligned uint32 — 4 bytes per load instead of 1.
    ...
    int nprobe, int M, int ck)
{
    ...
    // Shared memory: no LUT needed — only reduction buffers.
    // 10KB total vs 16KB in v7 → 100% SM occupancy.
    ...
    // Per-query byte LUT base pointer (L2-cached across all candidates)
    ...
    for (int local_t = tid; local_t < total; local_t += BLOCK) {
        ...
        // Read M codes using uint32 (4-byte aligned, M divisible by 4).
        // 4× fewer global loads vs byte-by-byte.
...
// ── build_residual_lut_batched_kernel (same as v7) ───────────────────────────
...
// ── residual_refine_batched_kernel (same as v7) ──────────────────────────────
...
// ── batched_topk_final_kernel (same as v7) ───────────────────────────────────
    ...
    int ck, int k)
{
    ...
    const int   BLOCK = 256;
    // Scan smem: no LUT — only reduction buffers (10KB vs 16KB in v7)
...
{
    if (ws.batch_cap <= 0)
        throw std::runtime_error("v10: workspace not initialised — call add() first");
    if (!ws.stream || !ws.h_q_pinned)
        throw std::runtime_error("v10: CUDA stream / pinned buffer not created");
