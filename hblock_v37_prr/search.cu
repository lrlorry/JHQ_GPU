...
// PRR = certified PQ racing.
// Adds three new search modes on top of v36's kernels:
//   FIXED_PER_BLOCK   — unchanged v36 LUT path
//   CORRECTED_FIXED   — per-block correct PQ using block's own cell centroid
//   CERTIFIED_PRR     — corrected PQ + L2/U2 interval pruning + exact rerank for survivors
...
// ══════════════════════════════════════════════════════════════════════════════
// Block beam search (identical to v36, new namespace)
// ══════════════════════════════════════════════════════════════════════════════
...
// ── Routing kernels (identical to v36, new namespace) ─────────────────────────
...
void route_gpu_v29(
    ...
    SearchWorkspace& ws)
{
    if (nq > ws.batch_cap) throw std::runtime_error("hblock_v37_prr: nq > batch_cap");
...
// ── v29 leaf kernel (identical to v36) ───────────────────────────────────────
...
// ══════════════════════════════════════════════════════════════════════════════
// CORRECTED_FIXED mode: per-block correct PQ distance using block's own centroid
// ══════════════════════════════════════════════════════════════════════════════

// Shared memory layout (all via extern char):
//   s_dist[leaf_size]     — PQ distances (sorted by bitonic)
//   s_id[leaf_size]       — vector IDs
//   s_q[d]                — query vector (also used for exact L2)
//   s_cent[d]             — cell abs centroid
//   s_wsum[4]             — warp reduction
//   s_sel[per_block_r]    — selected candidate IDs
//   s_exact[per_block_r]  — exact L2 distances
__global__ void leaf_corrected_pq_kernel(
    const int*     __restrict__ d_pair_leaf_ids,
    const int*     __restrict__ d_pair_qids,
    const int*     __restrict__ leaf_sizes,
    const uint8_t* __restrict__ leaf_codes,
    ...
    int per_block_r, int klocal)
{
    ...
    float* s_dist  = (float*)shm;
    int*   s_id    = (int*)(s_dist + leaf_size);
    float* s_q     = (float*)(s_id + leaf_size);
    float* s_cent  = s_q + d;
    float* s_wsum  = s_cent + d;
    int*   s_sel   = (int*)(s_wsum + 4);
    float* s_exact = (float*)(s_sel + per_block_r);

    // Load query and centroid into shared memory cooperatively
    const float* q_ptr    = d_q_batch   + (long long)qid     * d;
    ...
    // Compute corrected PQ distance for this thread's position
    const uint8_t* leaf_base = leaf_codes + (long long)leaf_blk * bpv * leaf_size;
    float my_dist = INF; int my_id = -1;
    ...
    // Bitonic sort within each warp (32-element sort per warp)
    for (int k = 2; k <= 32; k <<= 1) {
        bool asc = ((lane & k) == 0);
        for (int j = k >> 1; j > 0; j >>= 1) {
            float od = __shfl_xor_sync(FULL, my_dist, j);
    ...
    // Select top per_block_r candidates across all warps
    if (tid == 0) {
        int ptrs[4] = {0, 32, 64, 96};
        int actual_r = (per_block_r < n_vecs) ? per_block_r : n_vecs;
        for (int r = 0; r < actual_r; r++) {
    ...
    // Exact L2 for top candidates (all 128 threads collaborate)
    for (int r = 0; r < per_block_r; r++) {
        int cand_id = s_sel[r];
        float partial = 0.f;
        if (cand_id >= 0) {
...
void launch_leaf_corrected_pq(
    ...
    cudaStream_t stream)
{
    // smem: s_dist[leaf_size] + s_id[leaf_size] + s_q[d] + s_cent[d] + s_wsum[4]
    //       + s_sel[per_block_r] + s_exact[per_block_r]
    int smem = leaf_size * (int)(sizeof(float) + sizeof(int))
             + 2 * d     * (int)sizeof(float)
             + 4         * (int)sizeof(float)
             + per_block_r * (int)(sizeof(int) + sizeof(float));
    leaf_corrected_pq_kernel<<<n_pairs, leaf_size, smem, stream>>>(
        d_pair_leaf_ids, d_pair_qids,
        d_leaf_sizes, d_leaf_codes, d_leaf_ids_data,
        d_block_cell_id, d_abs_cents, d_fine_c1d,
...
// ══════════════════════════════════════════════════════════════════════════════
// CERTIFIED_PRR Pass A: L2/U2 interval kernel
// ══════════════════════════════════════════════════════════════════════════════

// Shared memory:
//   s_q[d]         — query
//   s_cent[d]      — cell centroid
//   s_u2[leaf_size] — U2 values for top-k selection
__global__ void leaf_prr_interval_kernel(
    const int*     __restrict__ d_pair_leaf_ids,
    const int*     __restrict__ d_pair_qids,
    const int*     __restrict__ leaf_sizes,
    const uint8_t* __restrict__ leaf_codes,
    ...
    float*         d_prr_l2,
    float*         d_prr_u2topk,
    int d, int Kr, int Br, int bpv, int leaf_size, int klocal)
{
    const float INF = 1e38f;
    const int pi   = blockIdx.x;
    const int tid  = threadIdx.x;
    ...
    // Compute corrected PQ dist and interval for this thread's position
    const uint8_t* leaf_base = leaf_codes + (long long)leaf_blk * bpv * leaf_size;
    ...
    if (tid < n_vecs) {
        ...
        // Fetch epsilon for this position
    ...
    // Write L2 bound
    d_prr_l2[(long long)pi * leaf_size + tid] = L2;

    // Collect U2 in shared for top-k selection
    s_u2[tid] = U2;
    __syncthreads();

    // Thread 0 finds top-klocal U2 values
    if (tid == 0) {
        float topk[16]; // klocal <= 16
        for (int r = 0; r < klocal; r++) topk[r] = INF;
        for (int pos = 0; pos < n_vecs; pos++) {
            float u = s_u2[pos];
            // Find max slot in topk
            float mx = topk[0]; int mi = 0;
            for (int r = 1; r < klocal; r++) {
                if (topk[r] > mx) { mx = topk[r]; mi = r; }
            }
            if (u < mx) topk[mi] = u;
        }
        for (int r = 0; r < klocal; r++)
            d_prr_u2topk[(long long)pi * klocal + r] = topk[r];
    }
}

void launch_leaf_prr_interval(
    const int* d_pair_leaf_ids, const int* d_pair_qids,
    const uint8_t* d_leaf_codes, const int* d_leaf_sizes,
    const int* d_block_cell_id, const float* d_abs_cents,
    const float* d_fine_c1d, const float* d_q_batch,
    const float* d_block_eps, int eps_stride,
    float* d_prr_l2, float* d_prr_u2topk,
    int n_pairs, int d, int Kr, int Br, int bpv, int leaf_size,
    int klocal, cudaStream_t stream)
{
    // smem: s_q[d] + s_cent[d] + s_u2[leaf_size]
    int smem = 2 * d * (int)sizeof(float) + leaf_size * (int)sizeof(float);
    leaf_prr_interval_kernel<<<n_pairs, leaf_size, smem, stream>>>(
        d_pair_leaf_ids, d_pair_qids,
        d_leaf_sizes, d_leaf_codes,
        d_block_cell_id, d_abs_cents,
        d_fine_c1d, d_q_batch,
        d_block_eps, eps_stride,
        d_prr_l2, d_prr_u2topk,
        d, Kr, Br, bpv, leaf_size, klocal);
    CUDA_CHECK(cudaGetLastError());
}
    ...
    int n_pairs, int k, int batch_size)
{
    ...
    // tau2 = k-th smallest = max of topk_tau
    float tau2 = topk_tau[0];
    for (int r = 1; r < k; r++) tau2 = fmaxf(tau2, topk_tau[r]);
    d_prr_tau2[qi] = tau2;
}
...
// Shared memory:
//   s_query[d]            — query vector
//   s_surv[per_block_r]   — survivor IDs (compacted via atomic)
//   s_nsurv[1]            — survivor count
//   s_exact_dists[per_block_r] — exact L2 distances
//   s_exact_ids[per_block_r]   — survivor IDs (for output)
//   s_wsum[4]             — warp partial sums
__global__ void prr_exact_rerank_kernel(
    const int*   __restrict__ d_pair_leaf_ids,
    const int*   __restrict__ d_pair_qids,
    const int*   __restrict__ leaf_sizes,
    const int*   __restrict__ leaf_ids_data,
    const float* __restrict__ d_prr_l2,
    const float* __restrict__ d_prr_tau2,
    const float* __restrict__ d_base_vecs,
    const float* __restrict__ d_q_batch,
    float* d_out_dists, int* d_out_ids,
...
void launch_prr_exact_rerank(
    const int* d_pair_leaf_ids, const int* d_pair_qids,
    const int* d_leaf_ids_data, const int* d_leaf_sizes,
