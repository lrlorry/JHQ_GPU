...
{
    if (nq > ws.batch_cap)
        throw std::runtime_error("hblock_v20: nq > batch_cap");
    ...
    // L1: project q → d_proj, GEMM K1 centroids, top-ck1
    ...
    // Gather leaf block start indices
...
__global__ void leaf_flat_kernel_v20(
    ...
    cudaStream_t   stream)
{
    int smem = leaf_size * (int)sizeof(float)
             ...
             + 4 * (int)sizeof(float)
             + 4 * (int)sizeof(int);
    leaf_flat_kernel_v20<<<n_pairs, leaf_size, smem, stream>>>(
...
void gpu_merge_pq_topk_v20(int nq, int n_pairs, int rerank_r, SearchWorkspace& ws)
{
    cudaStream_t s = ws.stream;
