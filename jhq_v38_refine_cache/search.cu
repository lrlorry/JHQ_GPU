...
// Two kinds of data meet in the refine kernel and compete for the same L1.
//
// res_c1d is 128 KB at M=128, Kr=256, and every candidate of every query reads
// all of it: one lookup per dimension, d of them a candidate. It is the hot
// set, and at 0.13% of a 96 MB L2 it is certainly L2-resident; the question is
// only whether it also stays in L1.
//
// list_res is the opposite. A candidate's bpv bytes are read once, in order,
// and never again. Streaming it through the default policy is what evicts the
// codebook. __ldcs marks those loads evict-first, which is the whole of
// JHQ_REFINE_LDCS: no shared memory, no occupancy cost, just telling the cache
// which of the two is worth keeping.
__device__ __forceinline__ uint8_t load_rc(const uint8_t* p) {
#if JHQ_REFINE_LDCS
    return __ldcs(p);
#else
    return *p;
    ...
    int ck, int k, int ntotal)
{
    ...
    if (!resid_lut_materialised()) {
        ...
        size_t cap = 40960;
#if JHQ_REFINE_SMEM || JHQ_REFINE_L1
        static int cfg = 0;
        if (!cfg) {
            cfg = 1;
            int optin = 40960;
...
#endif
#if JHQ_REFINE_L1
            // 0 asks for the smallest shared partition, which leaves the most
            // of the unified L1/shared block as L1 -- the opposite trade to
            // JHQ_REFINE_SMEM, and the one that helps if the codebook only
            // needs to stop being evicted rather than move.
            CUDA_CHECK(cudaFuncSetAttribute(residual_refine_fused_kernel,
...
#if JHQ_REFINE_SMEM
        { int o = 40960;
          cudaDeviceGetAttribute(&o, cudaDevAttrMaxSharedMemoryPerBlockOptin, 0);
          cap = (size_t)o; }
#endif
#endif
        const int cb_smem = (q_bytes + cb_bytes <= cap) ? 1 : 0;
