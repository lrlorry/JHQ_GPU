...
// leaf_flat_kernel_v39 (id sub-record must be int-aligned after the code
...
// v39: ported from hblock_v38 (W=4 batched expansion + HNSW early
// termination scaffolding retained from the v34/v35 lineage; no PRR).
...
// only the *exact rerank* stage (leaf_flat_kernel) reads int8 directly and
// accumulates in int32.
//
// v39 additionally resolves PQ codes/vector ids and raw vectors through a
// bounded GPU region pool (region_id -> pool slot indirection) instead of
// flat permanently-resident arrays — see leaf_flat_kernel_v39 below and
// HBlockIndex::fetch_code_regions()/fetch_raw_regions() in jhq_gpu_index.cu.
// Fetch planning (which regions must be resident before this kernel runs)
// happens host-side, between gpu_build_and_sort_pairs_v29() and
// launch_leaf_flat_v39(); every region a launched pair touches is guaranteed
// resident by the time the kernel starts.
struct SearchWorkspace {
    ...
    // before launch_leaf_flat_v39 runs) before touching the GPU payload.
...
// Per-block: PQ distances → bitonic sort → top per_block_r → exact L2
// (int8 base vectors, int32 accumulation) → top klocal.
//
// v39: codes/ids and raw vectors are NOT flat resident arrays. Each block's
// (codes+ids) record lives at byte offset d_block_code_offset[blk] inside
// code region d_block_code_region[blk]; that region must already be
// resident at pool slot d_code_region_slot[region] (guaranteed by the
// fetch-planning pass that runs before this launch — the kernel does not
// fault or fetch on demand, it only reads the slot table). Raw vectors are
// resolved the same way through the raw pool, using d_vec_local_pos[cand_id]
// to find a candidate's position inside its (already-known) home block.
