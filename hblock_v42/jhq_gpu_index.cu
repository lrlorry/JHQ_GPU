...
#include "hblock_v42/search.cuh"   // for align4() and the region-indirected leaf kernel
      ...
      region_file_dir_(p.region_file_dir)
{
    ...
    code_region_path_ = region_file_dir_ + "/hblock_v42_" + std::to_string((long long)getpid()) + "_codes.region";
    raw_region_path_  = region_file_dir_ + "/hblock_v42_" + std::to_string((long long)getpid()) + "_raw.region";
    ...
    int total_blocks)
{
    ...
    // filling regions up to region_bytes_ before starting a new one. v39/v42
    // use physical block order (no graph-aware repacking yet -- see the
    // class comment in jhq_gpu_index.cuh for why v39's SPACEV runs found
    // this insufficient at large batch sizes, and what would actually fix
    // it -- out of scope for this version).
