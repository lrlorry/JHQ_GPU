...
// HBlock v42: v39 (logical region partitioning + bounded GPU region pool)
// with the region store backed by real mmap'd files on disk instead of an
// in-process host RAM buffer. v39 already bounded *GPU* residency but still
// held the entire code/raw region store in host RAM during search() (a
// std::vector sized to the whole dataset) -- not actually out-of-core with
// respect to host memory, which matters once the dataset no longer fits in
// host RAM either (SPACEV-1B territory). v42 writes the same region-store
// bytes v39 already computes to two files (hblock_v42_<pid>_codes.region,
// _raw.region) during add(), then mmaps them read-only; search-time host
// RAM footprint for the payload is now whatever the OS page cache decides
// to keep resident, not a fixed full-dataset-sized buffer.
//
// Region *packing* (block -> region assignment) is unchanged from v39:
// blocks are already grouped by (c1,c2,c3) routing cell in physical order
// (the add() pipeline sorts by cell key before packing), so "same cell
// first, then physical block order" -- the plan docs' stated first-pass
// locality rule -- was already true in v39. What v39's SPACEV runs actually
// found is that *cell-lexicographic* order does not track the block
// graph's cross-cell edges (a query's beam search jumps between cells that
// are spatially/graph-adjacent but numbered far apart), so per-batch region
// footprint stayed high (57-91%) at realistic batch sizes and only dropped
// (11-33%) at small batch sizes -- see results/hblock_v39_findings.md.
// Fixing *that* needs graph-aware region repacking, deliberately deferred
// past this version (matches the plan docs' own v42 scope: real
// out-of-core files, still physical block order, no repacking yet).
//
// Same tree routing + block graph + per-block fixed top-16 exact rerank as
// v36/v38/v39 -- this version changes payload residency and addressing
// only, not recall semantics.
...
public:
    struct Params {
        ...
        // during add() (hblock_v42_<pid>_codes.region / _raw.region).
        std::string region_file_dir = "/tmp";
    };
    ...
    // block-major logical regions and written to two mmap'd files on disk
    // (see build_region_layout()) rather than uploaded as permanent flat
    // GPU-resident arrays or held in a full-dataset-sized host buffer;
    // search() stages only the regions it needs, reading from the mmap.
...
private:
    ...
    // Real mmap'd (PROT_READ, MAP_SHARED) region files, written once in
    // build_region_layout() and read from directly by fetch_code_regions()/
    // fetch_raw_regions() -- host RAM for the payload is now whatever the
    // OS page cache keeps resident, not a fixed dataset-sized buffer.
    // code_region_map_/raw_region_map_ are null iff the corresponding fd is
    // -1 (not yet built, or the store is empty).
    std::string code_region_path_, raw_region_path_;
    int code_region_fd_ = -1, raw_region_fd_ = -1;
    const uint8_t* code_region_map_ = nullptr;
    const uint8_t* raw_region_map_  = nullptr;
    size_t code_region_map_bytes_ = 0, raw_region_map_bytes_ = 0;
    ...
    // writes them to two mmap'd files on disk (code_region_path_/
    // raw_region_path_), and allocates the bounded GPU pool + slot
