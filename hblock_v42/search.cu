...
// v42: forked from hblock_v39 unchanged at the kernel level -- still named
// leaf_flat_kernel_v39/launch_leaf_flat_v39 below because the *kernel* and
// its region_id -> pool-slot indirection scheme are exactly what v39
// introduced and did not need to change. What v42 changes is host-side only
// (jhq_gpu_index.cu): the region store backing fetch_code_regions()/
// fetch_raw_regions() is now two real mmap'd files instead of an
// in-process host RAM buffer. Routing/beam-search/pair-build kernels are
// copied verbatim from v38/v36 (float-based, unchanged algorithm); raw
// base vectors and raw queries are read as int8 with int32
// squared-difference accumulation (inherited from v38 unchanged).
// No PRR / certified bounds; single search mode (v36's
// fixed-per-block-top-16 behavior).
#include "hblock_v42/search.cuh"
...
void route_gpu_v29(
    ...
    SearchWorkspace& ws)
{
    if (nq > ws.batch_cap) throw std::runtime_error("hblock_v42: nq > batch_cap");
...
// ── v39 key kernel: per-block PQ filter → exact L2 (int8, int32 accum) → top klocal ─
// Codes/ids and raw vectors are resolved through the GPU region pool
// indirection instead of flat resident arrays — see search.cuh's comment
// above launch_leaf_flat_v39() for the residency contract.

// align4() is declared in search.cuh (shared with jhq_gpu_index.cu's
// build_region_layout(), which must use the identical alignment rule when
// placing each block's id sub-record after its code sub-record).

__global__ void leaf_flat_kernel_v39(
    ...
    const int*     __restrict__ d_vec_local_pos,
    ...
    int per_block_r, int klocal)
{
    ...
    // Fetch planning (host side, before this kernel is launched) guarantees
    // both of these regions are already resident — the slot lookup here is
    // never -1 for a block that actually appears in d_pair_leaf_ids.
    ...
    // This block's vector ids follow its codes in the same packed record,
    // at a 4-byte-aligned offset (see build_region_layout()).
    ...
    // Shared-memory layout (see align4() comment above):
    //   s_dist  [leaf_size]  float
    //   s_id    [leaf_size]  int
    //   s_query [d]          int8_t   (raw query bytes for exact rerank)
    //   -- align to 4 bytes --
    //   s_wsum  [4]          int      (per-warp partial int32 sums)
    //   s_sel   [per_block_r] int
    //   s_exact [per_block_r] float
    extern __shared__ char shm[];
    size_t off = 0;
    float*  s_dist  = (float*)(shm + off); off += (size_t)leaf_size * sizeof(float);
    int*    s_id    = (int*)  (shm + off); off += (size_t)leaf_size * sizeof(int);
    int8_t* s_query = (int8_t*)(shm + off); off += (size_t)d * sizeof(int8_t);
    off = align4(off);
    int*    s_wsum  = (int*)  (shm + off); off += 4 * sizeof(int);
    int*    s_sel   = (int*)  (shm + off); off += (size_t)per_block_r * sizeof(int);
    float*  s_exact = (float*)(shm + off); off += (size_t)per_block_r * sizeof(float);
    ...
    // Every s_sel[r] candidate came from this same leaf_blk's own threads
    // (my_id above), so it always lives in leaf_blk's raw region entry —
    // resolve that once, then index by each candidate's local position.
    const int raw_region = d_block_raw_region[leaf_blk];
    const int raw_slot   = d_raw_region_slot[raw_region];
    const uint8_t* raw_blk_base = d_raw_region_pool
        + (long long)raw_slot * region_bytes
    ...
    for (int r = 0; r < per_block_r; r++) {
        ...
        int partial = 0;
        if (cand_id >= 0) {
            int local_pos = __ldg(&d_vec_local_pos[cand_id]);
            const int8_t* vec = (const int8_t*)(raw_blk_base + (long long)local_pos * d);
            for (int j = tid; j < d; j += blockDim.x) {
                int diff = (int)s_query[j] - (int)vec[j];
                partial += diff * diff;
            }
        }
        for (int o = 16; o >= 1; o >>= 1)
...
void launch_leaf_flat_v39(
    ...
    const int* d_vec_local_pos,
    const int* d_leaf_sizes,
    const float* d_lut_fine, const int8_t* d_q_batch_i8,
    float* d_out_dists, int* d_out_ids,
    int region_bytes,
    ...
    cudaStream_t stream)
{
    size_t smem = (size_t)leaf_size * (sizeof(float) + sizeof(int))
                + (size_t)d * sizeof(int8_t);
    smem = align4(smem);
    smem += 4 * sizeof(int)
          + (size_t)per_block_r * (sizeof(int) + sizeof(float));
    leaf_flat_kernel_v39<<<n_pairs, leaf_size, (int)smem, stream>>>(
        d_pair_leaf_ids, d_pair_qids,
        d_code_region_pool, d_code_region_slot, d_block_code_region, d_block_code_offset,
