                              ...
                              cudaStream_t stream)
{
    ...
    CUDA_CHECK(cudaMalloc(&d_stat,  3 * sizeof(int)));
    ...
    for (int m0 = 0; m0 < M; m0 += mc) {
        ...
        int it = 0, h_stat[3] = {0, 0, 0};
        bool conv = false;
        for (; it < cap; ++it) {
            CUDA_CHECK(cudaMemsetAsync(d_stat, 0, 3 * sizeof(int), stream));
            ...
            update_sort_kernel<<<mb, BLOCK, Kr * sizeof(float), stream>>>(
                d_sum, d_cnt, d_sorted, c_out, seg, Kr, d_stat + 1, d_stat + 2);
            CUDA_CHECK(cudaGetLastError());
            if ((it + 1) % CHECK_EVERY == 0) {
                CUDA_CHECK(cudaMemcpyAsync(h_stat, d_stat, 3 * sizeof(int),
        ...
        CUDA_CHECK(cudaStreamSynchronize(stream));
        {
            ...
            std::fprintf(stderr, "[res-train] subspaces %d-%d: %s at %d iterations, "
                         "rel %.3e, %d boundaries still moving\n",
                         m0, m0 + mb - 1, conv ? "converged" : "CAP", it,
                         ca > 0.f ? dc / ca : 0.f, h_stat[0]);
        }
    }
                                          ...
                                          float* d_res_c1d, cudaStream_t stream)
{
    ...
    for (; it < cap; ++it) {
        ...
        update_sort_kernel<<<M_chunk, BLOCK, Kr * sizeof(float), stream>>>(
            d_sum, d_cnt, d_sorted, d_res_c1d, seg, Kr, d_stat + 1, d_stat + 2);
        CUDA_CHECK(cudaGetLastError());
        if ((it + 1) % CHECK_EVERY == 0) {
            CUDA_CHECK(cudaMemcpyAsync(h_stat, d_stat, 3 * sizeof(int),
                                       cudaMemcpyDeviceToHost, stream));
