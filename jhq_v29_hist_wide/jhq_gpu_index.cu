#include "jhq_v29_hist_wide/jhq_gpu_index.cuh"
...
#endif
#include "jhq_v29_hist_wide/encode.cuh"
#include "jhq_v29_hist_wide/train_pq_gpu.cuh"
#include "jhq_v29_hist_wide/train_res_gpu.cuh"
#include "jhq_v29_hist_wide/search.cuh"
...
void JHQGpuIndex::train_residual_codebook_streamed(const float* h_x, int n)
{
    ...
    // How many subspaces to fit at once. The base is re-read once per group,
    ...
    if (const char* ge = std::getenv("JHQ_RES_GROUP")) {
        ...
        if (G < 1) G = 1;
        if (G > M_) G = M_;
        while (G > 1 && M_ % G) --G;                       // whole groups only
    }
    if (G < 1) G = 1;
    if (G > M_) G = M_;
    std::fprintf(stderr, "[res-train] %d subspaces per group, %d passes over "
                         "the base (R_group %.1f GB)\n",
                 G, (M_ + G - 1) / G, (double)n * G * Ds_ * 4.0 / 1073741824.0);
    ...
    float* d_res_batch = nullptr;
    CUDA_CHECK(cudaMalloc(&d_res_batch, (size_t)batch * G * Ds_ * sizeof(float)));
    ...
    for (int m0 = 0; m0 < M_; m0 += G) {
        ...
        for (int off = 0; off < n; off += batch) {
            ...
            launch_primary_encode(d_yg, d_code,
                                  ...
                                  nb, gd, gm, Ds_, K_);
            CUDA_CHECK(cudaDeviceSynchronize());
            // Subtract on the device, then copy the residuals out once.
            {
                const long long tot = (long long)nb * gd;
                const int blk = 256;
                const int grid = (int)std::min((tot + blk - 1) / blk, 65535LL);
                residual_values_kernel<<<grid, blk>>>(
                    d_yg, d_code, d_cent_ + (size_t)m0 * K_ * Ds_, d_res_batch,
                    nb, gd, gm, Ds_, K_);
                CUDA_CHECK(cudaGetLastError());
                CUDA_CHECK(cudaMemcpy(R_group.data() + (size_t)off * gd, d_res_batch,
                                      (size_t)nb * gd * sizeof(float),
                                      cudaMemcpyDeviceToHost));
            }
        }

        // Exact sorted 1-D k-means, one subspace at a time, as before.
        // Gather a subspace's values into the layout the estimator wants,
        // rather than a copy per subspace. R_group is [n][gd] and a subspace
        // owns a stride of Ds within each row; copying that into a contiguous
        // array doubled the memory for the duration, which is what forced the
        // group small enough to need sixteen passes over the base. One buffer,
        // reused across the group, costs n*Ds instead of gm*n*Ds.
        {
            std::vector<float> resid_m((size_t)n * Ds_);
            for (int g = 0; g < gm; ++g) {
                for (int i = 0; i < n; ++i)
                    std::memcpy(resid_m.data() + (size_t)i * Ds_,
                                R_group.data() + (size_t)i * gd + (size_t)g * Ds_,
                                (size_t)Ds_ * sizeof(float));
                std::vector<float> cb = train_1d_kmeans(resid_m.data(),
                                                        (int)resid_m.size(), Kr_);
    ...
    cudaFree(d_x); cudaFree(d_yg); cudaFree(d_code); cudaFree(d_res_batch);
}
...
// jhq_v29_hist_wide/jhq_gpu_index.cuh's Params::add_batch comment for
