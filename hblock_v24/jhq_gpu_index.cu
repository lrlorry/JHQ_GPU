#include "hblock_v24/jhq_gpu_index.cuh"
...
void HBlockIndex::train(const float* h_x, int n_train)
{
    ...
    printf("[v24 train] d=%d d_proj=%d K1=%d K2=%d K3=%d ck1=%d ck2=%d ck3=%d"
    ...
    printf("[v24 train] total=%.1f ms  sigma_r3=%.4f\n", Ms(Clock::now()-T0).count(), sigma);
}
...
void HBlockIndex::add(const float* h_x, int n)
{
    ...
    printf("[v24 add] n=%d  d=%d  batch=%d\n", n, d_, 8192);
    ...
    printf("[v24 add total] %.1f ms  blocks=%d\n",
    ...
    const std::vector<int>& h_pair_blk_count)
{
    ...
    gpu_build_block_adj_v24(d_blk_proj_gpu_, d_blk_norm_gpu_,
                          ...
                          float* h_dists, int* h_ids) const
{
    ...
    for(int qstart=0;qstart<nq;qstart+=batch_size_){
        ...
        auto t1=Clock::now();
        gpu_block_search_v24(nb, n_leaf_blocks_, d_proj_,
        ...
        auto t3=Clock::now();
        gpu_build_and_sort_pairs_v24(nb,n_pairs,n_leaf_blocks_,ws_.max_leaf_sel,ws_);
        ms_pairs+=Ms(Clock::now()-t3).count();

        auto t4=Clock::now();
        launch_leaf_flat_v24(ws_.d_pair_leaf_b,ws_.d_pair_qid_b,
        ...
        auto t5=Clock::now();
        gpu_merge_pq_topk_v24(nb,n_pairs,rerank_r_,top_p_,ws_);
        ms_merge+=Ms(Clock::now()-t5).count();

        auto t6=Clock::now();
        launch_gather_vecs_v24(d_base_vecs_,ws_.d_pq_ids,ws_.d_cand_vecs,nb,rerank_r_,d_,s);
        ms_gather+=Ms(Clock::now()-t6).count();

        auto t7=Clock::now();
        launch_exact_rerank_v24(ws_.d_q_batch,ws_.d_cand_vecs,ws_.d_pq_ids,
