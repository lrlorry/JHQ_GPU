...
// ── Kernel: batched short primary-prefix scan ────────────────────────────────
...
__global__ void scan_ivf_batched_kernel(
    ...
    const int*     __restrict__ list_offsets,    // [nlist+1]
    const uint8_t* __restrict__ list_primary_lm, // [M, N]
    ...
    int m_begin, int m_end)
{
    ...
    for (int local_t = tid; local_t < total; local_t += blockDim.x) {
        ...
        for (int m = m_begin; m < m_end; ++m) {
            uint8_t cm = list_primary_lm[(long long)m * N + abs_pos];
...
// ── Kernel: add another primary-code byte range for compact survivors ───────
__global__ void refine_primary_chunk_batched_kernel(
    const int*     __restrict__ in_pos,       // [B, in_count]
    const float*   __restrict__ in_dist,      // [B, in_count]
    const float*   __restrict__ d_lut,        // [B, M*Ds*K1D]
    const uint8_t* __restrict__ list_primary_lm,
    float*                      out_dist,     // [B, in_count]
    int*                        out_pos,      // [B, in_count]
    int in_count,
    int N,
    int M,
    int Ds,
    ...
    int B)
{
    long long total = (long long)B * in_count;
    const int kmask = K1D - 1;
    for (long long i = (long long)blockIdx.x * blockDim.x + threadIdx.x;
         i < total; i += (long long)gridDim.x * blockDim.x) {
        int bqi = (int)(i / in_count);
        int pos = in_pos[i];
        out_pos[i] = pos;
        if (pos < 0) {
            out_dist[i] = __int_as_float(0x7F800000);
            continue;
        }

        const float* my_lut = d_lut + (long long)bqi * M * Ds * K1D;
        float dist = in_dist[i];
        for (int m = m_begin; m < m_end; ++m) {
            uint8_t cm = list_primary_lm[(long long)m * N + pos];
            const float* lm = my_lut + m * Ds * K1D;
            for (int k = 0; k < Ds; ++k)
                dist += lm[k * K1D + ((cm >> (k * bits_per_dim)) & kmask)];
        }
        out_dist[i] = dist;
...
__global__ void residual_refine_batched_kernel(
    ...
    const uint8_t* __restrict__ list_res_lm,   // [bpv, N]
    const float*   __restrict__ list_corr,     // [N]
    float*                      comp_dists,    // [B, ck]
    int ck, int N, int d, int Kr, int Br, int bpv, int B)
{
    long long total = (long long)B * ck;
    for (long long i = (long long)blockIdx.x * blockDim.x + threadIdx.x;
         i < total; i += (long long)gridDim.x * blockDim.x) {
        ...
        float d_res = 0.0f;
        for (int j = 0; j < d; ++j) {
            int byte_idx = (Br == 4) ? (j / 2) : j;
            uint8_t rbyte = list_res_lm[(long long)byte_idx * N + pos];
            int ri = (Br == 4)
                ? ((j % 2 == 0) ? (rbyte & 0x0F) : (rbyte >> 4))
                : rbyte;
...
static void realloc_ck_buffers(SearchWorkspace& ws, int batch_cap, int work_cap, int k) {
    if (work_cap > ws.ck_cap) {
        ...
        long long n = (long long)batch_cap * work_cap;
    ...
    int*   h_out_ids)
{
    if (ws.batch_cap <= 0 || ws.cap_per_query <= 0)
        throw std::runtime_error("v5: workspace not initialised — call add() first");

    int ck = std::min(
                 std::max(k, (int)std::ceil(alpha * (float)k)),
                 (int)ws.cap_per_query);
    stage1 = std::min(std::max(stage1, ck), (int)ws.cap_per_query);
    stage2 = std::min(std::max(stage2, ck), stage1);
    short_M = std::min(std::max(short_M, 1), M);
    mid_M = std::min(std::max(mid_M, short_M), M);
    int work_cap = std::max(stage1, std::max(stage2, ck));

    realloc_ck_buffers(ws, ws.batch_cap, work_cap, k);
    ...
    for (int qoff = 0; qoff < nq; qoff += batch_size) {
        ...
        // ── 6. Short primary-prefix scan for all B queries ──────────────────
        {
            int smem = lut_size * (int)sizeof(float);
            scan_ivf_batched_kernel<<<B, BLOCK, smem>>>(
                ws.d_lut, ws.d_probe_ids, ws.d_probe_offsets,
                d_list_offsets, d_list_primary_lm, ws.d_query_total,
                ws.d_cand_dist, ws.d_cand_pos,
                ws.cap_per_query, N, nprobe, M, Ds, K1D, bits_per_dim,
                0, short_M);
            CUDA_CHECK(cudaGetLastError());
        }
        ...
        // ── 8. Gather stage1 survivors per query ────────────────────────────
        {
            long long total = (long long)B * stage1;
            int grid = (int)std::min((total + BLOCK - 1) / BLOCK, (long long)65535);
            gather_topck_kernel<<<grid, BLOCK>>>(
                ws.d_keys, ws.d_cand_pos,
                ws.d_topck_primary, ws.d_topck_pos,
                ws.cap_per_query, stage1, B);
            CUDA_CHECK(cudaGetLastError());
        }

        // ── 9. Read more primary layers for stage1 survivors, keep stage2 ────
        if (short_M < mid_M) {
            long long total = (long long)B * stage1;
            int grid = (int)std::min((total + BLOCK - 1) / BLOCK, (long long)65535);
            refine_primary_chunk_batched_kernel<<<grid, BLOCK>>>(
                ws.d_topck_pos, ws.d_topck_primary,
                ws.d_lut, d_list_primary_lm,
                ws.d_comp_dists, ws.d_cand_pos,
                stage1, N, M, Ds, K1D, bits_per_dim,
                short_M, mid_M, B);
            CUDA_CHECK(cudaGetLastError());
            pack_keys2_kernel<<<grid, BLOCK>>>(ws.d_comp_dists, ws.d_keys2, total, stage1);
            CUDA_CHECK(cudaGetLastError());
            thrust::sort_by_key(t_keys2, t_keys2 + total, t_pos);

            long long gather_total = (long long)B * stage2;
            grid = (int)std::min((gather_total + BLOCK - 1) / BLOCK, (long long)65535);
            gather_topck_kernel<<<grid, BLOCK>>>(
                ws.d_keys2, ws.d_cand_pos,
                ws.d_topck_primary, ws.d_topck_pos,
                stage1, stage2, B);
            CUDA_CHECK(cudaGetLastError());
        } else if (stage2 < stage1) {
            long long gather_total = (long long)B * stage2;
            int grid = (int)std::min((gather_total + BLOCK - 1) / BLOCK, (long long)65535);
            gather_topck_kernel<<<grid, BLOCK>>>(
                ws.d_keys, ws.d_cand_pos,
                ws.d_topck_primary, ws.d_topck_pos,
                ws.cap_per_query, stage2, B);
            CUDA_CHECK(cudaGetLastError());
        }

        // ── 10. Finish primary code for stage2 survivors, keep ck ────────────
        if (mid_M < M) {
            long long total = (long long)B * stage2;
            int grid = (int)std::min((total + BLOCK - 1) / BLOCK, (long long)65535);
            refine_primary_chunk_batched_kernel<<<grid, BLOCK>>>(
                ws.d_topck_pos, ws.d_topck_primary,
                ws.d_lut, d_list_primary_lm,
                ws.d_comp_dists, ws.d_cand_pos,
                stage2, N, M, Ds, K1D, bits_per_dim,
                mid_M, M, B);
            CUDA_CHECK(cudaGetLastError());
            pack_keys2_kernel<<<grid, BLOCK>>>(ws.d_comp_dists, ws.d_keys2, total, stage2);
            CUDA_CHECK(cudaGetLastError());
            thrust::sort_by_key(t_keys2, t_keys2 + total, t_pos);

            long long gather_total = (long long)B * ck;
            grid = (int)std::min((gather_total + BLOCK - 1) / BLOCK, (long long)65535);
            gather_topck_kernel<<<grid, BLOCK>>>(
                ws.d_keys2, ws.d_cand_pos,
                ws.d_topck_primary, ws.d_topck_pos,
        ...
        // ── 11. Build residual LUTs for all B queries ────────────────────────
        {
            long long total = (long long)B * d * Kr;
            int grid = (int)std::min((total + BLOCK - 1) / BLOCK, (long long)65535);
        ...
        // ── 12. Residual refinement on top-ck candidates ─────────────────────
        {
            long long total = (long long)B * ck;
            int grid = (int)std::min((total + BLOCK - 1) / BLOCK, (long long)65535);
            residual_refine_batched_kernel<<<grid, BLOCK>>>(
                ws.d_topck_pos, ws.d_topck_primary,
                ws.d_lut_r, d_list_res_lm, d_list_corr,
                ws.d_comp_dists, ck, N, d, Kr, Br, bpv, B);
            CUDA_CHECK(cudaGetLastError());
        }

        // ── 13. Pack & sort composite distances for final top-k ──────────────
        {
            long long total = (long long)B * ck;
            int grid = (int)std::min((total + BLOCK - 1) / BLOCK, (long long)65535);
        ...
        // ── 14. Gather final k ids + distances per query ─────────────────────
        {
            long long total = (long long)B * k;
            int grid = (int)std::min((total + BLOCK - 1) / BLOCK, (long long)65535);
        ...
        // ── 15. Copy this chunk's results back to host (one sync per chunk) ──
