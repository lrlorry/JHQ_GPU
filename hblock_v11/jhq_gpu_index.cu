#include "hblock_v11/jhq_gpu_index.cuh"
#include "hblock_v11/encode.cuh"
...
HBlockIndex::~HBlockIndex() {
    ...
    if (ws_.h_pair_leaf_ids) cudaFreeHost(ws_.h_pair_leaf_ids);
    ...
    cudaFree(ws_.d_pair_leaf_ids);
    cudaFree(ws_.d_pair_qids);
...
void HBlockIndex::alloc_workspace() {
    ...
    CUDA_CHECK(cudaMallocHost(&ws_.h_pair_leaf_ids, (long long)max_pairs * sizeof(int)));
    ...
    CUDA_CHECK(cudaMalloc(&ws_.d_pair_leaf_ids, (long long)max_pairs * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&ws_.d_pair_qids,     (long long)max_pairs * sizeof(int)));
                          ...
                          float* h_dists, int* h_ids) const
{
    ...
    // ── Phase 2: build flat pair list, sorted by leaf_id ─────────────────────
    // Sorting ensures consecutive GPU blocks read the same leaf codes → L2 reuse.
    struct Pair { int leaf_blk, qid; };
    std::vector<Pair> pairs;
    pairs.reserve((long long)nq * ck3_);
    for (int qi = 0; qi < nq; qi++) {
        int cnt = ws_.h_leaf_cnt[qi];
        for (int s = 0; s < cnt; s++) {
            int lb = ws_.h_leaf_sel[qi * ck3_ + s];
            if (lb >= 0 && lb < n_leaf_blocks_)
                pairs.push_back({lb, qi});
        }
    }
    std::stable_sort(pairs.begin(), pairs.end(),
        [](const Pair& a, const Pair& b){ return a.leaf_blk < b.leaf_blk; });

    int n_pairs = (int)pairs.size();
    for (int pi = 0; pi < n_pairs; pi++) {
        ws_.h_pair_leaf_ids[pi] = pairs[pi].leaf_blk;
        ws_.h_pair_qids    [pi] = pairs[pi].qid;
    }
    auto t2 = Clock::now();

    // ── Phase 3: ONE H2D + kernel (n_pairs blocks) + D2H ─────────────────────
    cudaStream_t s = ws_.stream;
    CUDA_CHECK(cudaMemcpyAsync(ws_.d_pair_leaf_ids, ws_.h_pair_leaf_ids,
                               (long long)n_pairs * sizeof(int), cudaMemcpyHostToDevice, s));
    CUDA_CHECK(cudaMemcpyAsync(ws_.d_pair_qids, ws_.h_pair_qids,
                               (long long)n_pairs * sizeof(int), cudaMemcpyHostToDevice, s));

    launch_leaf_flat(
        ws_.d_pair_leaf_ids, ws_.d_pair_qids,
    ...
    printf("  [v11] Route=%.1f ms  Pairs=%.1f ms (%d pairs)"
