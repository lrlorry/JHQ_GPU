#include "jhq_v36_all_default/jhq_gpu_index.cuh"
...
#endif
#include "jhq_v36_all_default/encode.cuh"
#include "jhq_v36_all_default/train_pq_gpu.cuh"
#include "jhq_v36_all_default/train_res_gpu.cuh"
#include "jhq_v36_all_default/search.cuh"
    ...
    long long nb, int gd, int gm, int Ds, int K)
{
    ...
    for (long long idx = (long long)blockIdx.x * blockDim.x + threadIdx.x;
        ...
        out[idx] = y[idx] - cent[((size_t)g * K + code[i * gm + g]) * Ds + k];
    }
}
...
void JHQGpuIndex::train_residual_codebook_streamed(const float* h_x, int n)
{
    ...
    // them and the chunk that consumes them. Holding all M is 67.8 GB at
    // stella; holding a chunk means re-reading the base once per chunk, which
    // is sixteen passes over the same 67.8 GB. Staging to disk is one pass over
    // the base plus one write and one read of the residuals, and
    // /root/autodl-tmp has the room. The file is laid out subspace-major so a
    // chunk is a contiguous read.
    ...
    std::vector<float> h_res((size_t)batch * d_);
    std::vector<float> h_sub((size_t)batch * Ds_);
    ...
    for (int off = 0; off < n; off += batch) {
        ...
        // Subspace-major on disk: subspace m owns [m*n*Ds, (m+1)*n*Ds).
        for (int m = 0; m < M_; ++m) {
            for (int i = 0; i < nb; ++i)
                std::memcpy(h_sub.data() + (size_t)i * Ds_,
                            h_res.data() + (size_t)i * d_ + (size_t)m * Ds_,
                            (size_t)Ds_ * sizeof(float));
            const long long pos = ((long long)m * n + off) * Ds_ * (long long)sizeof(float);
            if (std::fseek(f, pos, SEEK_SET) ||
                std::fwrite(h_sub.data(), sizeof(float), (size_t)nb * Ds_, f)
                    != (size_t)nb * Ds_) {
    ...
    std::fprintf(stderr, "[res-train] one pass over %d vectors, %.1f GB spilled to %s\n",
                 n, (double)n * d_ * 4.0 / 1073741824.0, path.c_str());

    // Read back a chunk of subspaces at a time and let the device estimator do
    // the sort, the prefix sum and the Lloyd iterations.
    ...
    std::vector<float> h_chunk((size_t)C * n * Ds_);
    for (int m0 = 0; m0 < M_; m0 += C) {
        const int cm = std::min(C, M_ - m0);
        const long long pos = (long long)m0 * n * Ds_ * (long long)sizeof(float);
        ...
        CUDA_CHECK(cudaMemcpy(d_chunk, h_chunk.data(),
                              (size_t)cm * n * Ds_ * sizeof(float),
                              cudaMemcpyHostToDevice));
...
// jhq_v36_all_default/jhq_gpu_index.cuh's Params::add_batch comment for
