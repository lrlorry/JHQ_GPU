#include "v9_step_timing/jhq_gpu_index.cuh"
#include "v9_step_timing/encode.cuh"
#include "v9_step_timing/search.cuh"
...
JHQGpuIndex::~JHQGpuIndex() {
    ...
    for (int s = 0; s < N_STEP_EVENTS; ++s)
        if (ws_.ev_step[s]) cudaEventDestroy(ws_.ev_step[s]);
...
void JHQGpuIndex::alloc_workspace(int batch_size) {
    ...
    for (int s = 0; s < N_STEP_EVENTS; ++s)
        if (!ws_.ev_step[s])
            CUDA_CHECK(cudaEventCreate(&ws_.ev_step[s]));
}
...
{
    if (n_train < nlist_)
        throw std::invalid_argument("n_train must be >= nlist for v9_step_timing");
    ...
    for (int iter = 0; iter < ivf_iters_; ++iter) {
        ...
        for (int c = 0; c < nlist_; ++c) {
            ...
            if (counts[c] == 0) {
                ...
                std::memcpy(cc, h_y_train + (long long)src * d_, (size_t)d_ * sizeof(float));
                continue;
            }
    ...
    const float* d_y_train, const uint8_t* d_codes_train, int n_train)
{
    ...
    CUDA_CHECK(cudaMemcpy(h_y.data(), d_y_train,
                          (long long)n_train * d_ * sizeof(float), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(h_codes.data(), d_codes_train,
                          (long long)n_train * M_ * sizeof(uint8_t), cudaMemcpyDeviceToHost));
...
void JHQGpuIndex::add(const float* h_x, int n) {
    ...
    if (ntotal_ != 0)
        throw std::runtime_error("v9_step_timing currently supports one add() call");
    ...
    cudaFree(d_assign); cudaFree(d_order);
    cudaFree(d_y);      cudaFree(d_pc);
    cudaFree(d_rc);     cudaFree(d_co);
