...
// The fixed point is detected on the bin boundaries rather than on the
// centroids. With the values sorted, the partition IS the Kr-1 boundaries, so
// boundaries that do not move mean sums and counts do not move, which means the
// centroids the update writes are the ones it just read -- a true fixed point,
// exactly, with no tolerance to pick. Checking costs one 4-byte read back, so
// it is done every CHECK_EVERY iterations; the at most CHECK_EVERY-1 extra
// iterations that buys are 5120 operations each and cost nothing.
...
__global__ void assign_from_sorted_kernel(
    ...
    int* d_changed)                        // set when any boundary moved
{
    const int m = blockIdx.x;
    ...
    // The convergence test. d_bound starts at -1 so the first pass always
    // reports a change. Several threads may store 1 into d_changed in the same
    // pass; they all store the same value, and the host only reads it after a
    // synchronise.
    for (int i = threadIdx.x; i <= Kr; i += blockDim.x) {
        const long long k = (long long)m * (Kr + 1) + i;
        if (d_bound[k] != s_bound[i]) { d_bound[k] = s_bound[i]; *d_changed = 1; }
    }
}
    ...
    for (int m0 = 0; m0 < M; m0 += mc) {
        ...
        for (; it < cap; ++it) {
            assign_from_sorted_kernel<<<mb, BLOCK, (size_t)(Kr + 1) * sizeof(long long), stream>>>(
                d_sorted, d_prefix, c_out, d_sum, d_cnt, seg, Kr, d_bound, d_changed);
            CUDA_CHECK(cudaGetLastError());
            update_sort_kernel<<<mb, BLOCK, Kr * sizeof(float), stream>>>(
                d_sum, d_cnt, d_sorted, c_out, seg, Kr);
            CUDA_CHECK(cudaGetLastError());
            if ((it + 1) % CHECK_EVERY == 0) {
                CUDA_CHECK(cudaMemcpyAsync(&moved, d_changed, sizeof(int),
        ...
        CUDA_CHECK(cudaStreamSynchronize(stream));
        std::fprintf(stderr, "[res-train] subspaces %d-%d: %s after %d iterations\n",
                     m0, m0 + mb - 1, moved ? "CAP" : "fixed point", it);
    }

    cudaFree(d_seg); cudaFree(d_sorted); cudaFree(d_prefix);
    cudaFree(d_sum); cudaFree(d_cnt); cudaFree(d_off); cudaFree(d_tmp);
    cudaFree(d_bound); cudaFree(d_changed);
}
                                          ...
                                          float* d_res_c1d, cudaStream_t stream)
{
    ...
    for (; it < cap; ++it) {
        assign_from_sorted_kernel<<<M_chunk, BLOCK,
            ...
            d_sorted, d_prefix, d_res_c1d, d_sum, d_cnt, seg, Kr, d_bound, d_changed);
    ...
    std::fprintf(stderr, "[res-train] %d subspaces: %s after %d iterations\n",
                 M_chunk, moved ? "CAP" : "fixed point", it);
    CUDA_CHECK(cudaStreamSynchronize(stream));
