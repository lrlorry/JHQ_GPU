    ...
    int ck, int k, int ntotal)
{
    ...
    if (exact_topck()) {
        ...
        // This kernel is 91% of search (results/v38_refine_share/), and when
        // the table does not fit in shared it reads all of it -- M*256 entries,
        // 128 KB at M=128 with a float lut_t -- out of the cache, while
        // list_primary_t streams past. That is the same collision the refine
        // kernel had, at thirteen times the weight, and the same answer: ask
        // for the smallest shared partition so the unified block stays L1.
        // When the table is in shared there is nothing to protect, and the
        // carveout would only take room the table needs.
        if (!ex_lut)
            CUDA_CHECK(cudaFuncSetAttribute(scan_ivf_exact_kernel,
