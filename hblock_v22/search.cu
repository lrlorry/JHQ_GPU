...
// GPU kernels are identical to v20.
// Key difference: gather_leaf_blocks_hierarchical_kernel is REMOVED.
// route_gpu_v22 = v20 routing pipeline up to fine LUT, then D2H beam results.
// CPU block-graph traversal is in jhq_gpu_index.cu (has access to h_block_adj_).
...
// ── Routing kernels (identical to v20) ───────────────────────────────────────
...
// ── route_gpu_v22 ─────────────────────────────────────────────────────────────
// Same as v20's route_queries_v20, but WITHOUT gather_leaf_blocks step.
// After beam + LUT, D2H the beam indices so the CPU can do block traversal.
...
{
    if (nq > ws.batch_cap)
        throw std::runtime_error("hblock_v22: nq > batch_cap");
    ...
    // Extract best-path r3 for fine LUT
    ...
    // D2H: beam indices for CPU block-graph traversal
    CUDA_CHECK(cudaMemcpyAsync(ws.h_top1_ids,
                               ws.d_top1_ids,
                               (long long)B * ck1 * sizeof(int),
                               cudaMemcpyDeviceToHost, s));
    CUDA_CHECK(cudaMemcpyAsync(ws.h_top2_beam,
                               ws.d_top2_beam,
                               (long long)B * ck1 * ck2 * sizeof(int),
                               cudaMemcpyDeviceToHost, s));
    CUDA_CHECK(cudaMemcpyAsync(ws.h_top3_beam,
                               ws.d_top3_beam,
                               (long long)B * ck1 * ck2 * ck3 * sizeof(int),
                               cudaMemcpyDeviceToHost, s));
    CUDA_CHECK(cudaStreamSynchronize(s));
    // GPU: routing + LUT done.  CPU: now do block-graph traversal.
}

// ── build_pairs + PQ + merge + rerank (identical to v20, renamed _v22) ───────

__global__ void build_pairs_kernel_v22(
    ...
    int max_leaf_sel, SearchWorkspace& ws)
{
    ...
    build_pairs_kernel_v22<<<(nq + 255) / 256, 256, 0, s>>>(
...
__global__ void leaf_flat_kernel_v22(
    ...
    cudaStream_t   stream)
{
    int smem = leaf_size * (int)sizeof(float)
             ...
             + 4 * (int)sizeof(float)
             + 4 * (int)sizeof(int);
    leaf_flat_kernel_v22<<<n_pairs, leaf_size, smem, stream>>>(
...
__global__ void fill_iota_kernel_v22(int* arr, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) arr[i] = i;
}

__global__ void segmented_topk_kernel_v22(
...
void gpu_merge_pq_topk_v22(int nq, int n_pairs, int rerank_r, SearchWorkspace& ws)
{
    cudaStream_t s = ws.stream;
    int k = std::min(rerank_r, (int)K_MAX);
    fill_iota_kernel_v22<<<(n_pairs + 255) / 256, 256, 0, s>>>(ws.d_pair_leaf_a, n_pairs);
    CUDA_CHECK(cub::DeviceRadixSort::SortPairs(
        ws.d_cub_tmp, ws.cub_bytes,
    ...
    segmented_topk_kernel_v22<<<nq, BLOCK, smem, s>>>(
...
__global__ void gather_vecs_kernel_v22(
...
{
    dim3 grid(R, B);
    gather_vecs_kernel_v22<<<grid, 128, 0, s>>>(d_base_vecs, d_cand_ids, d_cand_vecs, R, d);
    CUDA_CHECK(cudaGetLastError());
}

__global__ void exact_rerank_kernel_v22(
    ...
    int B, int R, int d, int k, cudaStream_t s)
{
    ...
    exact_rerank_kernel_v22<<<B, BLOCK, smem, s>>>(
