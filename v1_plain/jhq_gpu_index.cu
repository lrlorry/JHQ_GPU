#include "v1_plain/jhq_gpu_index.cuh"
#include "v1_plain/encode.cuh"
      ...
      jl_(d, p.seed)
{
    ...
    if (p.B > 8)
        throw std::invalid_argument("B must be <= 8 (primary code is uint8_t)");
    if (p.Br != 4 && p.Br != 8)
...
void JHQGpuIndex::alloc_workspace(long long N) {
    // Free existing buffers (cudaFree(nullptr) is a no-op)
...
float* JHQGpuIndex::rotate_on_gpu(const float* h_x, int n) const {
    // Upload X, compute Y = X * Pi^T via cuBLAS, return d_Y (caller frees).
    float* d_x = nullptr;
    float* d_y = nullptr;
    ...
    // d_Pi is Π stored col-major (d×d).
    // We want Y [n×d] row-major = X [n×d] · Πᵀ
    // In col-major (cuBLAS): Yᵀ [d×n] = Π [d×d] · Xᵀ [d×n]
    // X [n×d] row-major  ≡ Xᵀ [d×n] col-major  (same bytes, lda = d)
    // Y [n×d] row-major  ≡ Yᵀ [d×n] col-major  (same bytes, lda = d)
...
{
    // Decode primary reconstruction on CPU, compute residuals, run 1D k-means.
    // We D2H the rotated training vectors and codes, then use CPU codebook.
    ...
    // Collect all per-dimension residuals into a flat array
    ...
    // 1D k-means (CPU) — residuals are mixture of truncated Gaussians
...
void JHQGpuIndex::train(const float* h_x, int n_train) {
    // ── CPU: QR + sigma + Lloyd-Max codebook ─────────────────────────────────
    jl_.estimate_sigma(h_x, n_train);
    cb_ = std::make_unique<LloydMaxCodebook>(d_, M_, B_, jl_.sigma());

    // ── Upload rotation matrix Π to GPU ──────────────────────────────────────
    {
        // Pi_.data() is Π col-major from LAPACK — upload directly.
    ...
    // ── GPU: rotate training vectors, primary encode ──────────────────────────
    ...
    // ── CPU 1D k-means on residuals, upload res_c1d ──────────────────────────
...
void JHQGpuIndex::add(const float* h_x, int n) {
    ...
    // Grow permanent GPU buffers to hold ntotal_ + n vectors
...
void JHQGpuIndex::search(const float* h_q, int nq, int k,
    ...
    // Upload query batch (small, nq*d floats — not part of the hot workspace)
    float* d_q = nullptr;
