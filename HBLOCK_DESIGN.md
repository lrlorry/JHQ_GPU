...
### Recall / Routing 优化方向 3：分层旋转级联（Hierarchical Projection Cascade）

**核心洞察**：query 进来本质上是一个方向（角度），routing 的目标只是保证 top-k 邻居落在同一 cell，
不需要精确保持距离——这是比 metric preservation 弱得多的条件，可以用更激进的投影。

**当前问题**：
JL 是随机投影，信息均匀分散到 768 维。用前 6 维做 L1 路由，只用到了全部信息的 6/768 ≈ 0.8%，
路由精度受限，导致需要较大的 ck3 才能保证 recall，即更多 HBM 读。

**提出方案：PCA 级联 + 残差路由**

```
现在：
  Pi (768×768, 随机正交) → 用前 6 维做 L1，前 7 维做 L2

改为：
  P1 (768 × k1)：训练数据的 top-k1 PCA 主成分
                  把全局最大方差方向压到前 k1 维 → L1 路由
  r1 = x - C1[c1]（L1 残差，768D）

  P2 (768 × k2)：r1 的 top-k2 PCA 主成分（在各 L1 cell 内训练）
                  把 r1 的最大差异压到前 k2 维 → L2 路由
  r2 = r1 - C2[c2]（L2 残差 → fine PQ）
```

**k1, k2 的选取**：
- K1=64 cells → 需要 k1 = log2(64) = 6 维足够表示 64 个方向（若用 k-means 路由可以更多）
- K2=128 cells → k2 = 7 维
- 实际可以取 k1=16, k2=16（冗余维度让 k-means 更好区分边界情况）

**非正交投影**：
- P1, P2 不要求正交，只要保证 top-k 邻居路由精度即可
- 可用 SGD/度量学习优化：最大化"真实邻居落在同一 cell"的概率
- 非正交时等价于 metric learning 里的 Mahalanobis 距离学习

**存储对比**：

| 矩阵 | 当前 JL | 分层投影 |
|------|---------|---------|
| L1 routing | Pi 768×768 = 2.25MB（全矩阵） | P1 768×k1 = 18KB |
| L2 routing | 同一个 Pi，取不同维度 | P2 768×k2 = 21KB |
| 合计 | 2.25MB | **< 40KB** |

**对 HBM 随机读的影响**：
更精准的 routing → 同样 recall 需要更小的 ck3 → 更少 leaf block → 更少 HBM 读：
- 若 ck3 从 256 降到 64（精准 routing 实现）：HBM 读量从 3.2GB → 0.8GB，LeafFine 43ms → ~11ms
- 与转置 kernel（思路 A）叠加：效果相乘

**Search 开销**：
- 当前：cublasSgemm (768×768) × B → 旋转开销较大
- 改后：z1 = P1 @ q（768×k1 矩阵乘）+ z2 = P2 @ r1（768×k2 矩阵乘）
- 计算量从 O(d²×B) 降到 O(d×(k1+k2)×B)，约 50x 减少

**Build 流程**：
```
1. 对训练数据做 SVD → P1 (top k1 左奇异向量)
2. 用 P1 做 L1 k-means（在 k1 维子空间里）
3. 计算每个向量的 L1 残差 r1
4. 对 r1 做 SVD → P2（可以 per-L1-cell 各训练一个，也可以全局）
5. 用 P2 做 L2 k-means
```

**Bonus：连 JL rotation（2.25MB 矩阵）都可以省掉**，直接用 P1/P2 替代。

---

### Recall 优化方向 2：JL + per-dimension sigma Lloyd-Max

**背景**：JHQ 选择 JL 的原因不是 JL 最优，而是 JL 让 codebook 变得极其简单——
旋转后维度近似独立且近似 N(0, σ²)，Lloyd-Max 解析解直接可用，无需训练。

若要在保住"解析码本"这一优点的前提下做增强，最优方案不是 OPQ，而是：

**JL + per-dimension sigma Lloyd-Max**

原理：JL 旋转后各维度近似独立，但 σ_j 并不完全相等
（有限样本效应 + L1/L2 残差后方差略有结构性差异）。
对 N(0, σ_j²) 的 Kr 级标量 Lloyd-Max 量化质心有解析解：

```
c_i(j) = σ_j × lloyd_max_unit[i]    (lloyd_max_unit 是单位 Gaussian 的标准质心表)
```

只需存 σ_j，质心随时推算，不需要 k-means。

**当前问题**：`d_fine_c1d` 只有 Kr=16 个值，所有 768 维用同一套质心，
等价于假设所有维度 σ_j 完全相同。这是一个不必要的简化。
...
|------|---------|---------|---------|
| 当前（全局单一 σ）| 64B | 零 | 基线 |
| 全局 per-dim σ | d × 4B = 3KB | 统计一次 | 小幅提升 |
| per-block per-dim σ | n_blocks × d × 4B = **24MB** | 每 block 统计 σ | 中等提升，最优性价比 |
| per-block 完整 codebook | n_blocks × d × Kr × 4B = 402MB | k-means 训练 | 最大提升，代价高 |

**per-block per-dim σ 是最优性价比的点**：
- 仅 24MB，比 per-block codebook（402MB）便宜 17x
- 既捕获 block 内局部方差结构，又保住解析推导（无需训练 codebook）
- Build 阶段：对每个 leaf block 的向量统计每维的标准差 σ_j → 存储
- Search 阶段：LUT 构建时用 `d_sigma[block_idx * d + j]` 缩放质心位置

**与 OPQ 的比较**：
- OPQ：找最优旋转 R 使子空间方差均匀 → search 时要额外旋转 query（ck3 次矩阵乘）
- JL + per-sigma LM：用随机 JL 近似均匀化，再用 σ_j 校正剩余不均匀性 → search 时无额外旋转，只是 LUT 质心位置不同
- 后者保留了 JL 的简洁性，实现成本更低

#### 详细方案：Per-block per-dim σ Lloyd-Max
...
现有流程：
  x_rot = Pi @ x
  L1_code = argmin ||x_rot - C1||²;  r1 = x_rot - C1[L1_code]
  L2_code = argmin ||r1    - C2||²;  r2 = r1    - C2[L2_code]
  fine_code = scalar_PQ(r2, 全局 d_fine_c1d)   ← 目前这步用全局质心

新增步骤（在 add() 里，组织好 leaf block 后）：
  for each leaf block b:
      收集该 block 内所有向量的 r2（~114 个向量 × 768 维）
      sigma[b][j] = std(r2[:, j])   for j in 0..767
      # 共 8192 × 768 × 4B = 24MB

编码时用 per-block 质心：
  c_i(b, j) = sigma[b][j] * lloyd_max_unit[i]
  fine_code[j] = argmin_i |r2[j] - c_i(b, j)|
```

`lloyd_max_unit[Kr]`：单位 Gaussian N(0,1) 的 Kr 级 Lloyd-Max 最优质心（16 个常数，编译期写死）。

**Search 阶段变化**

省掉 `build_fine_lut_kernel`，把距离计算移入 `leaf_fine_compute_kernel`：

```
// 现在（全局 LUT）：
build_fine_lut_kernel → lut_fine[B × d × Kr]（12MB）
leaf_fine_compute     → fd += lut_fine[bqi * d * Kr + j * Kr + ri]

// 改后（per-block on-the-fly）：
// 不需要 LUT，直接在 leaf_fine_compute 里计算：
float sigma_j = d_sigma[leaf_idx * d + j];          // L2 hit（24MB 驻留）
float c_ri    = sigma_j * lloyd_max_unit[ri];        // ri 已解码，lloyd_max_unit 在寄存器
float qr2_j   = d_q_r2[bqi * d + j];                // L2 hit（768KB）
fd += (qr2_j - c_ri) * (qr2_j - c_ri);
```
...
| 数据 | 大小 | 访问模式 | 是否 L2 驻留 |
|------|------|---------|------------|
| d_sigma[n_blocks × d] | 24MB | 按 leaf_idx 随机，但 24MB < L2 40MB | 是 |
| d_q_r2[B × d] | 768KB | 顺序（per query） | 是 |
| lloyd_max_unit[Kr] | 64B | 常量，进寄存器 | 寄存器 |
| ~~lut_fine~~ | ~~12MB~~ | 省掉 | — |
...
| 阶段 | 现有 | 改后 | 差异 |
|------|------|------|------|
| Build | 全局统计一次 σ | 每 block 统计 σ，~7 亿次运算 | +几秒 |
| 额外存储 | 64B（全局质心） | 24MB（per-block sigma）| +24MB |
| Search LUT build | 0.049ms | **0ms（省掉）** | -0.049ms |
| Search LeafFine | 查 lut_fine | on-the-fly 算，多几次乘法 | HBM bound，基本不变 |
| 净效果 | 基线 | 省一个 kernel，recall 提升 | 正收益 |

**实现要点**

1. `encode.cu`：`build_leaf_blocks()` 后新增 `compute_block_sigma()` 函数
2. GPU 上存 `d_sigma[n_leaf_blocks * d]`（24MB），随 index 常驻
3. `search.cu`：删除 `build_fine_lut_kernel` 及其调用；`leaf_fine_compute_kernel` 签名增加 `d_sigma` 和 `d_q_r2` 参数，删除 `lut_fine` 参数
4. `d_q_r2` 已经在 workspace 里，直接传入即可
