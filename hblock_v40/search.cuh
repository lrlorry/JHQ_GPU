...
// v40: split-leaf version of the v38 int8 path. Routing and graph traversal
// are unchanged; PQ selection and exact rerank are separate kernels so their
// payloads can be streamed independently in later versions.
// beam_size = ef (no cap, v35/v36 semantics).
// Output: d_leaf_sel records EXPANDED blocks in expansion order (v30 semantics).
//
// SIFT bvecs are shifted from uint8 to int8 without changing L2 distance.
// The routing/beam-search pipeline (JL projection, L1/L2/L3
// centroid routing, block graph traversal) is unchanged and stays float —
// only the exact-rerank stage reads int8 directly and accumulates in int32.
struct SearchWorkspace {
    ...
    // PQ top-p vector IDs [max_pairs × per_block_r]. This is the hand-off
    // between the future code-region and raw-region pipelines.
...
// Kernel A: per-(query, block) PQ scan -> top per_block_r vector IDs.
...
// Kernel B: exact int8 L2 for Kernel A's candidates -> per-block top klocal.
void launch_leaf_exact_v40(
    const int*     d_pair_qids,
    const int*     d_candidate_ids,
    const int8_t*  d_base_vecs_i8,
