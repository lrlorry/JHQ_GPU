#include "hblock_v40/jhq_gpu_index.cuh"
      ...
      add_batch_size_(p.add_batch_size)
{
    ...
    if (p.leaf_size != 128)      throw std::invalid_argument("v40 requires leaf_size=128");
    if (p.per_block_r <= 0 || p.per_block_r > 32)
    ...
    if (p.add_batch_size <= 0)   throw std::invalid_argument("add_batch_size must be positive");
...
HBlockIndex::~HBlockIndex()
{
    ...
    FH(ws_.h_q_pinned); FH(ws_.h_q_pinned_i8);
    FH(ws_.h_leaf_cnt); FH(ws_.h_final_dists); FH(ws_.h_final_ids);
    FH(ws_.h_top1_ids); FH(ws_.h_top2_beam); FH(ws_.h_top3_beam); FH(ws_.h_block_sel);
    FD(ws_.d_q_batch); FD(ws_.d_q_batch_i8); FD(ws_.d_q_proj1); FD(ws_.d_dots1);
    ...
    FD(ws_.d_pq_candidate_ids); FD(ws_.d_out_dists); FD(ws_.d_out_ids);
...
static __global__ void i8_to_float_kernel(
    const int8_t* __restrict__ src, float* __restrict__ dst, long long n)
{
...
void HBlockIndex::train(const float* h_x, int n_train)
{
    ...
    printf("[v35 train] d=%d d_proj=%d K1=%d K2=%d K3=%d ck1=%d ck2=%d ck3=%d"
    ...
    printf("[v40 train] total=%.1f ms  sigma_r3=%.4f\n", Ms(Clock::now()-T0).count(), sigma);
}

void HBlockIndex::add(BVecsReader& reader, long long requested_n)
{
    if (!d_Pi1_)      throw std::runtime_error("call train() before add()");
    if (ntotal_ != 0) throw std::runtime_error("HBlock supports one add() call");
    if (!reader.f || reader.dim != d_) throw std::invalid_argument("invalid bvecs reader or dimension");
    const long long use_n = requested_n < 0 ? reader.npts : std::min(requested_n, reader.npts);
    if (use_n <= 0 || use_n > INT32_MAX)
        throw std::invalid_argument("v40 requires 0 < n <= INT32_MAX");
    const int n = static_cast<int>(use_n);
    using Clock = std::chrono::high_resolution_clock;
    auto T_add = Clock::now();
    printf("[v40 add] n=%d  d=%d  batch=%d  mini_km_iters=%d  mini_km_max_cell=%d\n",
           n, d_, add_batch_size_, mini_km_iters_, mini_km_max_cell_);

    const int BATCH = std::min(add_batch_size_, n);
    const float one=1.f, zero=0.f;
    CUBLAS_CHECK(cublasSetStream(cublas_, nullptr));
    ...
    CUDA_CHECK(cudaMalloc(&d_x_i8,  (long long)BATCH*d_));
    ...
    CUDA_CHECK(cudaMalloc(&d_base_vecs_i8_, (long long)n*d_));
    ...
    std::vector<int8_t>  h_i8_batch;

    int n_batches = (n + BATCH - 1) / BATCH;
    int report_every = std::max(1, n_batches / 10);
    for (int s=0;s<n;s+=BATCH) {
        int nb=std::min(BATCH,n-s);
        const int got = reader.read_batch(s, nb, h_i8_batch);
        if (got != nb) throw std::runtime_error("short bvecs read during add()");
        const long long elems = (long long)nb * d_;
        CUDA_CHECK(cudaMemcpy(d_x_i8, h_i8_batch.data(), elems, cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(d_base_vecs_i8_ + (long long)s*d_,
                              h_i8_batch.data(), elems, cudaMemcpyHostToDevice));
        i8_to_float_kernel<<<(unsigned)((elems + 255) / 256), 256>>>(d_x_i8, d_x, elems);
    ...
    cudaFree(d_x_i8);cudaFree(d_x);cudaFree(d_proj1);cudaFree(d_proj2);cudaFree(d_proj3);
    ...
    // Sort by (L3 cell, projected coordinate). The secondary key gives large
    // cells a cheap locality-preserving order when exact balanced mini-kmeans
    // would be too expensive.
    std::vector<long long> h_keys(n);
    for (int i = 0; i < n; i++) {
        const uint64_t cell = (uint64_t)h_code1[i]*K2K3 +
                              (uint64_t)h_code2[i]*K3_ + h_code3[i];
        uint32_t bits = 0;
        const float coordinate = h_proj1_all[(long long)i*d_proj_];
        std::memcpy(&bits, &coordinate, sizeof(bits));
        const uint32_t ordered = (bits & 0x80000000u) ? ~bits :
                                 (bits ^ 0x80000000u);
        h_keys[i] = (long long)((cell << 32) | ordered);
    }
    {
    ...
    std::vector<long long>().swap(h_keys);
    ...
    if (mini_km_iters_ > 0) {
        ...
        printf("  [block packing] balanced_kmeans=%d  radix_large_cells=%d  %.1f ms\n",
               n_cells_reordered, n_large_cells_radix,
               Ms(Clock::now()-T0).count());
    }
    ...
    h_block_cent_proj_.assign((long long)total_blocks*d_proj_, 0.f);
    ...
    for(int i=0,j;i<n;i=j){
        ...
        auto pack_one = [&](int blk, int pos, int oid) {
            ...
            const float* xv=h_proj1_all.data()+(long long)oid*d_proj_;
            float* bc=h_block_cent_proj_.data()+(long long)blk*d_proj_;
            for(int dim=0;dim<d_proj_;dim++) bc[dim]+=xv[dim];
        };
        if (blk_szs.empty()) {
            // Fixed-size chunks. Large cells already have a locality order
            // from the secondary radix key; one-block cells are unchanged.
    ...
    for(int b=0;b<total_blocks;b++){
        ...
        float inv=1.0f/sz;
        float* bc=h_block_cent_proj_.data()+(long long)b*d_proj_;
        for(int dim=0;dim<d_proj_;dim++) bc[dim]*=inv;
    }
    printf("  [pack+centroids] %.1f ms\n", Ms(Clock::now()-T0).count());

    // The packed leaf arrays and block centroids are now self-contained.
    // Release the O(N*d_proj) and O(N*bpv) build intermediates before the
    // leaf upload and graph build to keep SIFT100M host memory bounded.
    std::vector<float>().swap(h_proj1_all);
    std::vector<uint8_t>().swap(h_fc_all);
    std::vector<int>().swap(h_code1);
    std::vector<int>().swap(h_code2);
    std::vector<int>().swap(h_code3);
    std::vector<int>().swap(order);
    std::vector<int8_t>().swap(h_i8_batch);
    ...
    printf("  [projected centroid norms] %.1f ms\n", Ms(Clock::now()-T0).count());
    ...
    std::vector<uint8_t>().swap(h_leaf_codes);
    std::vector<int>().swap(h_leaf_ids);
    std::vector<int>().swap(h_leaf_sizes);
    ...
    printf("[v40 add total] %.1f ms  blocks=%d\n",
...
#define FD(p) do{if(ws_.p){cudaFree(ws_.p);ws_.p=nullptr;}}while(0)
    FH(h_q_pinned);FH(h_q_pinned_i8);FH(h_leaf_cnt);FH(h_final_dists);FH(h_final_ids);
    ...
    FD(d_q_batch);FD(d_q_batch_i8);FD(d_q_proj1);FD(d_dots1);FD(d_top1_ids);
    ...
    // Explicit hand-off between PQ scan and exact rerank.
    FD(d_pq_candidate_ids);FD(d_out_dists);FD(d_out_ids);
    ...
    // Query worst-case temporary storage for all 32 key bits. SIFT100M fits
    // in 20 block-id bits, but the workspace remains valid for later 1B use.
    cub::DeviceRadixSort::SortPairs(nullptr,sort_leaf,(int*)nullptr,(int*)nullptr,(int*)nullptr,(int*)nullptr,max_pairs,0,32);
    cub::DeviceRadixSort::SortPairs(nullptr,sort_qid, (int*)nullptr,(int*)nullptr,(int*)nullptr,(int*)nullptr,max_pairs,0,32);
...
{
    if(!h_q || !h_dists || !h_ids) throw std::invalid_argument("null search buffer");
    if(nq<=0)       throw std::invalid_argument("nq must be positive");
    ...
    ws_.beam_size    = ef;                 // v35: beam = ef, no cap
    ws_.max_leaf_sel = ef;                 // d_leaf_sel holds up to ef expanded blocks

    using Clock=std::chrono::high_resolution_clock;
    double ms_route=0,ms_trav=0,ms_pairs=0,ms_pq=0,ms_exact=0,ms_merge=0,ms_d2h=0;
    ...
    for(int qstart=0;qstart<nq;qstart+=batch_size_){
        ...
        const int8_t* h_qb=h_q+(long long)qstart*d_;

        const long long query_elems=(long long)nb*d_;
        std::memcpy(ws_.h_q_pinned_i8,h_qb,query_elems*sizeof(int8_t));
        if(nb<batch_size_)
            std::memset(ws_.h_q_pinned_i8+query_elems,0,
                        (long long)(batch_size_-nb)*d_*sizeof(int8_t));
        for(long long i=0;i<query_elems;i++)
            ws_.h_q_pinned[i]=(float)ws_.h_q_pinned_i8[i];
        CUDA_CHECK(cudaMemcpyAsync(ws_.d_q_batch_i8,ws_.h_q_pinned_i8,
                                   (long long)batch_size_*d_*sizeof(int8_t),
        ...
        route_gpu_v29(cublas_,
                      ...
                      d_route2_cents_proj_,d_route2_cents_full_,d_route2_norms_,
                      d_route3_cents_proj_,d_route3_cents_full_,d_route3_norms_,
                      d_fine_c1d_,ws_.h_q_pinned,nb,d_,d_proj_,
        ...
        if(n_pairs==0){
            std::fill(h_dists+(long long)qstart*k,
                      h_dists+(long long)(qstart+nb)*k,
                      std::numeric_limits<float>::infinity());
            std::fill(h_ids+(long long)qstart*k,
                      h_ids+(long long)(qstart+nb)*k,-1);
            continue;
        }
        ...
        // Kernel A: per-block PQ top-p. This output is the streaming boundary.
        auto t4=Clock::now();
        launch_leaf_pq_topk_v40(
            ws_.d_pair_leaf_b, ws_.d_pair_qid_b,
            d_leaf_codes_, d_leaf_ids_, d_leaf_sizes_,
            ws_.d_lut_fine,
            ws_.d_pq_candidate_ids,
            n_pairs, d_, Kr_, Br_, bpv_, leaf_size_,
            per_block_r_, s);
        ms_pq+=Ms(Clock::now()-t4).count();

        // Kernel B: exact int8 L2 only for Kernel A's candidate IDs.
        auto t4b=Clock::now();
        launch_leaf_exact_v40(
            ws_.d_pair_qid_b, ws_.d_pq_candidate_ids,
            d_base_vecs_i8_, ws_.d_q_batch_i8,
            ws_.d_out_dists, ws_.d_out_ids,
            n_pairs, d_, leaf_size_, per_block_r_, klocal_, s);
        ms_exact+=Ms(Clock::now()-t4b).count();
    ...
    // printf("  [v40] Route=%.2f Traverse=%.2f Pairs=%.2f PQ=%.2f Exact=%.2f Merge=%.2f D2H=%.2f ms\n",
    //        ms_route,ms_trav,ms_pairs,ms_pq,ms_exact,ms_merge,ms_d2h);
    // printf("  [v40 stats] avg_visited=%.1f  avg_pairs=%.1f  (over %d queries)\n",
