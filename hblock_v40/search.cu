...
// v40: v38 int8 routing/graph semantics with the fused leaf kernel split into
// a PQ top-p stage and an exact int8-L2 stage. This is the boundary needed for
// independent code/raw payload streaming in later versions.
...
void route_gpu_v29(
    ...
    SearchWorkspace& ws)
{
    if (nq > ws.batch_cap) throw std::runtime_error("hblock_v40: nq > batch_cap");
    const int B = ws.batch_cap, BLOCK = 128;
    const float one = 1.f, zero = 0.f;
    cudaStream_t s = ws.stream;

    if (h_queries != ws.h_q_pinned)
...
__host__ __device__ __forceinline__ size_t align4(size_t x) {
    return (x + 3u) & ~size_t(3u);
}

__global__ void leaf_pq_topk_kernel_v40(
    ...
    int d, int Kr, int Br, int bpv, int leaf_size, int per_block_r)
{
    ...
    const uint8_t* leaf_base =
        leaf_codes + (long long)leaf_blk * bpv * leaf_size;
    ...
    if (tid < n_vecs) {
        ...
        if (Br == 4) {
            for (int b = 0; b < bpv; ++b) {
                const uint8_t c = __ldg(&leaf_base[b * leaf_size + tid]);
                const int j0 = b * 2;
    ...
    if (tid == 0) {
        ...
        for (int r = 0; r < actual_r; ++r) {
            ...
            d_candidate_ids[(long long)pi * per_block_r + r] =
                (best_w >= 0 && best_d < INF) ? s_id[ptrs[best_w]] : -1;
...
void launch_leaf_pq_topk_v40(
    const int* d_pair_leaf_ids, const int* d_pair_qids,
    const uint8_t* d_leaf_codes, const int* d_leaf_ids_data,
    const int* d_leaf_sizes, const float* d_lut_fine,
    int* d_candidate_ids,
    int n_pairs, int d, int Kr, int Br, int bpv, int leaf_size,
    int per_block_r, cudaStream_t stream)
{
    const size_t smem = (size_t)leaf_size * (sizeof(float) + sizeof(int));
    leaf_pq_topk_kernel_v40<<<n_pairs, leaf_size, (int)smem, stream>>>(
        d_pair_leaf_ids, d_pair_qids,
        d_leaf_sizes, d_leaf_codes, d_leaf_ids_data,
        d_lut_fine, d_candidate_ids,
        d, Kr, Br, bpv, leaf_size, per_block_r);
    CUDA_CHECK(cudaGetLastError());
}

__global__ void leaf_exact_kernel_v40(
    const int*    __restrict__ d_pair_qids,
    ...
    int d, int per_block_r, int klocal)
{
    const float INF = __int_as_float(0x7F800000);
    constexpr unsigned FULL = 0xffffffff;
    const int pi = blockIdx.x;
        ...
        for (int slot = 0; slot < klocal; ++slot) {
            ...
            if (best_r >= 0) {
                ...
                d_out_ids[out_base + slot] =
                    d_candidate_ids[(long long)pi * per_block_r + best_r];
                used[best_r] = true;
            }
        }
    }
...
void launch_leaf_exact_v40(
    const int* d_pair_qids, const int* d_candidate_ids,
    const int8_t* d_base_vecs_i8, const int8_t* d_q_batch_i8,
    float* d_out_dists, int* d_out_ids,
    int n_pairs, int d, int leaf_size, int per_block_r, int klocal,
    cudaStream_t stream)
{
    size_t smem = (size_t)d * sizeof(int8_t);
    smem = align4(smem);
    smem += 4 * sizeof(int) + (size_t)per_block_r * sizeof(float);
    leaf_exact_kernel_v40<<<n_pairs, leaf_size, (int)smem, stream>>>(
        d_pair_qids, d_candidate_ids,
        d_base_vecs_i8, d_q_batch_i8,
        d_out_dists, d_out_ids,
        d, per_block_r, klocal);
    CUDA_CHECK(cudaGetLastError());
}
