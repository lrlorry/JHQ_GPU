...
{
    assign_from_dots_kernel<<<(n + 255) / 256, 256, 0, s>>>(
        d_dots, d_cent_norms, d_assigns, K, n);
    CUDA_CHECK(cudaGetLastError());
}
...
__global__ void kmeans_update_kernel(
    const float* __restrict__ d_x_proj,
    const int*   __restrict__ d_assigns,
    float*                    d_new_cents,
    int*                      d_counts,
    int n, int d_proj, int K)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    int c = d_assigns[i];
    ...
    for (int j = 0; j < d_proj; ++j)
        atomicAdd(&d_new_cents[(long long)c * d_proj + j],
                  d_x_proj[(long long)i * d_proj + j]);
}

__global__ void kmeans_normalize_kernel(float* d_cents, const int* d_counts, int K, int d_proj)
{
    int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= K) return;
    int cnt = d_counts[c];
    if (cnt <= 0) return;  // empty cluster: center stays at 0 (handled by caller)
    for (int j = 0; j < d_proj; ++j)
        d_cents[(long long)c * d_proj + j] /= (float)cnt;
}
...
{
    kmeans_update_kernel<<<(n + 255) / 256, 256, 0, s>>>(
        d_x_proj, d_assigns, d_new_cents, d_counts, n, d_proj, K);
    CUDA_CHECK(cudaGetLastError());
    kmeans_normalize_kernel<<<(K + 255) / 256, 256, 0, s>>>(
        d_new_cents, d_counts, K, d_proj);
    CUDA_CHECK(cudaGetLastError());
}
