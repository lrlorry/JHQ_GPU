...
// all access the same 48KB LUT region → L2 stays warm across slots).
// Leaf codes use AoS [n_blocks, leaf_size, bpv]: each thread reads its own
// 3 consecutive cache lines sequentially, then L1-hits for all 768 j iterations.
// Occupancy: ~100% (no shared memory constraint) vs 18% in v1.
...
{
    const float INF = __int_as_float(0x7F800000);
    int bqi  = blockIdx.x;  // query index
    int slot = blockIdx.y;  // leaf slot index
    int v    = threadIdx.x; // vector within the leaf block
    ...
    // AoS: thread v's codes are in 3 consecutive cache lines → loaded once,
    // then 768 j iterations are all L1 hits.
