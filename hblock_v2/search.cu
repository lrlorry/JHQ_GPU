...
// where coarse_dist is computed DIRECTLY from d_q_rot + d_c1d_coarse + d_coarse_t.
//
// hblock_v2 vs hblock_v1: no LUT build for coarse. With ck1≤256 candidates,
// direct computation costs ck1×M×Ds ≈ 30K FMAs, vs 50M FMAs for a full LUT build.
//
__global__ void coarse_cascade_kernel(
    const float*   __restrict__ d_q_rot,          // [B, d]
    const float*   __restrict__ d_c1d_coarse,     // [K1D] centroids (K1D=2, tiny)
    const int*     __restrict__ topck1_pos,        // [B, ck1]
    const float*   __restrict__ topck1_primary,    // [B, ck1]
    const uint8_t* __restrict__ d_coarse_t,        // [M, N] coarse codes (IVF order, transposed)
    int*                        topck2_pos,         // [B, ck2] output
    float*                      topck2_primary,     // [B, ck2] primary_dist of survivors
    int ck1, int ck2, int M, long long N,
    int d, int Ds, int K1D, int bits_per_dim)
{
    const float INF   = __int_as_float(0x7F800000);
    const int   BLOCK = blockDim.x;
    int bqi = blockIdx.x, tid = threadIdx.x;
    ...
    const float* my_q    = d_q_rot         + (long long)bqi * d;
    ...
    for (int ci = tid; ci < ck1; ci += BLOCK) {
        ...
        #pragma unroll 4
        for (int m = 0; m < M; ++m) {
            uint8_t byte = __ldg(&d_coarse_t[(long long)m * N + abs_pos]);
            const float* qm = my_q + m * Ds;
            for (int kk = 0; kk < Ds; ++kk) {
                int j = (byte >> (kk * bits_per_dim)) & kmask;
                float diff = qm[kk] - __ldg(&d_c1d_coarse[j]);
                cd += diff * diff;
            }
        }
    ...
    int ck1, int ck2, int k, long long ntotal)
{
    ...
    // 6. Coarse cascade → top-ck2 (direct computation — no LUT build)
    coarse_cascade_kernel<<<B, BLOCK, cascade_smem, ws.stream>>>(
        ws.d_q_rot, d_c1d_coarse,
        ws.d_topck1_pos, ws.d_topck1_primary,
        d_coarse_t,
        ws.d_topck2_pos, ws.d_topck2_primary,
        ck1, ck2, M, ntotal, d, Ds, K1D, bits_per_dim);
    ...
    // 8. Fine residual refine on ck2 survivors
    {
    ...
    // 9. Final top-k from ck2 refined candidates
