...
// hblock_v2: three-stage cascade (direct coarse distance — no LUT rebuild)
//   1. IVF primary scan (coalesced [M,N], same as v12) → top-ck1
//   2. Coarse cascade: compute dist directly from d_q_rot+d_c1d_coarse+d_coarse_t → top-ck2
//   3. Fine residual refine (same as v12) → top-k
//
// vs hblock_v1: removes coarse LUT build (50M FMAs) in favour of direct computation
//   (ck1 × M × Ds ≈ 30K FMAs for typical ck1=40, M=96, Ds=8).
//
