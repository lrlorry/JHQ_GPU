// hblock_v23/search.cu  — GPU kernels identical to v22, namespace hblock_v23.
...
void route_gpu_v23(
    ...
    SearchWorkspace& ws)
{
    if (nq > ws.batch_cap) throw std::runtime_error("hblock_v23: nq > batch_cap");
    ...
    CUBLAS_CHECK(cublasSgemm(cublas, CUBLAS_OP_T, CUBLAS_OP_N,
                             K1, B, d_proj, &one,
                             d_route1_cents_proj, d_proj, ws.d_q_proj1, d_proj, &zero, ws.d_dots1, K1));
    {
    ...
    CUDA_CHECK(cudaMemcpyAsync(ws.h_top1_ids,  ws.d_top1_ids,
                               (long long)B*ck1*sizeof(int), cudaMemcpyDeviceToHost, s));
    CUDA_CHECK(cudaMemcpyAsync(ws.h_top2_beam, ws.d_top2_beam,
                               (long long)B*ck1*ck2*sizeof(int), cudaMemcpyDeviceToHost, s));
    CUDA_CHECK(cudaMemcpyAsync(ws.h_top3_beam, ws.d_top3_beam,
                               (long long)B*ck1*ck2*ck3*sizeof(int), cudaMemcpyDeviceToHost, s));
    CUDA_CHECK(cudaStreamSynchronize(s));
}

__global__ void build_pairs_kernel_v23(
...
void gpu_build_and_sort_pairs_v23(
    ...
    int max_leaf_sel, SearchWorkspace& ws)
{
    ...
    build_pairs_kernel_v23<<<(nq+255)/256, 256, 0, s>>>(
...
__global__ void leaf_flat_kernel_v23(
    ...
    int d, int Kr, int Br, int bpv, int leaf_size, int top_p)
{
    ...
    int leaf_blk = d_pair_leaf_ids[pi], qid = d_pair_qids[pi];
...
void launch_leaf_flat_v23(
    ...
    const float* d_lut_fine, float* d_out_dists, int* d_out_ids,
    int n_pairs, int d, int Kr, int Br, int bpv, int leaf_size, cudaStream_t stream)
{
    int smem = leaf_size*(int)sizeof(float) + leaf_size*(int)sizeof(int)
             + 4*(int)sizeof(float) + 4*(int)sizeof(int);
    leaf_flat_kernel_v23<<<n_pairs, leaf_size, smem, stream>>>(
...
__global__ void fill_iota_kernel_v23(int* arr, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) arr[i] = i;
}

__global__ void segmented_topk_kernel_v23(
...
void gpu_merge_pq_topk_v23(int nq, int n_pairs, int rerank_r, SearchWorkspace& ws)
{
    cudaStream_t s = ws.stream;
    int k = std::min(rerank_r, (int)K_MAX);
    fill_iota_kernel_v23<<<(n_pairs+255)/256, 256, 0, s>>>(ws.d_pair_leaf_a, n_pairs);
    CUDA_CHECK(cub::DeviceRadixSort::SortPairs(
        ws.d_cub_tmp, ws.cub_bytes,
    ...
    segmented_topk_kernel_v23<<<nq, BLOCK, smem, s>>>(
...
__global__ void gather_vecs_kernel_v23(
    const float* __restrict__ d_base_vecs,
    const int*   __restrict__ d_cand_ids,
    float* d_cand_vecs, int R, int d)
{
    int qi = blockIdx.y, ri = blockIdx.x;
    int id = d_cand_ids[(long long)qi * R + ri];
    if (id < 0) return;
    float* out = d_cand_vecs + ((long long)qi * R + ri) * d;
    const float* src = d_base_vecs + (long long)id * d;
    for (int j = threadIdx.x; j < d; j += blockDim.x) out[j] = src[j];
}

void launch_gather_vecs_v23(
    const float* d_base_vecs, const int* d_cand_ids, float* d_cand_vecs,
    int B, int R, int d, cudaStream_t s)
{
    dim3 grid(R, B);
    gather_vecs_kernel_v23<<<grid, 128, 0, s>>>(d_base_vecs, d_cand_ids, d_cand_vecs, R, d);
    CUDA_CHECK(cudaGetLastError());
}

__global__ void exact_rerank_kernel_v23(
    const float* __restrict__ d_q_batch,
    const float* __restrict__ d_cand_vecs,
    const int*   __restrict__ d_cand_ids,
    float* d_exact_dists, float* d_final_dists, int* d_final_ids,
    int R, int d, int k)
{
    const float INF = __int_as_float(0x7F800000);
    int qi = blockIdx.x, tid = threadIdx.x;
    int lane = tid & 31, wid = tid >> 5, n_warps = blockDim.x >> 5;
    ...
    for (int ri = wid; ri < R; ri += n_warps) {
        ...
        if (ids[ri] >= 0) {
            ...
            for (int j = lane; j < d; j += 32) { float diff = q[j]-c[j]; l2sq += diff*diff; }
        }
        for (int mask = 16; mask > 0; mask >>= 1) l2sq += __shfl_xor_sync(0xffffffff, l2sq, mask);
    ...
    if (tid == 0) {
        ...
        for (int r = 0; r < R; ++r) {
            ...
            float dist = s_dots[r]; ed[r] = dist;
            int worst = 0;
            for (int i = 1; i < k; i++) if (out_d[i] > out_d[worst]) worst = i;
            if (dist < out_d[worst]) { out_d[worst] = dist; out_i[worst] = ids[r]; }
        }
    }
}

void launch_exact_rerank_v23(
    const float* d_q_batch, const float* d_cand_vecs, const int* d_cand_ids,
    float* d_exact_dists, float* d_final_dists, int* d_final_ids,
    int B, int R, int d, int k, cudaStream_t s)
{
    const int BLOCK = 128;
    int smem = R * (int)sizeof(float);
    exact_rerank_kernel_v23<<<B, BLOCK, smem, s>>>(
        d_q_batch, d_cand_vecs, d_cand_ids,
        d_exact_dists, d_final_dists, d_final_ids, R, d, k);
    CUDA_CHECK(cudaGetLastError());
}

// ─────────────────────────────────────────────────────────────────────────────
// GPU block graph beam search (CAGRA-style, one warp per query)
// ─────────────────────────────────────────────────────────────────────────────

// Projected L2² from shared query projection to block b
__device__ __forceinline__ float
blk_qdist_v23(const float* __restrict__ s_q, float q_norm,
...
// Atomic test-and-set; returns true on first visit
__device__ __forceinline__ bool
try_visit_v23(int* __restrict__ vis, int b, int n_blks)
{
...
// ─────────────────────────────────────────────────────────────────────────────
// P2/P6: GPU block adjacency build
// One thread per source block; iterates its per-cell candidate list and keeps
// top-degree neighbours in a register-based max-heap (degree ≤ 32).
// ─────────────────────────────────────────────────────────────────────────────
__global__ void compute_adj_kernel_v23(
    const float* __restrict__ d_blk_proj,   // [n_blks, d_proj]
    const float* __restrict__ d_blk_norm,   // [n_blks]
    const int*   __restrict__ d_csr_start,  // [n_cells+1]
    const int*   __restrict__ d_csr_list,   // [total_cands]
    const int*   __restrict__ d_cell_id,    // [n_blks]
    int*   d_adj,                           // [n_blks, degree]
    int n_blks, int d_proj, int degree)
{
    int b = blockIdx.x * blockDim.x + threadIdx.x;
    if(b >= n_blks) return;

    int cell = d_cell_id[b];
    int cs   = d_csr_start[cell];
    int cnt  = d_csr_start[cell + 1] - cs;

    const float* my_proj = d_blk_proj + (long long)b * d_proj;
    float my_norm = d_blk_norm[b];

    float hd[32]; int hi[32]; int hs = 0; float hmax = 1e38f;

    for(int ci = 0; ci < cnt; ci++){
        int cand = d_csr_list[cs + ci];
        if(cand == b) continue;
        const float* cp = d_blk_proj + (long long)cand * d_proj;
        float dot = 0.f;
        for(int jp = 0; jp < d_proj; jp++) dot += my_proj[jp] * cp[jp];
        float dist = my_norm + d_blk_norm[cand] - 2.f * dot;

        if(hs < degree){
            hd[hs] = dist; hi[hs] = cand; hs++;
            if(hs == degree){
                hmax = hd[0];
                for(int i = 1; i < degree; i++) if(hd[i] > hmax) hmax = hd[i];
            }
        } else if(dist < hmax){
            int mp = 0;
            for(int i = 1; i < degree; i++) if(hd[i] > hd[mp]) mp = i;
            hd[mp] = dist; hi[mp] = cand;
            hmax = hd[0];
            for(int i = 1; i < degree; i++) if(hd[i] > hmax) hmax = hd[i];
        }
    }

    int* my_adj = d_adj + (long long)b * degree;
    for(int r = 0; r < hs; r++) my_adj[r] = hi[r];
    for(int r = hs; r < degree; r++) my_adj[r] = -1;
}

void gpu_build_block_adj_v23(
...
{
    if(degree > 32)
        throw std::runtime_error("gpu_build_block_adj: graph_degree must be <= 32");
    int grid = (n_blks + 127) / 128;
    compute_adj_kernel_v23<<<grid, 128, 0, s>>>(
        d_blk_proj, d_blk_norm, d_csr_start, d_csr_list, d_cell_id,
        d_adj, n_blks, d_proj, degree);
    CUDA_CHECK(cudaGetLastError());
}

// ─────────────────────────────────────────────────────────────────────────────
// P4: Fused entry-selection + beam search (one warp per query).
// Entry selection reads routing tables directly (no cpu_select_entries step).
// Beam: 32 register slots, one per thread.
// Shared: s_q[d_proj] = projected query.
// ─────────────────────────────────────────────────────────────────────────────
__global__ void block_search_fused_v23(
    // Routing outputs (already on GPU after route_gpu_v23)
    const int*   __restrict__ d_top1,        // [B, ck1]
    const int*   __restrict__ d_top2,        // [B, ck1*ck2]
    const int*   __restrict__ d_top3,        // [B, ck1*ck2*ck3]
    // Cell→block mapping
    const int*   __restrict__ d_blk_start,   // [K1*K2*K3]
    const int*   __restrict__ d_blk_count,   // [K1*K2*K3]
    // Block graph
    const int*   __restrict__ d_block_adj,   // [n_blks, degree]
    const float* __restrict__ d_blk_proj,    // [n_blks, d_proj]
    const float* __restrict__ d_blk_norm,    // [n_blks]
    // Query projection (already on GPU)
    const float* __restrict__ d_q_proj1,     // [B, d_proj]
    // Output
    int*   d_leaf_sel,   // [B, max_ls]
    int*   d_leaf_cnt,   // [B]
    int*   d_visited,    // [B, bitmap_words]
    ...
    int entry_per_cell, int bitmap_words)
{
    ...
    float my_dist = 1e38f; int my_id = -1; bool my_exp = true;
    ...
    auto try_insert = [&](float nd, int ni) {
        ...
        if(nd >= bmax) return;
        unsigned em = __ballot_sync(FULL, my_dist == bmax);
        int el = __ffs(em) - 1;
        if(tid == el){ my_dist = nd; my_id = ni; my_exp = false; }
    };

    // ── Phase 1: Entry selection from routing output ──────────────────────────
    for(int i1 = 0; i1 < ck1; i1++){
        ...
        for(int i2 = 0; i2 < ck2; i2++){
            ...
            for(int i3 = 0; i3 < ck3; i3++){
                ...
                // Process blocks in chunks of 32; select top-entry_per_cell per chunk
                for(int start = 0; start < bc; start += 32){
                    int bi = start + tid;
                    float ck_d = 1e38f; int ck_id = -1;
                    if(bi < bc){
                        ck_d  = blk_qdist_v23(s_q, q_norm, d_blk_proj, d_blk_norm,
                                               bs + bi, d_proj);
                        ck_id = bs + bi;
                    }
                    ...
                    for(int ep = 0; ep < entry_per_cell; ep++){
                        ...
                        // tid==0 marks visited (avoid 32 atomic ops on same word)
                        bool fv = false;
                        if(tid == 0 && ins_id >= 0)
                            fv = try_visit_v23(vis, ins_id, n_blks);
    ...
    for(int iter = 0; iter < budget; iter++){
        float sd = my_exp ? 1e38f : my_dist;
        float mn = sd;
        for(int o = 16; o >= 1; o >>= 1)
            mn = fminf(mn, __shfl_xor_sync(FULL, mn, o));
        if(mn >= 1e37f) break;

        unsigned wm   = __ballot_sync(FULL, sd == mn);
        int win_lane  = __ffs(wm) - 1;
        int expand_id = __shfl_sync(FULL, my_id, win_lane);
        if(tid == win_lane) my_exp = true;

        if(tid == 0 && out_cnt < max_ls)
            d_leaf_sel[(long long)qi * max_ls + out_cnt] = expand_id;
        out_cnt++;

        float new_d = 1e38f; int new_id = -1;
        if(tid < degree){
            int nb = d_block_adj[(long long)expand_id * degree + tid];
            if(try_visit_v23(vis, nb, n_blks)){
                new_d  = blk_qdist_v23(s_q, q_norm, d_blk_proj, d_blk_norm, nb, d_proj);
                new_id = nb;
            }
        }
...
{
    if(degree > 32)
        throw std::runtime_error("gpu_block_search: graph_degree must be <= 32");
    int smem = d_proj * (int)sizeof(float);
    block_search_fused_v23<<<B, 32, smem, ws.stream>>>(
        ws.d_top1_ids, ws.d_top2_beam, ws.d_top3_beam,
        ...
        n_blks, d_proj, K2, K3,
        ck1, ck2, ck3,
        degree, budget, max_ls, entry_per_cell,
        ws.bitmap_words);
    CUDA_CHECK(cudaGetLastError());
}
