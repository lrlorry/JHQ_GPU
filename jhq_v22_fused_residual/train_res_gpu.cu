                                   ...
                                   cudaStream_t stream)
{
    ...
    CUDA_CHECK(cudaGetLastError());
    // Both of these ask for more than the 48 KB a kernel gets unasked: the
    // histogram wants nbuckets*(4+8) bytes and the Lloyd pass wants
    // (nbuckets+1)*16 plus the centroids. 2048 buckets keeps them at 24 and
    // 33 KB.
