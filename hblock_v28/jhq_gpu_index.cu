#include "hblock_v28/jhq_gpu_index.cuh"
...
HBlockIndex::HBlockIndex(int d, Params p)
    : d_(d), d_proj_(p.d_proj), Kr_(p.Kr), Br_(p.Br),
      ...
      beam_size_(p.beam_size), mini_km_iters_(p.mini_km_iters)
{
...
void HBlockIndex::train(const float* h_x, int n_train)
{
    ...
    printf("[v28 train] total=%.1f ms  sigma_r3=%.4f\n", Ms(Clock::now()-T0).count(), sigma);
}
...
void HBlockIndex::add(const float* h_x, int n)
{
    ...
    printf("[v28 add] n=%d  d=%d  batch=%d  mini_km_iters=%d\n", n, d_, 8192, mini_km_iters_);
    ...
    printf("[v28 add total] %.1f ms  blocks=%d\n",
                          ...
                          float* h_dists, int* h_ids) const
{
    ...
    for(int qstart=0;qstart<nq;qstart+=batch_size_){
        ...
        auto t3=Clock::now();
        gpu_build_and_sort_pairs_v28(nb,n_pairs,n_leaf_blocks_,ws_.max_leaf_sel,ws_);
        ms_pairs+=Ms(Clock::now()-t3).count();

        // Per-block: PQ filter → exact L2 → klocal top candidates
        auto t4=Clock::now();
        launch_leaf_flat_v28(
        ...
        auto t5=Clock::now();
        launch_final_merge_v28(nb, n_pairs, klocal_, k, ws_);
