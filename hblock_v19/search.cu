...
// ── L1 routing: top-ck selection from pre-computed dot products ───────────────
...
// ── L2 beam routing: one block per (query, c1) ───────────────────────────────
// Computes Pi_route2 @ (q - c1) in shared memory, then finds top-ck2.
// d_proj_route is the routing projection dimension (independent of leaf scan).

__global__ void route_l2_beam_kernel(
    const float* __restrict__ q,
    const float* __restrict__ C1_full,
    const float* __restrict__ Pi2,          // [d_proj_route, d]
    const float* __restrict__ C2_proj,      // [K1*K2, d_proj_route]
    const float* __restrict__ C2_norms,     // [K1*K2]
    const int*   __restrict__ top1,
    float*       r1_beam,
    int*         top2_beam,
    int B, int d, int d_proj_route, int K2, int ck1, int ck2)
{
    int beam = blockIdx.x;
    int qi   = beam / ck1;
    int c1_i = beam % ck1;
    ...
    float* s_r1   = shm_l2;
    float* s_proj = s_r1 + d;
    float* s_dist = s_proj + d_proj_route;
    ...
    for (int jp = threadIdx.x; jp < d_proj_route; jp += blockDim.x) {
        float v = 0.f;
    ...
    const float* c2_proj_base  = C2_proj  + (long long)c1 * K2 * d_proj_route;
    const float* c2_norms_base = C2_norms + (long long)c1 * K2;
    for (int c2 = threadIdx.x; c2 < K2; c2 += blockDim.x) {
        float dot = 0.f;
        const float* c2_row = c2_proj_base + (long long)c2 * d_proj_route;
        for (int jp = 0; jp < d_proj_route; jp++) dot += s_proj[jp] * c2_row[jp];
...
// ── L3 beam routing: one block per (query, c1, c2) ───────────────────────────

__global__ void route_l3_beam_kernel(
    const float* __restrict__ r1_beam,
    const float* __restrict__ C2_full,
    const float* __restrict__ Pi3,          // [d_proj_route, d]
    const float* __restrict__ C3_proj,      // [K1*K2*K3, d_proj_route]
    const float* __restrict__ C3_norms,     // [K1*K2*K3]
    const int*   __restrict__ top1,
    const int*   __restrict__ top2_beam,
    int*         top3_beam,
    int B, int d, int d_proj_route, int K2, int K3, int ck1, int ck2, int ck3)
{
    int blk  = blockIdx.x;
    int qi   = blk / (ck1 * ck2);
    int rem  = blk % (ck1 * ck2);
    ...
    const float* c3_proj_base  = C3_proj  + (long long)c12 * K3 * d_proj_route;
    const float* c3_norms_base = C3_norms + (long long)c12 * K3;
    for (int c3 = threadIdx.x; c3 < K3; c3 += blockDim.x) {
        float dot = 0.f;
        const float* c3_row = c3_proj_base + (long long)c3 * d_proj_route;
        for (int jp = 0; jp < d_proj_route; jp++) dot += s_proj[jp] * c3_row[jp];
...
void route_queries_v19(
    cublasHandle_t cublas,
    const float*   d_Pi_route1,
    const float*   d_Pi_route2,
    const float*   d_Pi_route3,
    const float*   d_Pi_scan,
    ...
    int nq, int d, int d_proj_route, int d_proj_scan,
...
{
    if (nq > ws.batch_cap)
        throw std::runtime_error("hblock_v19: nq > batch_cap");
    ...
    // L1 routing projection: d_q_route1 = Pi_route1 @ q  [B, d_proj_route]
    CUBLAS_CHECK(cublasSgemm(cublas, CUBLAS_OP_T, CUBLAS_OP_N,
                             d_proj_route, B, d, &one,
                             d_Pi_route1, d, ws.d_q_batch, d, &zero,
                             ws.d_q_route1, d_proj_route));

    // Leaf scan projection: d_q_scan = Pi_scan @ q  [B, d_proj_scan]
    CUBLAS_CHECK(cublasSgemm(cublas, CUBLAS_OP_T, CUBLAS_OP_N,
                             d_proj_scan, B, d, &one,
                             d_Pi_scan, d, ws.d_q_batch, d, &zero,
                             ws.d_q_scan, d_proj_scan));

    // L1 dot products and top-ck1 selection
    CUBLAS_CHECK(cublasSgemm(cublas, CUBLAS_OP_T, CUBLAS_OP_N,
                             K1, B, d_proj_route, &one,
                             d_route1_cents_proj, d_proj_route,
                             ws.d_q_route1, d_proj_route, &zero,
                             ws.d_dots1, K1));
    {
    ...
    // L2 beam routing
    {
        int shm_l2 = (d + d_proj_route + K2) * (int)sizeof(float);
        route_l2_beam_kernel<<<B * ck1, BLOCK, shm_l2, s>>>(
            ws.d_q_batch, d_route1_cents_full, d_Pi_route2,
            d_route2_cents_proj, d_route2_norms,
            ws.d_top1_ids, ws.d_r1_beam, ws.d_top2_beam,
            B, d, d_proj_route, K2, ck1, ck2);
        CUDA_CHECK(cudaGetLastError());
    }

    // L3 beam routing
    {
        int shm_l3 = (d + d_proj_route + K3) * (int)sizeof(float);
        route_l3_beam_kernel<<<B * ck1 * ck2, BLOCK, shm_l3, s>>>(
            ws.d_r1_beam, d_route2_cents_full, d_Pi_route3,
            d_route3_cents_proj, d_route3_norms,
            ws.d_top1_ids, ws.d_top2_beam, ws.d_top3_beam,
            B, d, d_proj_route, K2, K3, ck1, ck2, ck3);
        CUDA_CHECK(cudaGetLastError());
    }
...
void gpu_build_and_sort_pairs_v19(
...
// ── JL leaf scan ──────────────────────────────────────────────────────────────
// One CUDA block per (query, leaf_block) pair; blockDim = leaf_size.
// Uses d_proj_scan (separate from routing dimension) and d_q_scan.

__global__ void leaf_jl_scan_kernel(
    const int*   __restrict__ d_pair_leaf_ids,
    const int*   __restrict__ d_pair_qids,
    const float* __restrict__ d_leaf_proj_vecs,  // [n_lb, d_proj_scan, leaf_size]
    const float* __restrict__ d_q_scan_all,       // [B, d_proj_scan]
    ...
    int d_proj_scan, int leaf_size)
{
    ...
    extern __shared__ char shm[];
    float* s_q    = (float*)shm;
    float* s_dist = s_q + d_proj_scan;
    int*   s_id   = (int*)(s_dist + leaf_size);

    for (int j = vi; j < d_proj_scan; j += blockDim.x)
        s_q[j] = d_q_scan_all[(long long)qid * d_proj_scan + j];
    __syncthreads();

    float dist    = INF;
    int   orig_id = -1;
    if (vi < nvec) {
        const float* base = d_leaf_proj_vecs + (long long)lb * d_proj_scan * leaf_size;
        dist = 0.f;
...
void gpu_merge_jl_topk_v19(int nq, int n_pairs, int rerank_r, SearchWorkspace& ws)
{
    cudaStream_t s = ws.stream;
