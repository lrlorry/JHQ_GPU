#include "hblock_v19/jhq_gpu_index.cuh"
#include "hblock_v19/search.cuh"
#include "hblock_v17/encode.cuh"   // reuse assign/subtract/kmeans kernels
#include "common/cuda_utils.cuh"

using hblock_v17::launch_assign_from_dots;
using hblock_v17::launch_subtract_centroid;
using hblock_v17::launch_kmeans_assign;
using hblock_v17::launch_kmeans_update;
...
HBlockIndex::HBlockIndex(int d, Params p)
    : d_(d), d_proj_route_(p.d_proj_route), d_proj_scan_(p.d_proj_scan),
...
HBlockIndex::~HBlockIndex() {
    ...
    free_d(d_leaf_proj_vecs_);  free_d(d_leaf_ids_); free_d(d_leaf_sizes_);
    free_d(d_base_vecs_);
...
void HBlockIndex::init_jl_proj(int d, int proj_dim, int seed, std::vector<float>& Pi)
{
    Pi.resize((long long)proj_dim * d);
    std::mt19937 rng(seed);
    std::normal_distribution<float> g(0.f, 1.f / std::sqrt((float)proj_dim));
    for (float& v : Pi) v = g(rng);
}

// ── GPU k-means (in proj_dim-D projected space) ───────────────────────────────
...
{
    h_cents_proj.resize((long long)K * proj_dim);

    float *d_x_proj, *d_dots;
    int   *d_assigns, *d_counts;
    float *d_cents, *d_norms;
    CUDA_CHECK(cudaMalloc(&d_x_proj,  (long long)n * proj_dim * sizeof(float)));
    ...
    CUDA_CHECK(cudaMalloc(&d_cents,   (long long)K * proj_dim * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_norms,   (long long)K * sizeof(float)));
    CUDA_CHECK(cudaMemcpy(d_x_proj, h_x_proj, (long long)n * proj_dim * sizeof(float), cudaMemcpyHostToDevice));

    {
        std::mt19937 rng(42);
        std::uniform_int_distribution<int> uni(0, n - 1);
        std::vector<float> init_c((long long)K * proj_dim);
        for (int k = 0; k < K; k++) {
            int idx = uni(rng);
            std::memcpy(init_c.data() + (long long)k * proj_dim,
                        h_x_proj + (long long)idx * proj_dim,
                        proj_dim * sizeof(float));
        }
        CUDA_CHECK(cudaMemcpy(d_cents, init_c.data(), (long long)K * proj_dim * sizeof(float), cudaMemcpyHostToDevice));
    }

    const float one = 1.f, zero = 0.f;
    std::vector<float> h_norms_tmp(K);
    for (int iter = 0; iter < km_iters_; iter++) {
        CUDA_CHECK(cudaMemcpy(h_cents_proj.data(), d_cents,
                              (long long)K * proj_dim * sizeof(float), cudaMemcpyDeviceToHost));
        for (int k = 0; k < K; k++) {
            float s = 0.f;
            for (int j = 0; j < proj_dim; j++) {
                float v = h_cents_proj[(long long)k * proj_dim + j];
                s += v * v;
            }
            h_norms_tmp[k] = s;
        }
        CUDA_CHECK(cudaMemcpy(d_norms, h_norms_tmp.data(), K * sizeof(float), cudaMemcpyHostToDevice));

        CUBLAS_CHECK(cublasSgemm(cublas_, CUBLAS_OP_T, CUBLAS_OP_N,
                                 K, n, proj_dim, &one,
                                 d_cents, proj_dim,
                                 d_x_proj, proj_dim, &zero,
                                 d_dots, K));

        launch_kmeans_assign(d_dots, d_norms, d_assigns, K, n, nullptr);
        CUDA_CHECK(cudaDeviceSynchronize());

        CUDA_CHECK(cudaMemset(d_cents,  0, (long long)K * proj_dim * sizeof(float)));
        CUDA_CHECK(cudaMemset(d_counts, 0, K * sizeof(int)));
        launch_kmeans_update(d_x_proj, d_assigns, d_cents, d_counts, n, proj_dim, K, nullptr);
        CUDA_CHECK(cudaDeviceSynchronize());
    }
...
// ── Upload centroids with validity mask ───────────────────────────────────────
// Empty cells (h_valid[k] = false) get norm = 1e30f so routing never selects them.

void HBlockIndex::upload_cents(const std::vector<float>& h_proj,
                                ...
                                float*& d_proj_out, float*& d_full_out, float*& d_norms_out)
{
    ...
    for (int k = 0; k < K; k++) {
        ...
        float s = 0.f;
        for (int j = 0; j < proj_dim; j++) {
            float v = h_proj[(long long)k * proj_dim + j];
            s += v * v;
        }
        h_norms[k] = s;
    }
    ...
    CUDA_CHECK(cudaMalloc(&d_proj_out,  (long long)K * proj_dim * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_full_out,  (long long)K * d_        * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_norms_out, (long long)K * sizeof(float)));
    CUDA_CHECK(cudaMemcpy(d_proj_out,  h_proj.data(),  (long long)K * proj_dim * sizeof(float), cudaMemcpyHostToDevice));
...
static __global__ void local_assign_kernel(
    ...
    int*         assign,
    int n, int proj_dim, int K_inner)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    int og = outer_id[i];
    const float* r_i    = r_proj  + (long long)i  * proj_dim;
    const float* c_base = C_proj  + (long long)og * K_inner * proj_dim;
    const float* n_base = C_norms + (long long)og * K_inner;
    float bv = 1e30f; int bi = 0;
    for (int k = 0; k < K_inner; k++) {
        const float* c_k = c_base + (long long)k * proj_dim;
        float dot = 0.f;
        for (int j = 0; j < proj_dim; j++) dot += r_i[j] * c_k[j];
...
// ── Scatter JL projections into leaf-block layout ─────────────────────────────
// d_all_proj: [n, d_proj_scan]
// output:     [n_lb, d_proj_scan, leaf_size]

static __global__ void scatter_jl_to_leaf_kernel(
    const float* __restrict__ d_all_proj,
    const int*   __restrict__ d_leaf_ids,
    const int*   __restrict__ d_leaf_sizes,
    float*       d_leaf_proj,
    int n_lb, int d_proj_scan, int leaf_size)
{
    int lb = blockIdx.x;
    int vi = threadIdx.x;
    if (lb >= n_lb || vi >= d_leaf_sizes[lb]) return;

    int orig_id = d_leaf_ids[(long long)lb * leaf_size + vi];
    if (orig_id < 0) return;

    const float* src     = d_all_proj + (long long)orig_id * d_proj_scan;
    float*       dst_dim = d_leaf_proj + (long long)lb * d_proj_scan * leaf_size + vi;

    for (int dim = 0; dim < d_proj_scan; dim++)
        dst_dim[(long long)dim * leaf_size] = src[dim];
}
...
void HBlockIndex::train(const float* h_x, int n_train)
{
    ...
    printf("[v19 train] d=%d d_proj_route=%d d_proj_scan=%d K1=%d K2=%d K3=%d\n",
           d_, d_proj_route_, d_proj_scan_, K1_, K2_, K3_);
    printf("  n_train=%d n_km=%d km_iters=%d\n", n_train, n_km, km_iters_);

    const float one = 1.f, zero = 0.f;
    CUBLAS_CHECK(cublasSetStream(cublas_, nullptr));

    // ── Step 1: L1 routing Pi + k-means ──────────────────────────────────────
    printf("  [L1] init Pi_route1, k-means K1=%d ... ", K1_); fflush(stdout);
    auto t1 = Clock::now();
    {
        std::vector<float> Pi_r1;
        init_jl_proj(d_, d_proj_route_, 42, Pi_r1);
        CUDA_CHECK(cudaMalloc(&d_Pi_route1_, (long long)d_proj_route_ * d_ * sizeof(float)));
        CUDA_CHECK(cudaMemcpy(d_Pi_route1_, Pi_r1.data(),
                              (long long)d_proj_route_ * d_ * sizeof(float), cudaMemcpyHostToDevice));

        float *d_x_gpu, *d_y_proj;
        CUDA_CHECK(cudaMalloc(&d_x_gpu,  (long long)n_km * d_           * sizeof(float)));
        CUDA_CHECK(cudaMalloc(&d_y_proj, (long long)n_km * d_proj_route_ * sizeof(float)));
        CUDA_CHECK(cudaMemcpy(d_x_gpu, h_x, (long long)n_km * d_ * sizeof(float), cudaMemcpyHostToDevice));
        CUBLAS_CHECK(cublasSgemm(cublas_, CUBLAS_OP_T, CUBLAS_OP_N,
                                 d_proj_route_, n_km, d_, &one,
                                 d_Pi_route1_, d_, d_x_gpu, d_, &zero, d_y_proj, d_proj_route_));

        std::vector<float> h_y_proj((long long)n_km * d_proj_route_);
        CUDA_CHECK(cudaMemcpy(h_y_proj.data(), d_y_proj,
                              (long long)n_km * d_proj_route_ * sizeof(float), cudaMemcpyDeviceToHost));
        ...
        gpu_kmeans(h_y_proj.data(), h_x_km.data(), n_km, K1_, d_proj_route_,
                   h_c1_proj, h_c1_full, h_assign1);

        // L1 centroids: all valid (k-means always fills K1 entries)
        std::vector<bool> c1_valid(K1_, true);
        upload_cents(h_c1_proj, h_c1_full, c1_valid, K1_, d_proj_route_,
                     d_route1_cents_proj_, d_route1_cents_full_, d_route1_norms_);
        printf("%.1f ms\n", Ms(Clock::now() - t1).count());

        // ── Step 2: L2 routing Pi + per-c1 k-means ───────────────────────────
        printf("  [L2] init Pi_route2, k-means K2=%d per c1 ... ", K2_); fflush(stdout);
        auto t2 = Clock::now();
        ...
        std::vector<float> Pi_r2;
        init_jl_proj(d_, d_proj_route_, 43, Pi_r2);
        CUDA_CHECK(cudaMalloc(&d_Pi_route2_, (long long)d_proj_route_ * d_ * sizeof(float)));
        CUDA_CHECK(cudaMemcpy(d_Pi_route2_, Pi_r2.data(),
                              (long long)d_proj_route_ * d_ * sizeof(float), cudaMemcpyHostToDevice));

        float *d_r1_gpu, *d_r1_proj_gpu;
        CUDA_CHECK(cudaMalloc(&d_r1_gpu,      (long long)n_km * d_           * sizeof(float)));
        CUDA_CHECK(cudaMalloc(&d_r1_proj_gpu, (long long)n_km * d_proj_route_ * sizeof(float)));
        CUDA_CHECK(cudaMemcpy(d_r1_gpu, h_r1.data(), (long long)n_km * d_ * sizeof(float), cudaMemcpyHostToDevice));
        CUBLAS_CHECK(cublasSgemm(cublas_, CUBLAS_OP_T, CUBLAS_OP_N,
                                 d_proj_route_, n_km, d_, &one,
                                 d_Pi_route2_, d_, d_r1_gpu, d_, &zero, d_r1_proj_gpu, d_proj_route_));
        std::vector<float> h_r1_proj((long long)n_km * d_proj_route_);
        CUDA_CHECK(cudaMemcpy(h_r1_proj.data(), d_r1_proj_gpu,
                              (long long)n_km * d_proj_route_ * sizeof(float), cudaMemcpyDeviceToHost));
        cudaFree(d_r1_gpu); cudaFree(d_r1_proj_gpu);

        std::vector<float> h_all_c2_proj((long long)K1_ * K2_ * d_proj_route_, 0.f);
        std::vector<float> h_all_c2_full((long long)K1_ * K2_ * d_,            0.f);
        std::vector<bool>  h_c2_valid(K1_ * K2_, false);
        ...
        for (int c1 = 0; c1 < K1_; c1++) {
            ...
            if (nc1 == 0) continue;  // leave zeros + valid=false
            int K2e = std::min(K2_, nc1);

            std::vector<float> grp_proj((long long)nc1 * d_proj_route_);
            std::vector<float> grp_full((long long)nc1 * d_);
            for (int k = 0; k < nc1; k++) {
                std::memcpy(grp_proj.data() + (long long)k * d_proj_route_,
                            h_r1_proj.data() + (long long)idx[k] * d_proj_route_,
                            d_proj_route_ * sizeof(float));
                std::memcpy(grp_full.data() + (long long)k * d_,
                            h_r1.data()      + (long long)idx[k] * d_,
                            d_               * sizeof(float));
            }

            std::vector<float> c2p, c2f; std::vector<int> a2;
            gpu_kmeans(grp_proj.data(), grp_full.data(), nc1, K2e, d_proj_route_, c2p, c2f, a2);

            // copy K2e valid centroids; remaining stay zero (valid=false)
            std::memcpy(h_all_c2_proj.data() + (long long)c1 * K2_ * d_proj_route_,
                        c2p.data(), (long long)K2e * d_proj_route_ * sizeof(float));
            std::memcpy(h_all_c2_full.data() + (long long)c1 * K2_ * d_,
                        c2f.data(), (long long)K2e * d_             * sizeof(float));
            for (int k2 = 0; k2 < K2e; k2++)
                h_c2_valid[c1 * K2_ + k2] = true;
            for (int k = 0; k < nc1; k++) h_assign2[idx[k]] = a2[k];
        }
        upload_cents(h_all_c2_proj, h_all_c2_full, h_c2_valid, K1_ * K2_, d_proj_route_,
                     d_route2_cents_proj_, d_route2_cents_full_, d_route2_norms_);
        printf("%.1f ms\n", Ms(Clock::now() - t2).count());

        // ── Step 3: L3 routing Pi + per-(c1,c2) k-means ──────────────────────
        printf("  [L3] init Pi_route3, k-means K3=%d per (c1,c2) ... ", K3_); fflush(stdout);
        auto t3 = Clock::now();
        ...
        std::vector<float> Pi_r3;
        init_jl_proj(d_, d_proj_route_, 44, Pi_r3);
        CUDA_CHECK(cudaMalloc(&d_Pi_route3_, (long long)d_proj_route_ * d_ * sizeof(float)));
        CUDA_CHECK(cudaMemcpy(d_Pi_route3_, Pi_r3.data(),
                              (long long)d_proj_route_ * d_ * sizeof(float), cudaMemcpyHostToDevice));

        float *d_r2_gpu, *d_r2_proj_gpu;
        CUDA_CHECK(cudaMalloc(&d_r2_gpu,      (long long)n_km * d_           * sizeof(float)));
        CUDA_CHECK(cudaMalloc(&d_r2_proj_gpu, (long long)n_km * d_proj_route_ * sizeof(float)));
        CUDA_CHECK(cudaMemcpy(d_r2_gpu, h_r2.data(), (long long)n_km * d_ * sizeof(float), cudaMemcpyHostToDevice));
        CUBLAS_CHECK(cublasSgemm(cublas_, CUBLAS_OP_T, CUBLAS_OP_N,
                                 d_proj_route_, n_km, d_, &one,
                                 d_Pi_route3_, d_, d_r2_gpu, d_, &zero, d_r2_proj_gpu, d_proj_route_));
        std::vector<float> h_r2_proj((long long)n_km * d_proj_route_);
        ...
        std::vector<float> h_all_c3_proj(n_c12 * K3_ * d_proj_route_, 0.f);
        ...
        for (int c1 = 0; c1 < K1_; c1++) {
            for (int c2 = 0; c2 < K2_; c2++) {
                long long c12 = (long long)c1 * K2_ + c2;
                if (!h_c2_valid[c12]) continue;  // skip empty L2 cells
                std::vector<int> idx;
                for (int i = 0; i < n_km; i++)
                ...
                std::vector<float> grp_proj((long long)nc12 * d_proj_route_);
                std::vector<float> grp_full((long long)nc12 * d_);
                for (int k = 0; k < nc12; k++) {
                    std::memcpy(grp_proj.data() + (long long)k * d_proj_route_,
                                h_r2_proj.data() + (long long)idx[k] * d_proj_route_,
                                d_proj_route_ * sizeof(float));
                    std::memcpy(grp_full.data() + (long long)k * d_,
                                h_r2.data()      + (long long)idx[k] * d_,
                                d_               * sizeof(float));
                }

                std::vector<float> c3p, c3f; std::vector<int> a3;
                gpu_kmeans(grp_proj.data(), grp_full.data(), nc12, K3e, d_proj_route_, c3p, c3f, a3);

                std::memcpy(h_all_c3_proj.data() + (c12 * K3_) * d_proj_route_,
                            c3p.data(), (long long)K3e * d_proj_route_ * sizeof(float));
                std::memcpy(h_all_c3_full.data() + (c12 * K3_) * d_,
                            c3f.data(), (long long)K3e * d_             * sizeof(float));
                for (int k3 = 0; k3 < K3e; k3++)
                    h_c3_valid[c12 * K3_ + k3] = true;
            }
        }
        upload_cents(h_all_c3_proj, h_all_c3_full, h_c3_valid, n_c12 * K3_, d_proj_route_,
                     d_route3_cents_proj_, d_route3_cents_full_, d_route3_norms_);
        printf("%.1f ms\n", Ms(Clock::now() - t3).count());

        // ── Step 4: leaf scan Pi (separate, d_proj_scan dimensional) ─────────
        std::vector<float> Pi_scan;
        init_jl_proj(d_, d_proj_scan_, 99, Pi_scan);
        CUDA_CHECK(cudaMalloc(&d_Pi_scan_, (long long)d_proj_scan_ * d_ * sizeof(float)));
        CUDA_CHECK(cudaMemcpy(d_Pi_scan_, Pi_scan.data(),
                              (long long)d_proj_scan_ * d_ * sizeof(float), cudaMemcpyHostToDevice));
    }

    printf("[v19 train] total=%.1f ms\n", Ms(Clock::now() - T0).count());
}
...
{
    if (!d_Pi_route1_) throw std::runtime_error("call train() before add()");
    ...
    float *d_x, *d_r1, *d_r2, *d_proj1, *d_proj2, *d_proj3;
    int *d_c1, *d_c2, *d_c3;
    CUDA_CHECK(cudaMalloc(&d_x,     (long long)BATCH * d_            * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_proj1, (long long)BATCH * d_proj_route_  * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_proj2, (long long)BATCH * d_proj_route_  * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_proj3, (long long)BATCH * d_proj_route_  * sizeof(float)));
    ...
    for (int s = 0; s < n; s += BATCH) {
        ...
        // L1 assignment (routing projection)
        CUBLAS_CHECK(cublasSgemm(cublas_, CUBLAS_OP_T, CUBLAS_OP_N,
                                 d_proj_route_, nb, d_, &one,
                                 d_Pi_route1_, d_, d_x, d_, &zero, d_proj1, d_proj_route_));
        CUBLAS_CHECK(cublasSgemm(cublas_, CUBLAS_OP_T, CUBLAS_OP_N,
                                 K1_, nb, d_proj_route_, &one,
                                 d_route1_cents_proj_, d_proj_route_,
                                 d_proj1, d_proj_route_, &zero, d_dots1, K1_));
        ...
        CUBLAS_CHECK(cublasSgemm(cublas_, CUBLAS_OP_T, CUBLAS_OP_N,
                                 d_proj_route_, nb, d_, &one,
                                 d_Pi_route2_, d_, d_r1, d_, &zero, d_proj2, d_proj_route_));
        local_assign_kernel<<<(nb + 255) / 256, 256>>>(
            d_proj2, d_route2_cents_proj_, d_route2_norms_, d_c1, d_c2, nb, d_proj_route_, K2_);
        {
        ...
        CUBLAS_CHECK(cublasSgemm(cublas_, CUBLAS_OP_T, CUBLAS_OP_N,
                                 d_proj_route_, nb, d_, &one,
                                 d_Pi_route3_, d_, d_r2, d_, &zero, d_proj3, d_proj_route_));
        {
            std::vector<int> h_c1b(nb), h_c2b(nb), h_c3b(nb);
            CUDA_CHECK(cudaMemcpy(h_c1b.data(), d_c1, nb * sizeof(int), cudaMemcpyDeviceToHost));
            CUDA_CHECK(cudaMemcpy(h_c2b.data(), d_c2, nb * sizeof(int), cudaMemcpyDeviceToHost));
            std::vector<int> h_c12(nb);
            for (int i = 0; i < nb; i++) h_c12[i] = h_c1b[i] * K2_ + h_c2b[i];
            ...
            local_assign_kernel<<<(nb + 255) / 256, 256>>>(
                d_proj3, d_route3_cents_proj_, d_route3_norms_, d_c12, d_c3, nb, d_proj_route_, K3_);
            CUDA_CHECK(cudaDeviceSynchronize());
    ...
    cudaFree(d_c1); cudaFree(d_c2); cudaFree(d_c3); cudaFree(d_dots1);
    ...
    // JL leaf projections using Pi_scan (d_proj_scan dimensional)
    printf("  Building JL leaf projections (d_proj_scan=%d) %.2f GB ... ",
           d_proj_scan_,
           (double)total_blocks * d_proj_scan_ * leaf_size_ * 4 / 1e9); fflush(stdout);
    {
        const float one2 = 1.f, zero2 = 0.f;
        float* d_all_proj;
        CUDA_CHECK(cudaMalloc(&d_all_proj, (long long)n * d_proj_scan_ * sizeof(float)));
        CUBLAS_CHECK(cublasSgemm(cublas_, CUBLAS_OP_T, CUBLAS_OP_N,
                                 d_proj_scan_, n, d_,
                                 &one2, d_Pi_scan_, d_,
                                 d_base_vecs_, d_,
                                 &zero2, d_all_proj, d_proj_scan_));

        CUDA_CHECK(cudaMalloc(&d_leaf_proj_vecs_,
                              (long long)total_blocks * d_proj_scan_ * leaf_size_ * sizeof(float)));
        CUDA_CHECK(cudaMemset(d_leaf_proj_vecs_, 0,
                              (long long)total_blocks * d_proj_scan_ * leaf_size_ * sizeof(float)));

        scatter_jl_to_leaf_kernel<<<total_blocks, leaf_size_>>>(
            d_all_proj, d_leaf_ids_, d_leaf_sizes_,
            d_leaf_proj_vecs_, total_blocks, d_proj_scan_, leaf_size_);
        CUDA_CHECK(cudaDeviceSynchronize());
        cudaFree(d_all_proj);
    }
    printf("done\n");
    ...
    ntotal_ = n;
    printf("  Built %d leaf blocks (K1=%d K2=%d K3=%d leaf_size=%d)\n",
           total_blocks, K1_, K2_, K3_, leaf_size_);
...
#define FD(p) do { if (ws_.p) { cudaFree(ws_.p);     ws_.p = nullptr; } } while(0)
    ...
    FD(d_q_batch); FD(d_q_route1); FD(d_q_scan); FD(d_dots1); FD(d_top1_ids);
    FD(d_r1_beam); FD(d_top2_beam); FD(d_top3_beam);
    FD(d_leaf_sel); FD(d_leaf_cnt);

    CUDA_CHECK(cudaMalloc(&ws_.d_q_batch,    (long long)B * d_             * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ws_.d_q_route1,   (long long)B * d_proj_route_  * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ws_.d_q_scan,     (long long)B * d_proj_scan_   * sizeof(float)));
    ...
    CUDA_CHECK(cudaMalloc(&ws_.d_out_dists, (long long)max_pairs * JL_TOP_P * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ws_.d_out_ids,   (long long)max_pairs * JL_TOP_P * sizeof(int)));

    FD(d_jl_dists); FD(d_jl_ids);
    CUDA_CHECK(cudaMalloc(&ws_.d_jl_dists, (long long)B * K_MAX * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ws_.d_jl_ids,   (long long)B * K_MAX * sizeof(int)));
    ...
    ws_.rerank_r     = R;
    ws_.d_proj_route = d_proj_route_;
    ws_.d_proj_scan  = d_proj_scan_;
                          ...
                          float* h_dists, int* h_ids) const
{
    ...
    route_queries_v19(cublas_,
                      d_Pi_route1_, d_Pi_route2_, d_Pi_route3_, d_Pi_scan_,
                      d_route1_cents_proj_, d_route1_cents_full_, d_route1_norms_,
                      d_route2_cents_proj_, d_route2_cents_full_, d_route2_norms_,
                      d_route3_cents_proj_, d_route3_cents_full_, d_route3_norms_,
                      d_pair_blk_start_, d_pair_blk_count_,
                      h_q, nq, d_, d_proj_route_, d_proj_scan_,
                      K1_, K2_, K3_, ck1_, ck2_, ck3_, batch_size_, ws_);
    auto t1 = Clock::now();
    ...
    gpu_build_and_sort_pairs_v19(nq, n_pairs, n_leaf_blocks_, ws_.max_leaf_sel, ws_);
    auto t3 = Clock::now();

    cudaStream_t s = ws_.stream;
    launch_leaf_jl_v19(
        ws_.d_pair_leaf_b, ws_.d_pair_qid_b,
        d_leaf_proj_vecs_, ws_.d_q_scan,
        d_leaf_ids_, d_leaf_sizes_,
        ws_.d_out_dists, ws_.d_out_ids,
        n_pairs, d_proj_scan_, leaf_size_, s);
    auto t4 = Clock::now();

    gpu_merge_jl_topk_v19(nq, n_pairs, rerank_r_, ws_);
    auto t5 = Clock::now();

    launch_gather_vecs_v19(d_base_vecs_, ws_.d_jl_ids, ws_.d_cand_vecs,
                            nq, rerank_r_, d_, s);
    auto t6 = Clock::now();

    launch_exact_rerank_v19(ws_.d_q_batch, ws_.d_cand_vecs, ws_.d_jl_ids,
    ...
    printf("  [v19] Route=%.2fms  PairSort=%.2fms (%d pairs)"
           "  JLScan=%.2fms  Merge=%.2fms  Gather=%.2fms"
           "  Rerank=%.2fms  D2H=%.2fms  Extract=%.2fms  Total=%.2fms\n",
           Ms(t1-t0).count(), Ms(t3-t2).count(), n_pairs,
