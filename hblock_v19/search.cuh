...
static constexpr int JL_TOP_P = 16;   // candidates kept per leaf block (doubled from v18)
static constexpr int K_MAX    = 192;  // max rerank_r; smem: 32*192*8 = 48KB < 99KB limit
...
struct SearchWorkspace {
    ...
    float* d_q_route1  = nullptr;   // [B, d_proj_route]  L1 routing projection
    float* d_q_scan    = nullptr;   // [B, d_proj_scan]   leaf JL query projection
...
void route_queries_v19(
    ...
    const float*   d_Pi_route1,   // [d_proj_route, d]
    const float*   d_Pi_route2,   // [d_proj_route, d]
    const float*   d_Pi_route3,   // [d_proj_route, d]
    const float*   d_Pi_scan,     // [d_proj_scan, d]
...
void launch_leaf_jl_v19(
    ...
    const float* d_leaf_proj_vecs,  // [n_lb, d_proj_scan, leaf_size]
    const float* d_q_scan,          // [B, d_proj_scan]
