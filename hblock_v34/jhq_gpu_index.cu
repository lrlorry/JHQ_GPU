...
void HBlockIndex::train(const float* h_x, int n_train)
{
    ...
    printf("[v34 train] d=%d d_proj=%d K1=%d K2=%d K3=%d ck1=%d ck2=%d ck3=%d"
...
void HBlockIndex::add(const float* h_x, int n)
{
    ...
    printf("[v34 add] n=%d  d=%d  batch=%d  mini_km_iters=%d\n", n, d_, 8192, mini_km_iters_);
    ...
    printf("[v34 add total] %.1f ms  blocks=%d\n",
                          ...
                          float* h_dists, int* h_ids, int ef) const
{
    ...
    ws_.beam_size    = std::min(ef, 128);  // v34: cap beam at 128 (SPT ≤ 4)
    ...
    for(int qstart=0;qstart<nq;qstart+=batch_size_){
        ...
        auto t1=Clock::now();
        gpu_block_search_v34(nb, n_leaf_blocks_, d_proj_,
                                   ...
                                   const float* h_base, const int* h_gt, int gt_k) const
{
    ...
    for (int qstart = 0; qstart < nq; qstart += batch_size_) {
        ...
        ws_.beam_size    = std::min(max_ef_, 128);
    ...
    const int* h_gt, int gt_k) const
{
    ...
    for (int qstart = 0; qstart < nq; qstart += batch_size_) {
        ...
        gpu_block_search_v34(nb, n_blks, d_proj_,
                             K2_, K3_, ck1_, ck2_, ck3_,
