...
// v34: W=4 batched expansion + HNSW early termination.
// beam = min(ef, 128) [cap at 128, SPT ≤ 4].
// Output: d_leaf_sel records EXPANDED blocks in expansion order (v30 semantics).
#include "hblock_v34/search.cuh"
...
template <int SPT>
__global__ void block_search_v34_kernel(
    ...
    int entry_per_cell, int bitmap_words)
{
    ...
    // ── Phase 2: Batched beam search with HNSW early termination ─────────────
...
#define LAUNCH_V34(SPT_VAL) \
    block_search_v34_kernel<SPT_VAL><<<B, 32, smem, ws.stream>>>( \
...
void gpu_block_search_v34(
    ...
    SearchWorkspace& ws)
{
    if (degree > 32) throw std::runtime_error("gpu_block_search_v34: degree must be <= 32");
    const int smem = d_proj * (int)sizeof(float);
    const int beam = std::min(ef, 128);  // cap beam at 128 (SPT ≤ 4)
    const int max_rounds = ef / 4 + 1;   // expansion budget (early termination fires sooner)
    if      (beam <= 32)  { LAUNCH_V34(1); }
    else if (beam <= 64)  { LAUNCH_V34(2); }
    else                  { LAUNCH_V34(4); }
...
void route_gpu_v29(
    ...
    SearchWorkspace& ws)
{
    if (nq > ws.batch_cap) throw std::runtime_error("hblock_v34: nq > batch_cap");
