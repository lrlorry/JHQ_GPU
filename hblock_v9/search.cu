...
{
    if (nq > ws.batch_cap)
        throw std::runtime_error("hblock_v9: nq > batch_cap");
    ...
    cudaStream_t s = ws.streams[0];  // routing uses stream 0
    ...
    // D2H leaf routing results (CPU needs for fan-out).
    // d_lut_fine stays on GPU: 48 MB, never transferred — dispatch streams read it directly.
...
// ── leaf_flink_kernel ─────────────────────────────────────────────────────────
// Grid = n_leaves, Block = leaf_size (128), smem ≈ 50 KB
//
// Flink analogy:
//   - Each block is a leaf operator with local state (smem = leaf codes)
//   - Processes its queue of (leaf, query) pairs sequentially
//   - In-kernel top-p reduction → only TOP_P results written to HBM per pair
//   - No 128 MB fine_dists buffer: distances live and die in smem/registers
...
// ── launch_leaf_flink: set smem size and launch kernel ────────────────────────
// Caller is responsible for all memcpy (H2D before, D2H after).
// That separation lets the search() pipeline interleave H2D/D2H across streams.
