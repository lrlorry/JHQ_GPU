#include "jhq_v8_timing/jhq_gpu_index.cuh"
#include "jhq_v8_timing/encode.cuh"
#include "jhq_v8_timing/search.cuh"
...
JHQGpuIndex::~JHQGpuIndex() {
    ...
    if (ws_.ev_h2d_start)  cudaEventDestroy(ws_.ev_h2d_start);
    if (ws_.ev_h2d_done)   cudaEventDestroy(ws_.ev_h2d_done);
    if (ws_.ev_graph_done) cudaEventDestroy(ws_.ev_graph_done);
    if (ws_.ev_d2h_done)   cudaEventDestroy(ws_.ev_d2h_done);
...
void JHQGpuIndex::alloc_workspace(int batch_size) {
    ...
    if (!ws_.ev_h2d_start)  CUDA_CHECK(cudaEventCreate(&ws_.ev_h2d_start));
    if (!ws_.ev_h2d_done)   CUDA_CHECK(cudaEventCreate(&ws_.ev_h2d_done));
    if (!ws_.ev_graph_done) CUDA_CHECK(cudaEventCreate(&ws_.ev_graph_done));
    if (!ws_.ev_d2h_done)   CUDA_CHECK(cudaEventCreate(&ws_.ev_d2h_done));
}
...
{
    if (n_train < nlist_)
        throw std::invalid_argument("n_train must be >= nlist for v8_timing");
...
void JHQGpuIndex::add(const float* h_x, int n) {
    ...
    if (ntotal_ != 0)
        throw std::runtime_error("v8_timing currently supports one add() call");
