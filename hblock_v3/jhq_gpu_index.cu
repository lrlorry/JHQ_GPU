#include "hblock_v3/jhq_gpu_index.cuh"
#include "hblock_v3/encode.cuh"
...
// ── PCA: top-k eigenvectors of (1/n) X^T X ───────────────────────────────────
// Returns P (k×d, row-major): rows are eigenvectors (descending eigenvalue order).
// Memory layout trick: k×d row-major = d×k column-major.
// For CUBLAS GEMM with OP_T (transposing d×k col-major → k×d), this gives P @ q.
std::vector<float>
pca_top_k(const float* X, int n, int d, int k)
{
    // X^T X via ssyrk, upper triangle only, scaled by 1/n
    std::vector<float> XtX((long long)d * d, 0.f);
    cblas_ssyrk(CblasRowMajor, CblasUpper, CblasTrans,
                d, n, 1.0f / n, X, d, 0.0f, XtX.data(), d);
    // Mirror upper → lower (ssyev needs full matrix)
    for (int i = 0; i < d; ++i)
        for (int j = 0; j < i; ++j)
            XtX[(long long)i * d + j] = XtX[(long long)j * d + i];

    // After ssyev_: eigenvectors stored as columns (col-major), ascending eigenvalues.
    // In row-major view: row j = j-th eigenvector.
    // Top-k eigenvectors = last k rows (rows d-k .. d-1).
    ...
    float total_var = 0.f, top_var = 0.f;
    for (int i = 0; i < d; ++i) if (W[i] > 0) total_var += W[i];
    for (int i = d - k; i < d; ++i) if (W[i] > 0) top_var += W[i];
    printf("  PCA: top %d dims explain %.2f%% variance\n",
           k, total_var > 0 ? 100.f * top_var / total_var : 0.f);
...
// ── Project X (n×d) through P (k×d, row-major) → Z (n×k) ────────────────────
std::vector<float>
...
// ── K-means (Lloyd's) on Z (n×k_dim) → K centroids ───────────────────────────
// Returns: mu (K×k_dim, row-major), assigns (n).
void kmeans_cpu(const float* Z, int n, int k_dim, int K, int n_iters, int seed,
                std::vector<float>& mu, std::vector<int>& assigns)
{
    ...
    for (int c = 0; c < K; ++c)
        std::memcpy(mu.data() + (long long)c * k_dim,
                    Z + (long long)perm[c % n] * k_dim,
                    k_dim * sizeof(float));

    std::vector<float> dots((long long)n * K);
    ...
    for (int iter = 0; iter < n_iters; ++iter) {
        ...
        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
                    n, K, k_dim, 1.f,
                    Z, k_dim, mu.data(), k_dim,
                    0.f, dots.data(), K);
        bool changed = false;
        for (int i = 0; i < n; ++i) {
            const float* row = dots.data() + (long long)i * K;
            float best = norms[0] - 2.f * row[0];
            int   bc   = 0;
            for (int c = 1; c < K; ++c) {
...
// ── Full-dim centroids from assignments ───────────────────────────────────────
std::vector<float>
...
// ── Project K centroids (K×d) through P (k×d, row-major) → (K×k) ─────────────
...
// ── Analytical Lloyd-Max fine centroids ───────────────────────────────────────
static std::vector<float>
...
// ── upload_proj_centroids ─────────────────────────────────────────────────────
void HBlockIndex::upload_proj_centroids(
    const std::vector<float>& C_proj,
...
// ── compute_projections (PCA: eigenvectors of X^T X) ─────────────────────────
static void compute_projections(
...
{
    printf("  [L1] PCA top-%d of %d dims...\n", k1, d);
    P1_out = pca_top_k(h_x, n_train, d, k1);

    printf("  [L1] Projecting %d vectors to k1=%d...\n", n_train, k1);
    std::vector<float> Z1 = project(h_x, n_train, d, P1_out.data(), k1);

    printf("  [L1] K-means K=%d, iters=%d...\n", K1, kmeans_iters);
    std::vector<float> mu1;
    kmeans_cpu(Z1.data(), n_train, k1, K1, kmeans_iters, seed, mu1, assigns1_out);
    (void)mu1;

    printf("  [L1] Computing full-dim centroids...\n");
    ...
    printf("  [L2] PCA top-%d of %d dims (residual space)...\n", k2, d);
    P2_out = pca_top_k(R1.data(), n_train, d, k2);

    printf("  [L2] Projecting residuals to k2=%d...\n", k2);
    std::vector<float> Z2 = project(R1.data(), n_train, d, P2_out.data(), k2);

    printf("  [L2] K-means K=%d, iters=%d...\n", K2, kmeans_iters);
    std::vector<float> mu2;
    kmeans_cpu(Z2.data(), n_train, k2, K2, kmeans_iters, seed + 1, mu2, assigns2_out);
    (void)mu2;

    printf("  [L2] Computing full-dim residual centroids...\n");
...
void HBlockIndex::train(const float* h_x, int n_train) {
    ...
    compute_projections(h_x, n_train, d_, K1_, k1_, K2_, k2_,
                        kmeans_iters_, seed_,
    ...
    // Estimate sigma from actual L2 residuals on training data
    ...
    printf("  Fine codebook: sigma=%.4f (from L2 residuals)\n", sigma);
    ...
    CUDA_CHECK(cudaMemcpy(d_P1_, P1.data(),
                          (long long)k1_ * d_ * sizeof(float), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_P2_, P2.data(),
                          (long long)k2_ * d_ * sizeof(float), cudaMemcpyHostToDevice));
...
void HBlockIndex::add(const float* h_x, int n) {
    ...
    if (ntotal_ != 0) throw std::runtime_error("HBlock v3 currently supports one add() call");
    ...
    std::vector<int>     h_leaf_sizes(total_blocks, 0);
    {
        ...
        while (i < n) {
            ...
            for (int vi = i; vi < j; ++vi) {
                ...
                std::memcpy(h_leaf_codes.data() +
                            ((long long)blk_idx * leaf_size_ + pos_in_blk) * bpv_,
...
{
    if (ntotal_ == 0) throw std::runtime_error("HBlock v3 index is empty");
