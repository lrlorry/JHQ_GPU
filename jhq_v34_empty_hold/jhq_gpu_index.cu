#include "jhq_v34_empty_hold/jhq_gpu_index.cuh"
...
#endif
#include "jhq_v34_empty_hold/encode.cuh"
#include "jhq_v34_empty_hold/train_pq_gpu.cuh"
#include "jhq_v34_empty_hold/train_res_gpu.cuh"
#include "jhq_v34_empty_hold/search.cuh"
...
void JHQGpuIndex::train_residual_codebook_streamed(const float* h_x, int n)
{
    ...
    for (int m0 = 0; m0 < M_; m0 += C) {
        ...
        CUDA_CHECK(cudaMemcpy(d_chunk, h_chunk.data(),
                              ...
                              cudaMemcpyHostToDevice));
        launch_residual_codebook_from_values(d_chunk, (long long)n * Ds_, cm, Kr_, 1000,
    ...
    if (gpu_codebook) {
        ...
        if (streamed) {
            ...
            launch_residual_codebook(d_y_res, d_codes_res, d_cent_,
                                     n_res, d_, M_, Ds_, K_, Kr_, 1000, d_res_c1d_);
            CUDA_CHECK(cudaMemcpy(res_c1d_.data(), d_res_c1d_,
                                  (size_t)M_ * Kr_ * sizeof(float), cudaMemcpyDeviceToHost));
...
// jhq_v34_empty_hold/jhq_gpu_index.cuh's Params::add_batch comment for
