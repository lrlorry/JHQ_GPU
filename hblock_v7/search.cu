    ...
    int B, int ck1, int ck2, int ck3, int K2, int leaf_cap_per_pair)
{
    ...
    for (int i1 = 0; i1 < ck1 && cnt < ck3; ++i1) {
        ...
        for (int i2 = 0; i2 < ck2 && cnt < ck3; ++i2) {
            ...
            int pidx = c1 * K2 + c2;
            int ps = pair_start[pidx], pc = pair_cnt[pidx];
            int take = min(min(pc, leaf_cap_per_pair), ck3 - cnt);
            for (int b = 0; b < take; ++b) my_sel[cnt++] = ps + b;
        }
    }
    leaf_cnt[bqi] = cnt;
}
...
__global__ void build_pairs_kernel(
    const int* __restrict__ leaf_sel, const int* __restrict__ leaf_cnt,
    int* pair_keys, int* pair_vals, int B, int ck3)
{
    int flat = blockIdx.x * blockDim.x + threadIdx.x;
    if (flat >= B * ck3) return;
    int q = flat / ck3, slot = flat % ck3;
    pair_keys[flat] = (slot < leaf_cnt[q]) ? leaf_sel[q * ck3 + slot] : INT_MAX;
    pair_vals[flat] = flat;
}

// ── Fill fine_dists/fine_ids with sentinels ───────────────────────────────────
// Must run before leaf_transposed_kernel so unvisited slots have valid values.
__global__ void fill_inf_kernel(float* dists, int* ids, long long n)
{
    long long i = (long long)blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
        dists[i] = __int_as_float(0x7F800000);
        ids[i]   = -1;
    }
}

// ── Transposed LeafFine kernel ─────────────────────────────────────────────────
// Grid: n_unique_leaves (one block per unique leaf after RLE).
// Each block loads its 49 KB leaf codes into smem ONCE, then processes ALL
// (query, slot) pairs that selected this leaf sequentially.
//
// HBM reads per batch:
//   leaf codes : n_unique × 49 KB  (guaranteed once; v5 relied on L2 luck)
//   LUT        : nq × 49 KB        (one query LUT at a time, L2-resident)
//
// Note: AoS smem layout causes 32-way bank conflicts on the nibble decode loop;
// this can be eliminated by transposing to SoB layout in smem (future work).
__global__ void leaf_transposed_kernel(
    const int*     __restrict__ d_sorted_vals,
    const int*     __restrict__ d_run_starts,
    const int*     __restrict__ d_run_counts,
    const int*     __restrict__ d_unique_leaf_ids,
    ...
    float* fine_dists, int* fine_ids,
    int ck3, int leaf_size, int bpv, int d, int Kr, int Br)
{
    int uid      = blockIdx.x;
    int v        = threadIdx.x;
    int leaf_blk = d_unique_leaf_ids[uid];

    // Skip INT_MAX sentinel (padding entries from build_pairs_kernel)
    if ((unsigned)leaf_blk == (unsigned)INT_MAX) return;

    int start  = d_run_starts[uid];
    int count  = d_run_counts[uid];
    int n_vecs = leaf_sizes[leaf_blk];

    // Load leaf codes into smem (coalesced stride-1 bytes; AoS layout)
    // smem size = leaf_size * bpv bytes, allocated at launch
    extern __shared__ uint8_t s_codes[];
    ...
    // Process every (qid, slot) pair that selected this leaf block
    for (int i = 0; i < count; ++i) {
        int flat = d_sorted_vals[start + i];
        int qid  = flat / ck3;
        int slot = flat % ck3;
        long long base = ((long long)qid * ck3 + slot) * leaf_size;

        // fine_dists/fine_ids already filled with INF/-1 for v >= n_vecs
        if (v >= n_vecs) continue;

        const float*   my_lut = lut_fine + (long long)qid * d * Kr;
        const uint8_t* rc     = s_codes  + (long long)v   * bpv;
        ...
        fine_ids  [base + v] = leaf_ids_data[(long long)leaf_blk * leaf_size + v];
    }
}

// ── Final top-k ───────────────────────────────────────────────────────────────
__global__ void final_topk_kernel(
    float* fine_dists, const int* __restrict__ fine_ids,
    float* out_dists, int* out_ids, int n_cands, int k, int B)
{
    const int   BLOCK = blockDim.x;
    const float INF   = __int_as_float(0x7F800000);
    int bqi = blockIdx.x, tid = threadIdx.x;
...
{
    if (ws.batch_cap <= 0)
        throw std::runtime_error("hblock_v7: workspace not initialised");
    ...
    const int leaf_smem     = leaf_size * bpv;        // smem for leaf codes in transposed kernel
    ...
    for (int qoff = 0; qoff < nq; qoff += batch_size) {
        ...
        // RLE: find unique leaf_ids and their run lengths
        cub::DeviceRunLengthEncode::Encode(
            ws.d_cub_tmp2, ws.cub_bytes2,
            ws.d_pair_keys_s,
            ws.d_unique_leaf_ids,
            ws.d_run_counts,
            ws.d_num_unique,
            n_pairs, ws.stream);
        ...
        // Sync to get h_num_unique for kernel grid size (~50 µs overhead)
        CUDA_CHECK(cudaStreamSynchronize(ws.stream));
        int h_num_unique = 0;
        CUDA_CHECK(cudaMemcpy(&h_num_unique, ws.d_num_unique,
                              sizeof(int), cudaMemcpyDeviceToHost));

        // Prefix sum: run_starts[i] = sum of run_counts[0..i-1]
        if (h_num_unique > 0) {
            cub::DeviceScan::ExclusiveSum(
                ws.d_cub_tmp2, ws.cub_bytes2,
                ws.d_run_counts, ws.d_run_starts,
                h_num_unique, ws.stream);
        }

        cudaEventRecord(e[9], ws.stream);

        // Fill fine_dists/fine_ids with INF/-1 before transposed kernel
        {
            long long n_fine = (long long)B * n_cands;
            int grid = (int)std::min((n_fine + BLOCK - 1) / BLOCK, (long long)65535);
            fill_inf_kernel<<<grid, BLOCK, 0, ws.stream>>>(
                ws.d_fine_dists, ws.d_fine_ids, n_fine);
            CUDA_CHECK(cudaGetLastError());
        }
        ...
        // Transposed LeafFine: one block per unique leaf, processes all hitting queries
        if (h_num_unique > 0) {
            leaf_transposed_kernel<<<h_num_unique, leaf_size, leaf_smem, ws.stream>>>(
                ws.d_pair_vals_s,
                ws.d_run_starts,
                ws.d_run_counts,
                ws.d_unique_leaf_ids,
        ...
        final_topk_kernel<<<B, BLOCK, topk_smem, ws.stream>>>(
            ...
            n_cands, k, B);
        CUDA_CHECK(cudaGetLastError());
        cudaEventRecord(e[13], ws.stream);

        // D2H (not timed separately; included in total)
    ...
    static const char* snames[] = {
        ...
        "Sort+RLE", "Scan(+sync)", "FillInf", "FineLUT",
        "LeafTransposed", "FinalTopk"
    };
    float total = 0.f;
