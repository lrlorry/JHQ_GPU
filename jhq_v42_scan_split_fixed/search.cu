...
// Splitting the scan without a profiler.
//
// ncu cannot run here: the container inherits RmProfilingAdminOnly=1 from a
// driver it cannot reload, so ERR_NVGPUCTRPERM on every counter. The
// compile-out method can, and it already agreed with nsys to a point and a half
// on the refine stage.
//
// v41 tried a 2x2 and one of the cells was invalid. JHQ_SCAN_NO_LUT replaces
// the lookup with the raw byte, which changes every distance, which changes how
// often the threshold compaction fires -- so with the compaction still in, the
// variant came out 16-41% *slower* than the full kernel and measured nothing.
// The removal has to be done where there is no compaction left to perturb:
//
//     lookup     = nocompact - neither      (both have no compaction)
//     compaction = full - nocompact         (both have the real distances)
//
// Those are the only two cells that mean anything, and the (nolut, compact)
// cell is not built.
//
// The other v41 fault: with the compaction gone the kernel still has to emit
// candidate positions, and summing them overflowed into whatever the refine
// stage then read, which faulted. It emits a scattered but in-range position
// instead, so the stages after the scan do the same amount of work in every
// variant and the difference stays inside the scan.
//
// Recall in any variant but the full one is meaningless and is not reported.
    ...
    int nprobe, int M, int N, int ck, int cap, bool lut_in_smem, int tile_m)
{
        ...
        if (lt < total) {
            ...
            for (int m0 = 0; m0 < M; m0 += TILE_M) {
                ...
                for (int m = m0; m < mhi; ++m) {
                    const uint8_t cm = __ldg(&list_primary_t[(long long)m * N + pos]);
#if JHQ_SCAN_NO_LUT
                    a += (float)cm;          // the gather without the lookup
#else
...
#if JHQ_SCAN_NO_COMPACT
        // dist has to survive to the store or the whole gather folds away.
        sink += (dist < INF) ? dist : 0.0f;
        __syncthreads();
    }
    ...
    for (int c = tid; c < ck; c += BLOCK) {
        out_pri[c] = sink;
        // In range and scattered, so the refine stage that follows reads the
        // same spread of list_res it would have read, and the difference
        // between variants stays inside this kernel.
        out_pos[c] = (int)((((long long)bqi * ck + c) * 2654435761LL) % N);
    }
#else
