#include "jhq_v30_disk_stage/jhq_gpu_index.cuh"
...
#endif
#include "jhq_v30_disk_stage/encode.cuh"
#include "jhq_v30_disk_stage/train_pq_gpu.cuh"
#include "jhq_v30_disk_stage/train_res_gpu.cuh"
#include "jhq_v30_disk_stage/search.cuh"
    ...
    if (gpu_codebook) {
        ...
        if (streamed) {
            ...
            if (rh && rh[0] == '1') {
                ...
                // 2048 was chosen to fit 48 KB, the shared memory a kernel gets
                // without asking. Opting in raises the ceiling to ~227 KB, and
                // the bucket count is what bounds the error: a bucket
                // straddling a cell boundary goes wholly to one side, so with
                // Kr cells at most Kr-1 of the buckets are wrong and each
                // carries 1/nbuckets of the mass. 2048 buckets against 256
                // cells leaves 12% of them straddling; 8192 leaves 3%.
                const int nbuckets = nbe ? std::atoi(nbe) : 8192;
...
// jhq_v30_disk_stage/jhq_gpu_index.cuh's Params::add_batch comment for
