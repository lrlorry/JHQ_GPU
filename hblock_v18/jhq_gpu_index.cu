...
// Bring v17 helpers into scope for v18
...
// d_all_proj: [n, d_proj] all base vectors projected with Pi1
// d_leaf_ids: [n_lb, leaf_size] original vector IDs per slot
// d_leaf_proj: [n_lb, d_proj, leaf_size] output (coalesced layout for scan kernel)
...
static __global__ void scatter_jl_to_leaf_kernel(
    ...
    float*       d_leaf_proj,
    int n_lb, int d_proj, int leaf_size)
{
    int lb = blockIdx.x;
    int vi = threadIdx.x;
    ...
    const float* src     = d_all_proj + (long long)orig_id * d_proj;
    float*       dst_dim = d_leaf_proj + (long long)lb * d_proj * leaf_size + vi;
...
void HBlockIndex::train(const float* h_x, int n_train)
{
    ...
    printf("[v18 train] d=%d d_proj=%d K1=%d K2=%d K3=%d ck1=%d ck2=%d ck3=%d\n",
           d_, d_proj_, K1_, K2_, K3_, ck1_, ck2_, ck3_);
    printf("  n_train=%d n_km=%d km_iters=%d  (JL coarse scan, no PQ)\n",
           n_train, n_km, km_iters_);

    // Step 1: L1 JL + k-means
    ...
    auto t1 = Clock::now();
    {
        ...
        CUBLAS_CHECK(cublasSgemm(cublas_, CUBLAS_OP_T, CUBLAS_OP_N,
                                 d_proj_, n_km, d_, &one,
                                 d_Pi1_, d_, d_x_gpu, d_, &zero, d_y_proj, d_proj_));
        ...
        // Step 2: L2 per-c1 local k-means
        ...
        // Step 3: L3 per-(c1,c2) local k-means
...
void HBlockIndex::add(const float* h_x, int n)
{
    ...
    // Build leaf ID arrays (no PQ codes in v18)
    ...
    // Compute JL projections for all base vectors with Pi1, scatter into leaf layout
    printf("  Building JL leaf projections %.2f GB ... ",
           (double)total_blocks * d_proj_ * leaf_size_ * 4 / 1e9); fflush(stdout);
    {
        const float one2 = 1.f, zero2 = 0.f;
        float* d_all_proj;
        CUDA_CHECK(cudaMalloc(&d_all_proj, (long long)n * d_proj_ * sizeof(float)));
        CUBLAS_CHECK(cublasSetStream(cublas_, nullptr));
        // d_all_proj[n, d_proj] = d_base_vecs_[n, d] @ Pi1.T
        CUBLAS_CHECK(cublasSgemm(cublas_, CUBLAS_OP_T, CUBLAS_OP_N,
                                 d_proj_, n, d_,
                                 &one2, d_Pi1_, d_,
                                 d_base_vecs_, d_,
                                 &zero2, d_all_proj, d_proj_));

        CUDA_CHECK(cudaMalloc(&d_leaf_proj_vecs_,
                              (long long)total_blocks * d_proj_ * leaf_size_ * sizeof(float)));
        ...
        scatter_jl_to_leaf_kernel<<<total_blocks, leaf_size_>>>(
            ...
            d_leaf_proj_vecs_, total_blocks, d_proj_, leaf_size_);
        CUDA_CHECK(cudaDeviceSynchronize());
        cudaFree(d_all_proj);
    }
    printf("done\n");
    ...
    ntotal_ = n;
    printf("  Built %d leaf blocks (leaf_size=%d, K1=%d, K2=%d, K3=%d)\n",
           total_blocks, leaf_size_, K1_, K2_, K3_);
                          ...
                          float* h_dists, int* h_ids) const
{
    ...
    // Phase 1: Routing (d_q_proj1 is also the JL query for leaf scan)
    route_queries_v18(cublas_,
                      d_Pi1_, d_Pi2_, d_Pi3_,
                      d_route1_cents_proj_, d_route1_cents_full_, d_route1_norms_,
                      d_route2_cents_proj_, d_route2_cents_full_, d_route2_norms_,
                      d_route3_cents_proj_, d_route3_cents_full_, d_route3_norms_,
                      d_pair_blk_start_, d_pair_blk_count_,
                      h_q, nq, d_, d_proj_, K1_, K2_, K3_,
    ...
    // Phase 3: Sort pairs by leaf_id
    gpu_build_and_sort_pairs_v18(nq, n_pairs, n_leaf_blocks_, ws_.max_leaf_sel, ws_);
    auto t3 = Clock::now();

    // Phase 4: JL leaf scan
    cudaStream_t s = ws_.stream;
    launch_leaf_jl_v18(
        ws_.d_pair_leaf_b, ws_.d_pair_qid_b,
        d_leaf_proj_vecs_, ws_.d_q_proj1,
        d_leaf_ids_, d_leaf_sizes_,
        ws_.d_out_dists, ws_.d_out_ids,
        n_pairs, d_proj_, leaf_size_, s);
    auto t4 = Clock::now();

    // Phase 5: Merge JL → top-rerank_r candidates per query
    gpu_merge_jl_topk_v18(nq, n_pairs, rerank_r_, ws_);
    auto t5 = Clock::now();

    // Phase 6: Gather original vectors
    launch_gather_vecs_v18(d_base_vecs_, ws_.d_jl_ids, ws_.d_cand_vecs,
                            nq, rerank_r_, d_, s);
    auto t6 = Clock::now();

    // Phase 7: Exact L2 rerank
    launch_exact_rerank_v18(ws_.d_q_batch, ws_.d_cand_vecs, ws_.d_jl_ids,
    ...
    printf("  [v18] Route=%.2fms  PairCount=%.2fms  GPUSort=%.2fms (%d pairs)"
           "  JLScan=%.2fms  MergeJL=%.2fms  Gather=%.2fms"
