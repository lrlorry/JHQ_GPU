...
public:
    ...
    // alpha only sizes the refinement budget ck = alpha * k, which search_gpu
    // reads per call; nothing trained depends on it. The captured graph is
    // keyed on ck (search.cu compares ws.graph_ck) and re-captures itself when
    // this changes, so one index can be searched at several budgets. That is
    // what a calibration pass needs: the alternative, rebuilding the index per
    // alpha, would take minutes and measure training noise as well.
    void set_alpha(float a) { alpha_ = a; }
    // Calibration mode: no graph capture, because ck moves every call.
    void set_calibrating(bool c) { ws_.no_graph = c; }
    float alpha() const { return alpha_; }

    int ntotal() const { return ntotal_; }
    // out[id] = the IVF list that id lives in, for JHQ_DIAG's candidate
    // recall. Built from the inverted lists on demand, not kept resident.
    void vector_lists(std::vector<int>& out) const;
    int dim()    const { return d_; }

    // The trained coarse quantiser, so the CPU reference can be given the same
    // one. JHQ_official reads its centroids and assignments from files rather
    // than training its own, which is what makes a fair CPU-versus-GPU
    // comparison possible at all: hand both the same routing and the only
    // difference left is the quantisation and the search. Trained on the
    // rotated data, so the CPU side must rotate with the same Pi -- which the
    // cached JL state carries.
...
private:
    ...
    // Checks the Cartesian identity the factorised LUT rests on, against the
    // centroids rather than against which construction was asked for.
    void   verify_split_lut() const;
