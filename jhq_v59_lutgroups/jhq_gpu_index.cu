#include "jhq_v59_lutgroups/jhq_gpu_index.cuh"
...
#endif
#include "jhq_v59_lutgroups/encode.cuh"
#include "jhq_v59_lutgroups/train_pq_gpu.cuh"
#include "jhq_v59_lutgroups/train_res_gpu.cuh"
...
void JHQGpuIndex::verify_split_lut() const {
    // The factorisation is exact only if the primary codebook really is a
    // Cartesian product of per-dimension levels, so that a code's value in
    // one group of digits does not depend on the others. A Lloyd-refined
    // product quantiser is not, and would give wrong distances silently, so
    // this runs at train time and throws rather than trusting the flag.
    //
    // Generalised from the half-split: for each group, the representative
    // code that carries this code's digits for that group and zeros elsewhere
    // must agree with the code on exactly the dimensions the group covers.
    if (K_ != 256)
        throw std::runtime_error(
            "the factorised LUT needs K=256 (B=8); got K="
            + std::to_string(K_));
    if (Ds_ % JHQ_LUT_GROUPS != 0)
        throw std::runtime_error(
            "JHQ_LUT_GROUPS=" + std::to_string(JHQ_LUT_GROUPS) +
            " must divide Ds so the groups partition the dimensions; got Ds="
            + std::to_string(Ds_));
    const int GW = Ds_ / JHQ_LUT_GROUPS;
    for (int m = 0; m < M_; ++m) {
        const float* base = cb_->centroids(m);           // [K][Ds]
        for (int c = 0; c < K_; ++c) {
            const float* cc = base + (size_t)c * Ds_;
            for (int g = 0; g < JHQ_LUT_GROUPS; ++g) {
                const int sh   = 8 - (g + 1) * JHQ_LUT_GBITS;
                const int digs = (c >> sh) & (JHQ_LUT_GSIZE - 1);
                const float* rep = base + (size_t)(digs << sh) * Ds_;
                for (int j = g * GW; j < (g + 1) * GW; ++j)
                    if (cc[j] != rep[j])
                        throw std::runtime_error(
                            "the primary codebook is not a Cartesian product "
                            "-- subspace " + std::to_string(m) + ", code " +
                            std::to_string(c) + ", group " +
                            std::to_string(g) + ", dim " + std::to_string(j) +
                            ". The factorised LUT would be wrong. Use v46 for "
                            "a Lloyd-refined product quantiser.");
            }
        }
    }
}
...
// jhq_v59_lutgroups/jhq_gpu_index.cuh's Params::add_batch comment for
