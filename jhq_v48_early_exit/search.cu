    ...
    int nprobe, int M, int N, int ck, int cap, bool lut_in_smem, int tile_m)
{
    ...
    const int* my_ids  = probe_ids     + bqi * nprobe;
    const int* my_offs = probe_offsets + bqi * (nprobe + 1);
    // tile_m no longer tiles this kernel's subspace loop: every script sets
    // JHQ_TILE_M_RT to M, which made the tiling a single pass, and the loop
    // now strides by the exit granularity instead. It stays in the signature
    // for scan_ivf_coalesced_kernel, which still uses it.
    (void)tile_m;
    ...
    for (int chunk = 0; chunk < total; chunk += BLOCK) {
        ...
        {
            // Every table entry is a squared distance and so non-negative, and
            // the subspaces not yet visited can only add. A partial sum that
            // already exceeds the ck-th best therefore cannot reach the
            // top-ck, and the remaining subspaces need be neither gathered nor
            // looked up. Exact, not an estimate: it is the same argument the
            // compaction uses, applied one subspace at a time.
            //
            // The threshold is read once and without synchronising, as the
            // append below already does. Either value is safe: *s_thresh is
            // the ck-th best seen so far, which is always at or above the
            // ck-th best of the whole set, so a > thr implies a is past the
            // final cut whichever version of thr was read.
            //
            // The exit is per warp, not per thread. Lanes run the subspaces in
            // lockstep, so one lane still needing them costs the others
            // nothing to skip; __all_sync makes the break uniform and is why
            // the loop sits outside `if (live)` -- every lane of the warp has
            // to reach it, including the ones past `total` in the last chunk,
            // which enter with done already set.
            ...
            // A partial sum that stopped early is already past thr, and thr
            // only ever falls, so the append test below rejects it for the
            // same reason the exit fired. It is stored as-is rather than as
            // infinity so that a run with JHQ_EXIT_EVERY >= M -- no exit at
            // all -- is bit-identical to v47.
            if (live) dist = a;
        }
    ...
    int ck, int k, int ntotal)
{
    ...
    if (!resid_lut_materialised()) {
        ...
        if (cb_smem < 0) {
            ...
            CUDA_CHECK(cudaFuncSetAttribute(residual_refine_fused_kernel,
                cb_smem ? cudaFuncAttributeMaxDynamicSharedMemorySize
