#include "hblock_v21/jhq_gpu_index.cuh"
      ...
      batch_size_(p.batch_size)
{
    ...
    if (p.K2 > 16 || p.K3 > 16)
        throw std::invalid_argument("v21 bitmask requires K2,K3 <= 16");
...
HBlockIndex::~HBlockIndex() {
    ...
    free_d(d_fine_c1d_);
    free_d(d_valid_c2_); free_d(d_valid_c3_);
...
void HBlockIndex::train(const float* h_x, int n_train)
{
    ...
    printf("[v21 train] d=%d d_proj=%d K1=%d K2=%d K3=%d ck1=%d ck2=%d ck3=%d\n",
    ...
    auto t1 = Clock::now();
    {
        ...
        CUBLAS_CHECK(cublasSgemm(cublas_, CUBLAS_OP_T, CUBLAS_OP_N,
                                 d_proj_, n_km, d_, &one, d_Pi1_, d_, d_x_gpu, d_, &zero, d_y_proj, d_proj_));
        ...
        // Step 2: L2 hierarchical k-means + build valid_c2 bitmask
        ...
        std::vector<uint16_t> h_valid_c2(K1_, 0);  // bitmask per c1
        ...
        for (int c1 = 0; c1 < K1_; c1++) {
            ...
            // bitmask: bits 0..K2e-1 set for valid children
            uint16_t mask = 0;
            for (int k = 0; k < K2e; k++) mask |= (uint16_t)(1u << k);
            h_valid_c2[c1] = mask;
        }
        ...
        // Upload L2 bitmask
        CUDA_CHECK(cudaMalloc(&d_valid_c2_, (long long)K1_ * sizeof(uint16_t)));
        CUDA_CHECK(cudaMemcpy(d_valid_c2_, h_valid_c2.data(),
                              (long long)K1_ * sizeof(uint16_t), cudaMemcpyHostToDevice));

        auto t4 = Clock::now();
        printf("%.1f ms\n", Ms(t4 - t3).count());

        // Step 3: L3 hierarchical k-means + build valid_c3 bitmask
        ...
        std::vector<uint16_t> h_valid_c3(n_c12, 0);  // bitmask per (c1,c2)
        ...
        for (int c1 = 0; c1 < K1_; c1++) {
            for (int c2 = 0; c2 < K2_; c2++) {
                ...
                // bitmask: bits 0..K3e-1 set for valid children
                uint16_t mask = 0;
                for (int k = 0; k < K3e; k++) mask |= (uint16_t)(1u << k);
                h_valid_c3[c12] = mask;
            }
        }
        ...
        // Upload L3 bitmask
        CUDA_CHECK(cudaMalloc(&d_valid_c3_, n_c12 * sizeof(uint16_t)));
        CUDA_CHECK(cudaMemcpy(d_valid_c3_, h_valid_c3.data(),
                              n_c12 * sizeof(uint16_t), cudaMemcpyHostToDevice));
                          ...
                          float* h_dists, int* h_ids) const
{
    ...
    gpu_build_and_sort_pairs_v21(nq, n_pairs, n_leaf_blocks_, ws_.max_leaf_sel, ws_);
    auto t3 = Clock::now();

    cudaStream_t s = ws_.stream;
    launch_leaf_flat_v21(
    ...
    gpu_merge_pq_topk_v21(nq, n_pairs, rerank_r_, ws_);
    auto t5 = Clock::now();

    launch_gather_vecs_v21(d_base_vecs_, ws_.d_pq_ids, ws_.d_cand_vecs,
                           nq, rerank_r_, d_, s);
    auto t6 = Clock::now();

    launch_exact_rerank_v21(ws_.d_q_batch, ws_.d_cand_vecs, ws_.d_pq_ids,
    ...
    printf("  [v21] Route=%.2fms  PairCount=%.2fms  GPUSort=%.2fms (%d pairs)"
