...
// HBlock v21: v17 + sparse subtree bitmask routing
// valid_c2[c1]       = 16-bit mask over K2 children of c1
// valid_c3[c1*K2+c2] = 16-bit mask over K3 children of (c1,c2)
// Only non-empty children compete for beam slots → effective ck > naive ck
class HBlockIndex {
public:
    struct Params {
        int K1         = 16;
...
private:
    ...
    // Valid-children bitmasks (built during train, used in search)
    uint16_t* d_valid_c2_ = nullptr;  // [K1]      — K2 bits per c1
    uint16_t* d_valid_c3_ = nullptr;  // [K1*K2]   — K3 bits per (c1,c2)
