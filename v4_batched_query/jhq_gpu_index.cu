#include "v4_batched_query/jhq_gpu_index.cuh"
#include "v4_batched_query/encode.cuh"
...
{
    if (n_train < nlist_)
        throw std::invalid_argument("n_train must be >= nlist for v4_batched_query");
...
void JHQGpuIndex::add(const float* h_x, int n) {
    ...
    if (ntotal_ != 0)
        throw std::runtime_error("v4_batched_query currently supports one add() call");
