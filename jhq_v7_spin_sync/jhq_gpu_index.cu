#include "jhq_v7_spin_sync/jhq_gpu_index.cuh"
#include "jhq_v7_spin_sync/encode.cuh"
...
{
    if (n_train < nlist_)
        throw std::invalid_argument("n_train must be >= nlist for v7_spin_sync");
...
void JHQGpuIndex::add(const float* h_x, int n) {
    ...
    if (ntotal_ != 0)
        throw std::runtime_error("v7_spin_sync currently supports one add() call");
