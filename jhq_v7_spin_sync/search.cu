#include "jhq_v7_spin_sync/search.cuh"
...
// ── select_probes_kernel (identical to v4) ────────────────────────────────────
...
// ── build_primary_lut_batched_kernel (identical to v4) ────────────────────────
...
// ── scan_ivf_batched_topk_kernel (identical to v4) ───────────────────────────
...
// ── build_residual_lut_batched_kernel (identical to v4) ──────────────────────
...
// ── residual_refine_batched_kernel (identical to v4) ─────────────────────────
...
// ── batched_topk_final_kernel (NEW) ──────────────────────────────────────────
// One block per query. k rounds of block-wide argmin over ck comp_dists.
// Resolves final ids in-kernel, eliminating all thrust/sort overhead.
// Fully capturable in a CUDA Graph (no dynamic device allocations).
__global__ void batched_topk_final_kernel(
    const float* __restrict__ comp_dists,   // [B, ck]
    const int*   __restrict__ topck_pos,    // [B, ck]
    const int*   __restrict__ list_ids,
    float*                    final_dists,  // [B, k]
    int*                      final_ids,    // [B, k]
    int ck, int k, int B)
{
    const int   BLOCK = blockDim.x;
    ...
    // Shared: [s_dists: ck][s_pos: ck][red_val: BLOCK][red_idx: BLOCK]
    extern __shared__ char shm[];
...
// ── capture_graph ─────────────────────────────────────────────────────────────
// Records the full B_full-query pipeline into a CUDA Graph on ws.stream.
// cuBLAS must already be set to ws.stream before calling.
    ...
    int ck, int k)
{
    ...
    // 8. Final top-k + id gather (no thrust, no dynamic alloc)
...
{
    if (ws.batch_cap <= 0)
        throw std::runtime_error("v6: workspace not initialised — call add() first");
    if (!ws.stream || !ws.h_q_pinned)
        throw std::runtime_error("v6: CUDA stream / pinned buffer not created");
    ...
    // Lazy graph capture (or re-capture when ck / nprobe changes).
    ...
    for (int qoff = 0; qoff < nq; qoff += batch_size) {
        ...
        // CPU: copy this batch into the pinned staging buffer (fast, no driver
        // involvement), including zero-padding for partial last batch.
        ...
        // Async H→D directly into the graph's fixed input pointer.
        // Queued on ws.stream — graph launch below waits for it automatically.
        CUDA_CHECK(cudaMemcpyAsync(
            ws.d_q_batch, ws.h_q_pinned,
            (long long)B_full * d * sizeof(float),
            cudaMemcpyHostToDevice, ws.stream));

        // One graph launch replaces the entire multi-kernel pipeline.
        CUDA_CHECK(cudaGraphLaunch(ws.graph_exec, ws.stream));

        // Async D→H — overlaps with next batch's H2D + compute.
        CUDA_CHECK(cudaMemcpyAsync(
    ...
    // Busy-wait instead of sleep-based sync to avoid OS wakeup latency
    // (~60ms in container environments) when GPU kernels finish in <1ms.
    cudaError_t err;
