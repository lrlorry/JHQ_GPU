...
#endif
// PM = M by default since v24: Algorithm 1 step 3 computes the full primary
// distance for every candidate, and a build that does not is not the paper's
// algorithm. The cascade remains available and is worth having -- at 1/2 it
// buys 38% for a recall change inside the build's own +-2e-4 noise -- but it
// is opt-in, and it is reported as its own row rather than folded into JHQ-GPU.
//
// The old default of 1/4 is why this changed. It costs 0.9747 -> 0.9647 on
// vogue, fifty times the noise, and it applied to anyone who did not set
// JHQ_PFX_DEN in the environment.
#ifndef JHQ_PREFIX_DEN
#define JHQ_PREFIX_DEN 1
#endif
...
// Float by default since v24. The half table was measured against it on
// vogue: recall identical to the digit (0.9747/0.9748 half, 0.9747/0.9749
// float) and throughput 34,809/34,563 against 38,309/38,398 -- the float table
// is 10% faster. It does not fit in shared memory at M=96, where the half one
// does, so the expectation was the other way round; what it saves is a
