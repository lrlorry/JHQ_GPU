...
// v39: SPACEV-100M int8, forked from v38. Routing / beam-search / pair
// build kernels are copied verbatim from v38/v36 (float-based, unchanged
// algorithm). leaf_flat_kernel_v39 changes in two ways versus v38's
// leaf_flat_kernel_v29: (1) raw base vectors and raw queries are read as
// int8 with int32 squared-difference accumulation (inherited from v38
// unchanged), and (2) PQ codes/ids and raw vectors are resolved through a
// bounded GPU region pool (region_id -> pool slot indirection) instead of
// flat permanently-resident arrays (new in v39 — see the comment above
// launch_leaf_flat_v39() and HBlockIndex::build_region_layout() /
// fetch_code_regions() / fetch_raw_regions() in jhq_gpu_index.cu).
...
void route_gpu_v29(
    ...
    SearchWorkspace& ws)
{
    if (nq > ws.batch_cap) throw std::runtime_error("hblock_v39: nq > batch_cap");
