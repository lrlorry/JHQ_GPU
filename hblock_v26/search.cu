// hblock_v26/search.cu  — GPU kernels identical to v22, namespace hblock_v26.
...
void route_gpu_v26(
    ...
    SearchWorkspace& ws)
{
    if (nq > ws.batch_cap) throw std::runtime_error("hblock_v26: nq > batch_cap");
...
__global__ void build_pairs_kernel_v26(
...
void gpu_build_and_sort_pairs_v26(
    ...
    int max_leaf_sel, SearchWorkspace& ws)
{
    ...
    build_pairs_kernel_v26<<<(nq+255)/256, 256, 0, s>>>(
...
// v26: output ALL leaf_size candidates per pair directly — no per-block top_p sorting.
// Global top-rerank_r is done in segmented_topk_kernel_v26.
__global__ void leaf_flat_kernel_v26(
    ...
    int d, int Kr, int Br, int bpv, int leaf_size)
{
    ...
    long long out_base = (long long)pi * leaf_size;
    d_out_dists[out_base + tid] = fd;
    d_out_ids  [out_base + tid] = (tid < n_vecs)
        ? leaf_ids_data[(long long)leaf_blk * leaf_size + tid] : -1;
}

void launch_leaf_flat_v26(
    ...
    cudaStream_t stream)
{
    leaf_flat_kernel_v26<<<n_pairs, leaf_size, 0, stream>>>(
        d_pair_leaf_ids, d_pair_qids,
        d_leaf_sizes, d_leaf_codes, d_leaf_ids_data,
        d_lut_fine, d_out_dists, d_out_ids,
        d, Kr, Br, bpv, leaf_size);
    CUDA_CHECK(cudaGetLastError());
}

__global__ void fill_iota_kernel_v26(int* arr, int n) {
...
__global__ void segmented_topk_kernel_v26(
...
void gpu_merge_pq_topk_v26(int nq, int n_pairs, int rerank_r, int n_per_pair, SearchWorkspace& ws)
{
    cudaStream_t s = ws.stream;
    int k = std::min(rerank_r, (int)K_MAX);
    fill_iota_kernel_v26<<<(n_pairs+255)/256, 256, 0, s>>>(ws.d_pair_leaf_a, n_pairs);
    CUDA_CHECK(cub::DeviceRadixSort::SortPairs(
        ws.d_cub_tmp, ws.cub_bytes,
    ...
    segmented_topk_kernel_v26<<<nq, BLOCK, smem, s>>>(
...
__global__ void gather_vecs_kernel_v26(
...
void launch_gather_vecs_v26(
    const float* d_base_vecs, const int* d_cand_ids, float* d_cand_vecs,
    int B, int R, int d, cudaStream_t s)
{
    dim3 grid(R, B);
    gather_vecs_kernel_v26<<<grid, 128, 0, s>>>(d_base_vecs, d_cand_ids, d_cand_vecs, R, d);
    CUDA_CHECK(cudaGetLastError());
}

__global__ void exact_rerank_kernel_v26(
    ...
    int B, int R, int d, int k, cudaStream_t s)
{
    ...
    exact_rerank_kernel_v26<<<B, BLOCK, smem, s>>>(
...
blk_qdist_v26(const float* __restrict__ s_q, float q_norm,
...
try_visit_v26(int* __restrict__ vis, int b, int n_blks)
{
...
// ─────────────────────────────────────────────────────────────────────────────
__global__ void compute_adj_kernel_v26(
    ...
    int n_blks, int d_proj, int degree, cudaStream_t s)
{
    ...
    compute_adj_kernel_v26<<<grid, 128, 0, s>>>(
...
// ─────────────────────────────────────────────────────────────────────────────
template <int SPT>
__global__ void block_search_fused_v26(
    ...
    int entry_per_cell, int bitmap_words)
{
        ...
        for(int i2 = 0; i2 < ck2; i2++){
            ...
            for(int i3 = 0; i3 < ck3; i3++){
                ...
                for(int start = 0; start < bc; start += 32){
                    ...
                    if(bi < bc){
                        ck_d  = blk_qdist_v26(s_q, q_norm, d_blk_proj, d_blk_norm,
                                               bs + bi, d_proj);
                        ck_id = bs + bi;
                    }
                    for(int ep = 0; ep < entry_per_cell; ep++){
                        ...
                        bool fv = false;
                        if(tid == 0 && ins_id >= 0)
                            fv = try_visit_v26(vis, ins_id, n_blks);
    ...
    for(int iter = 0; iter < budget; iter++){
        ...
        if(tid < degree){
            ...
            if(try_visit_v26(vis, nb, n_blks)){
                new_d  = blk_qdist_v26(s_q, q_norm, d_blk_proj, d_blk_norm, nb, d_proj);
                new_id = nb;
            }
        }
...
#define LAUNCH_BEAM(SPT_VAL) \
    block_search_fused_v26<SPT_VAL><<<B, 32, smem, ws.stream>>>( \
        ws.d_top1_ids, ws.d_top2_beam, ws.d_top3_beam, \
