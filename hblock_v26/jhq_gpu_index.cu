#include "hblock_v26/jhq_gpu_index.cuh"
    ...
    const std::vector<int>& h_pair_blk_count)
{
    ...
    gpu_build_block_adj_v26(d_blk_proj_gpu_, d_blk_norm_gpu_,
...
#define FD(p) do{if(ws_.p){cudaFree(ws_.p);ws_.p=nullptr;}}while(0)
    ...
    CUDA_CHECK(cudaMalloc(&ws_.d_out_dists,(long long)max_pairs*leaf_size_*sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ws_.d_out_ids,  (long long)max_pairs*leaf_size_*sizeof(int)));
                          ...
                          float* h_dists, int* h_ids) const
{
    ...
    for(int qstart=0;qstart<nq;qstart+=batch_size_){
        ...
        auto t1=Clock::now();
        gpu_block_search_v26(nb, n_leaf_blocks_, d_proj_,
        ...
        auto t3=Clock::now();
        gpu_build_and_sort_pairs_v26(nb,n_pairs,n_leaf_blocks_,ws_.max_leaf_sel,ws_);
        ms_pairs+=Ms(Clock::now()-t3).count();

        auto t4=Clock::now();
        launch_leaf_flat_v26(ws_.d_pair_leaf_b,ws_.d_pair_qid_b,
        ...
        auto t5=Clock::now();
        gpu_merge_pq_topk_v26(nb,n_pairs,rerank_r_,leaf_size_,ws_);
        ms_merge+=Ms(Clock::now()-t5).count();

        auto t6=Clock::now();
        launch_gather_vecs_v26(d_base_vecs_,ws_.d_pq_ids,ws_.d_cand_vecs,nb,rerank_r_,d_,s);
        ms_gather+=Ms(Clock::now()-t6).count();

        auto t7=Clock::now();
        launch_exact_rerank_v26(ws_.d_q_batch,ws_.d_cand_vecs,ws_.d_pq_ids,
