...
// v38: SPACEV-100M int8 port of hblock_v36. Routing / beam-search / pair
// build kernels are copied verbatim from v36 (float-based, unchanged
// algorithm). The only kernel that changes is leaf_flat_kernel_v29's exact
// rerank stage: raw base vectors and raw queries are read as int8, and the
// squared-difference accumulation is done in int32 (not float) — see task
// spec section 4. No PRR / certified bounds; single search mode (v36's
// fixed-per-block-top-16 behavior).
#include "hblock_v38/search.cuh"
...
void route_gpu_v29(
    ...
    SearchWorkspace& ws)
{
    if (nq > ws.batch_cap) throw std::runtime_error("hblock_v38: nq > batch_cap");
...
// ── v38 key kernel: per-block PQ filter → exact L2 (int8, int32 accum) → top klocal ─

// Both host and device code must agree on this shared-memory layout, since
// the int8 s_query array (d bytes) generally does not leave the following
// int-typed arrays 4-byte aligned. align4() rounds a byte offset up to the
// next multiple of 4; used identically in the kernel and in
// launch_leaf_flat_v29's smem-size computation below.
...
__global__ void leaf_flat_kernel_v29(
    ...
    const int8_t*  __restrict__ d_base_vecs_i8,
    ...
    int per_block_r, int klocal)
{
    ...
    for (int r = 0; r < per_block_r; r++) {
        ...
        int partial = 0;
        if (cand_id >= 0) {
            const int8_t* vec = d_base_vecs_i8 + (long long)cand_id * d;
            for (int j = tid; j < d; j += blockDim.x) {
...
void launch_leaf_flat_v29(
    ...
    const int8_t* d_base_vecs_i8, const int8_t* d_q_batch_i8,
    float* d_out_dists, int* d_out_ids,
    ...
    cudaStream_t stream)
{
    ...
    leaf_flat_kernel_v29<<<n_pairs, leaf_size, (int)smem, stream>>>(
        d_pair_leaf_ids, d_pair_qids,
        d_leaf_sizes, d_leaf_codes, d_leaf_ids_data,
        d_lut_fine, d_base_vecs_i8, d_q_batch_i8,
