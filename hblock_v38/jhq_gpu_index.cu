#include "hblock_v38/jhq_gpu_index.cuh"
...
void HBlockIndex::train(const float* h_x, int n_train)
{
    ...
    printf("[v38 train] d=%d d_proj=%d K1=%d K2=%d K3=%d ck1=%d ck2=%d ck3=%d"
    ...
    printf("[v38 train] total=%.1f ms  sigma_r3=%.4f\n", Ms(Clock::now()-T0).count(), sigma);
}
...
void HBlockIndex::add(I8BinReader& reader, int n)
{
    ...
    printf("[v38 add] n=%d  d=%d  add_batch_size=%d  mini_km_iters=%d\n",
    ...
    printf("[v38 add total] %.1f ms  blocks=%d\n",
    ...
    bool verbose) const
{
    ...
    throw std::runtime_error(
        "HBlockIndex::diagnose_missed_gt: not implemented in v38 (this build exists "
        "purely as a flat-resident comparison baseline for v39/v42/v43's region "
        "partitioning; routing diagnostics were never needed for that comparison)");
}
