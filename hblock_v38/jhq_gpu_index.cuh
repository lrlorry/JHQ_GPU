...
// HBlock v38: plain SPACEV-100M int8 port of hblock_v36 (no PRR / certified
// bounds). Same tree routing + block graph + per-block fixed top-16 exact
// rerank as v36; the only algorithmic difference is the raw-vector storage
// dtype (int8 instead of float32) and the streamed add() path required to
// avoid materializing the 100M-vector base as a 40GB float array.
