...
// ── HBlock v5: transposed LeafFine (global leaf sort) ────────────────────────
//
// Same index structure as v2 (JL + analytical product code).
// Only the LeafFine step is changed: instead of one CUDA block per
// (query, leaf-slot) pair doing independent random HBM reads, v5 globally
// sorts all (query, leaf) pairs by leaf_id and launches one block per
// unique leaf. Each block loads its leaf codes into shared memory ONCE, then
// processes every query that needs it sequentially.
//
// Expected improvement: ~20x fewer HBM reads for leaf_codes → LeafFine
// drops from ~42 ms to ~1-3 ms.
class HBlockIndex {
public:
    struct Params {
        int   K1        = 64;
