#include "hblock_v16/jhq_gpu_index.cuh"
#include "hblock_v16/encode.cuh"
...
HBlockIndex::~HBlockIndex() {
    ...
    cudaFree(ws_.d_leaf_counts); cudaFree(ws_.d_seg_offsets);
...
void HBlockIndex::alloc_workspace() {
    ...
    // ── v16: leaf-block CSR buffers ───────────────────────────────────────────
    cudaFree(ws_.d_leaf_counts); cudaFree(ws_.d_seg_offsets);
    CUDA_CHECK(cudaMalloc(&ws_.d_leaf_counts, (long long)(n_leaf_blocks_ + 1) * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&ws_.d_seg_offsets, (long long)(n_leaf_blocks_ + 1) * sizeof(int)));
    ws_.n_leaf_blocks_cap = n_leaf_blocks_;

    // CUB temp: max of scan(B+1), leaf sort (14-bit), qid sort (10-bit),
    //           ExclusiveSum for CSR (n_leaf_blocks+1 items)
    size_t scan_bytes = 0, sort_leaf_bytes = 0, sort_qid_bytes = 0, csr_bytes = 0;
    cub::DeviceScan::ExclusiveSum(nullptr, scan_bytes,
        (int*)nullptr, (int*)nullptr, B + 1);
    ...
    cub::DeviceScan::ExclusiveSum(nullptr, csr_bytes,
        (int*)nullptr, (int*)nullptr, n_leaf_blocks_ + 1);
    ws_.cub_bytes = std::max({scan_bytes, sort_leaf_bytes, sort_qid_bytes, csr_bytes});
    cudaFree(ws_.d_cub_tmp);
                          ...
                          float* h_dists, int* h_ids) const
{
    ...
    // ── Phase 4: build CSR + multi-query leaf kernel ──────────────────────────
    cudaStream_t s = ws_.stream;
    gpu_find_segments_and_launch_leaf(
        n_pairs, n_leaf_blocks_,
        ws_.d_pair_qid_b,
    ...
    printf("  [v16] Route=%.1f ms  GPUSort=%.1f ms (%d pairs)"
