...
struct SearchWorkspace {
    ...
    // ── v16: leaf block CSR (for multi-query leaf kernel) ────────────────────
    int*   d_leaf_counts  = nullptr;   // [n_leaf_blocks + 1] per-leaf pair count
    int*   d_seg_offsets  = nullptr;   // [n_leaf_blocks + 1] exclusive prefix sum
    int    n_leaf_blocks_cap = 0;      // capacity of above two buffers
...
// Build CSR (d_seg_offsets) from sorted pair array, then launch multi-query leaf kernel.
// One block per leaf block; leaf codes loaded into shared mem once, all queries looped.
