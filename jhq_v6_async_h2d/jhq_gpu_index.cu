#include "jhq_v6_async_h2d/jhq_gpu_index.cuh"
#include "jhq_v6_async_h2d/encode.cuh"
...
{
    if (n_train < nlist_)
        throw std::invalid_argument("n_train must be >= nlist for v6_async_h2d");
...
void JHQGpuIndex::add(const float* h_x, int n) {
    ...
    if (ntotal_ != 0)
        throw std::runtime_error("v6_async_h2d currently supports one add() call");
