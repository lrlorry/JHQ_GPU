#!/bin/bash
# Run hblock_v28 on both vogue-768 and arxiv-768
# v28: per-block exact rerank — PQ prefilter (top per_block_r) → exact L2 → merge.
# Eliminates cross-block PQ ranking error; oracle recall ≈ actual recall.

set -e
BIN=./build/demo_hblock_v28
...
run_dataset() {
    ...
    local CSV=results/hblock_v28_${NAME}.csv
    echo "========================================"
...
echo ""
echo "All done. Results in results/hblock_v28_vogue.csv and results/hblock_v28_arxiv.csv"
