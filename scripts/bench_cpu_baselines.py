#!/usr/bin/env python3
"""
Run CPU baselines (FAISS IVFPQ, JQ+IVF, JHQ+IVF) on vogue-768 locally.
Requires: pip install faiss-cpu numpy
Run from /Users/apple/github/JHQ_GPU:
  python3 scripts/bench_cpu_baselines.py
"""
import os, sys, time, csv, re, subprocess
import numpy as np

JHQ_REPRO   = os.path.expanduser("~/github/JHQ_repro")
BUILD_BIN   = os.path.join(JHQ_REPRO, "build", "examples")
DATA_DIR    = os.path.join(JHQ_REPRO, "data", "vogue768")
OUT_CSV     = os.path.join(os.path.dirname(__file__), "..", "results", "cpu_vogue.csv")

BASE_F  = os.path.join(DATA_DIR, "vogue768_base.fvecs")
QUERY_F = os.path.join(DATA_DIR, "vogue768_query.fvecs")
GT_F    = os.path.join(DATA_DIR, "vogue768_groundtruth.ivecs")
...
try:
    ...
    print("faiss-cpu not found, skipping FAISS baselines")
