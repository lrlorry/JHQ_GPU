#!/usr/bin/env python3
"""
Sweep hblock_v17 recall-QPS tradeoff for one or more datasets.

Sweeps ck (ck1=ck2=ck3) and rerank_r independently, saving one CSV per dataset.

Usage (run on server from /root/JHQ_GPU):
  python3 scripts/sweep_hblock_v17.py --dataset vogue
  python3 scripts/sweep_hblock_v17.py --dataset arxiv
  python3 scripts/sweep_hblock_v17.py --dataset both
"""
import argparse, csv, os, re, subprocess, sys

REPO   = os.path.expanduser("~/JHQ_GPU")
BINARY = os.path.join(REPO, "build", "demo_hblock_v17")
...
DATASETS = {
    ...
    "arxiv": {
        ...
        "label": "HBlock-v17 (Arxiv-768)",
    },
}

# ck1=ck2=ck3 sweep points → probed cells = ck^3
CK_SWEEP     = [1, 2, 3, 4, 5, 6, 7, 8, 10, 12]
# rerank_r is set to max(64, 2 * ck^3) so it's always >= probed cells
K1, K2, K3   = 16, 16, 16
BATCH        = 1024
D_PROJ       = 64
K_TOPK       = 10
...
def main():
    ...
    for ds in targets:
        out = os.path.join(args.out_dir, f"hblock_v17_{ds}.csv")
