#!/bin/bash
# Run hblock_v25 on both vogue-768 and arxiv-768
# v25 key change: semantic blocks via mini k-means within L3 cells
# Sweeps: graph_budget × top_p (mini_km_iters=5 fixed)
# Output: results/hblock_v25_vogue.csv  results/hblock_v25_arxiv.csv

set -e
BIN=./build/demo_hblock_v25
...
run_dataset() {
    ...
    local CSV=results/hblock_v25_${NAME}.csv
    echo "========================================"
    ...
    for BUD in $BUDGETS; do
        for TP in $TOP_PS; do
            echo "--- budget=$BUD  top_p=$TP  mini_km=$MINI_KM ---"
            OUT=$($BIN $BASE $QRY $GT \
                $K1 $K2 $K3 $CK1 $CK2 \
                $K $BATCH $D_PROJ $RERANK_R $KM_ITERS \
                $DEGREE $BUD $ENTRY $C2N $C1N $TP $MINI_KM 2>&1)
            echo "$OUT"
...
echo ""
echo "All done. Results in results/hblock_v25_vogue.csv and results/hblock_v25_arxiv.csv"
