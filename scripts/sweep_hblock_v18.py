#!/usr/bin/env python3
"""
Sweep hblock_v18 recall-QPS tradeoff (JL coarse scan, no PQ).

Usage (run on server from /root/JHQ_GPU):
  python3 scripts/sweep_hblock_v18.py --dataset vogue
  python3 scripts/sweep_hblock_v18.py --dataset arxiv
  python3 scripts/sweep_hblock_v18.py --dataset both
"""
import argparse, csv, os, re, subprocess, sys

REPO   = os.path.expanduser("~/JHQ_GPU")
BINARY = os.path.join(REPO, "build", "demo_hblock_v18")
...
def run_one(base, query, gt, ck, rerank_r):
    ...
    if not m:
        raise RuntimeError(f"cannot parse RESULT from:\n{out[-1000:]}")
...
def sweep_dataset(ds_name, ds_info, out_csv):
    ...
    print(f"{'='*60}")
    for ck in CK_SWEEP:
        rerank_r = min(64, max(32, ck * ck * ck))
...
def main():
    ...
    for ds in targets:
        out = os.path.join(args.out_dir, f"hblock_v18_{ds}.csv")
