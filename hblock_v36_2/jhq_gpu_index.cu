...
// jhq_gpu_index.cuh) -- no region partitioning, no GPU pool. It exists
// specifically as a controlled comparison point for hblock_v39/v42/v43:
// identical train/add/search pipeline, identical SPACEV data, identical
// encoding -- the only difference is that PQ codes, vector ids, and raw
// vectors are flat permanently-GPU-resident arrays here instead of being
// routed through region_id -> pool-slot indirection. Any recall difference
// between this and v39/v42/v43 on the same data is attributable to the
// region-partitioning work, not to something else changing at the same time.
...
// Elementwise int8 -> float32 cast, GPU side. Used by add()'s bulk encode
// path so casting 100M-scale batches doesn't serialize with the GPU
// pipeline -- exact-integer cast, lossless (every int8 value fits exactly
// in a float32).
...
void HBlockIndex::add(I8BinReader& reader, int n)
{
    ...
    if (reader.dim != d_) throw std::runtime_error("I8BinReader dim mismatch with index dim");

    n = (n < 0) ? reader.npts : std::min(n, reader.npts);
    ...
    // Flat, permanently GPU-resident storage -- no region layer. This is
    // the deliberate contrast point against hblock_v39/v42/v43.
    T0 = Clock::now();
    ...
    T0 = Clock::now();
    printf("  [upload base vecs int8] %.2f GB ...\n",(double)n*d_*1/1e9);
    CUDA_CHECK(cudaMalloc(&d_base_vecs_i8_,(long long)n*d_*sizeof(int8_t)));
    CUDA_CHECK(cudaMemcpy(d_base_vecs_i8_,h_base_i8_all.data(),(long long)n*d_*sizeof(int8_t),cudaMemcpyHostToDevice));
                          ...
                          float* h_dists, int* h_ids, int ef) const
{
    ...
    std::vector<float> h_q_cast((long long)batch_size_ * d_);
...
// Ported from hblock_v36's diagnose_missed_gt (same routing/graph classification
// logic) with two changes: queries are int8 (cast to float per-batch, matching
// search()'s pattern) and ef is a parameter instead of implicitly == max_ef_, so
    ...
    bool verbose) const
{
    ...
    for (int qstart = 0; qstart < nq; qstart += batch_size_) {
        ...
        for (int qi = 0; qi < nb; qi++) {
            ...
            std::vector<bool> cell_sel((long long)K1_ * K2_ * K3_, false);
            for (int ci = 0; ci < n_c3; ci++)
                if (top3[ci] >= 0) cell_sel[top3[ci]] = true;

            int vcnt = ws_.h_leaf_cnt[qi];
            std::vector<bool> visited(n_blks, false);
            for (int sidx = 0; sidx < vcnt; sidx++) {
                int blk = ws_.h_block_sel[(long long)qi * max_ls + sidx];
            ...
            while (bfs_head < bfs_tail) {
                ...
                for (int dd = 0; dd < deg; dd++) {
                    int nb2 = h_block_adj_[(long long)cur * deg + dd];
            ...
            const int32_t* gt = h_gt + (long long)(qstart + qi) * gt_k;
            int n_valid = 0, n_hit = 0;
            for (int g = 0; g < k; g++) {
                int gt_vid = gt[g];
    ...
    RoutingDiag diag;
    diag.cnt_total = cnt_total;
    diag.cnt_found = cnt_found;
    diag.cnt_A = cnt_A;
    diag.cnt_B = cnt_B;
    diag.cnt_C = cnt_C;
    diag.n_eval = n_eval;
    ...
    if (verbose) {
        long long n_miss = cnt_A + cnt_B + cnt_C;
        printf("\n=== Missed-GT Diagnostic (%d queries, k=%d, ef=%d) ===\n", nq, k, ef);
