// demo_hblock_v39_sift100m: SIFT/BIGANN .bvecs run for hblock_v39's region
// partitioning + bounded GPU region pool, using the same BVecsReader /
// argument / recall conventions as demo_hblock_v40.cu so results are
// directly comparable to hblock_v40's full-resident split-kernel baseline
// on the same data. search() also prints per-call region fetch-planning
// diagnostics (request/unique/bytes-H2D counts) to stdout.
...
int main(int argc, char** argv)
{
    ...
    const int code_cap   = (argc > 16) ? std::atoi(argv[16]) : 4096;
    const int raw_cap    = (argc > 17) ? std::atoi(argv[17]) : 4096;
    ...
    hblock_v39::HBlockIndex::Params p;
    ...
    hblock_v39::HBlockIndex index(base_reader.dim, p);
