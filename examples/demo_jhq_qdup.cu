// A probe, not a search variant: how much of the primary-code gather is
// already served by L2?
//
// The cluster-centric proposal (one IVF list loaded once, scored against the
// LUTs of every query that probes it) rests on one premise: those code reads
// are repeated, and the repeats reach DRAM. At nprobe=128, nlist=4096 and a
// 1000-query batch each list is probed by about 31 queries, so the premise is
// plausible -- but this card has 96 MB of L2, and if the repeats are already
// served there the whole rewrite buys nothing.
//
// JHQ_QUERY_DUP=D answers it without touching a kernel. It keeps the first
// nq/D queries and repeats each D times, adjacent so the copies are scheduled
// close together. Query count, block count, LUT builds, candidate count and
// selection work are all unchanged -- the only thing that moves is how much
// the probe lists overlap, which goes up by D exactly.
//
//   QPS rises with D   -> the repeats were reaching DRAM; the reuse is real
//                         and worth a kernel that captures it.
//   QPS flat in D      -> L2 already serves them; the rewrite buys nothing.
//
// The ground truth is repeated the same way, so recall stays a sanity check
// that this is still the same search.
...
int main(int argc, char** argv) {
    ...
    // See the note at the top of this file. Applied before anything is timed,
    // trained or built -- only the query set changes.
    int qdup = 1;
    ...
    if (qdup > 1) {
        ...
        for (int i = 0; i < nq; ++i) {
            const int src = i / qdup;
            std::copy(query.begin() + (size_t)src * d_query,
                      query.begin() + (size_t)(src + 1) * d_query,
                      q2.begin() + (size_t)i * d_query);
            std::copy(gt.begin() + (size_t)src * d_gt,
                      gt.begin() + (size_t)(src + 1) * d_gt,
                      g2.begin() + (size_t)i * d_gt);
        }
        query.swap(q2);
        gt.swap(g2);
        ng = nq;
        printf("QUERY_DUP : %d distinct queries repeated %d times each "
               "(%d total, unchanged)\n", nu, qdup, nq);
    }
