// v15 = v14's index verbatim (the six sources are byte-identical apart from
// the include prefix) + a corrected measurement harness. Nothing here changes
// what the GPU computes, so any QPS difference against v14 is noise, not a
// code change -- that is the point: it makes "did the fix move performance?"
...
#include "jhq_v15_eval_fix/jhq_gpu_index.cuh"
...
int main(int argc, char** argv) {
    if (argc < 8) {
        fprintf(stderr,
            ...
            "[ivf_iters=8] [batch_size=256] [out_prefix]\n"
            "\n"
