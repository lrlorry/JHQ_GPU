...
int main(int argc, char** argv) {
    ...
    // Calibration and reporting on disjoint halves.  demo_jhq_alpha_fast.cu
    // strides the calibration sample out of `query` and then reports recall
    // and QPS on all of `query`, so the budget is chosen knowing S of the nq
    // queries it is then scored on -- 32 of 1,000 on the frontier.  Even
    // indices calibrate here and odd indices measure, which is the split
    // Section 6.3's held-out experiment already uses.
    //
    // JHQ_AS_HOLDOUT=0 restores the pooled behaviour so one run can report
    // both and the difference between them is measured rather than argued.
    const bool HOLD = [](){ const char* e=std::getenv("JHQ_AS_HOLDOUT");
                            return !e || e[0] != '0'; }();
    std::vector<float> cal_q, ev_q; std::vector<int> ev_gt;
    if (HOLD) {
        for (int i = 0; i < nq; ++i) {
            const float* src = query.data() + (size_t)i * d;
            if (i % 2 == 0) cal_q.insert(cal_q.end(), src, src + d);
            else {
                ev_q.insert(ev_q.end(), src, src + d);
                const int* g = gt.data() + (size_t)i * d_gt;
                ev_gt.insert(ev_gt.end(), g, g + d_gt);
            }
        }
    } else { cal_q = query; ev_q = query; ev_gt = gt; }
    const int n_cal = (int)(cal_q.size() / d), n_ev = (int)(ev_q.size() / d);

    const int Sn = (S < n_cal) ? S : n_cal;
    const int stride = n_cal / Sn;
    std::vector<float> sq((size_t)Sn * d);
    for (int i = 0; i < Sn; ++i)
        std::copy(cal_q.begin() + (size_t)(i * stride) * d,
                  cal_q.begin() + (size_t)(i * stride + 1) * d,
                  sq.begin() + (size_t)i * d);
    ...
    auto full = [&](float a, const char* tag) {
        idx.set_alpha(a);
        idx.search(ev_q.data(), n_ev, k, out_dists.data(), out_ids.data());
        ...
        t /= R;
        RecallResult rr = evaluate_recall(out_ids.data(), ev_gt.data(), n_ev, k, d_gt);
        printf("%-10s alpha=%-6.0f recall=%.4f  qps=%.0f  (%.2f ms)  n_ev=%d\n",
               tag, a, rr.recall, n_ev / (t / 1000.0), t, n_ev);
        return std::make_pair(rr.recall, n_ev / (t / 1000.0));
    };
