...
int main(int argc, char** argv)
{
    if (argc < 4) {
        fprintf(stderr,
            ...
            "         [search_mode=0]   -- 0=FIXED_PER_BLOCK 1=CORRECTED_FIXED 2=CERTIFIED_PRR\n"
            "         [eps_mode=0]      -- 0=BLOCK_128 1=SUBBLOCK_32 2=VECTOR_LEVEL\n"
            "         [csv=]\n",
            argv[0]);
        return 1;
    }
    ...
    int search_mode = (argc > 13) ? atoi(argv[13]) : 0;
    int eps_mode    = (argc > 14) ? atoi(argv[14]) : 0;
    const char* csv_path = (argc > 15) ? argv[15] : nullptr;

    const char* mode_names[] = { "FIXED_PER_BLOCK", "CORRECTED_FIXED", "CERTIFIED_PRR" };
    const char* eps_names[]  = { "BLOCK_128", "SUBBLOCK_32", "VECTOR_LEVEL" };
    ...
    printf("max_ef=%d  K1=%d K2=%d K3=%d  graph_degree=%d  entry_per_cell=%d\n"
           ...
           "  search_mode=%s  eps_mode=%s\n",
           max_ef, K1, K2, K3, deg, epc, dprj, pbr, batch,
           mode_names[search_mode], eps_names[eps_mode]);

    hblock_v37_prr::HBlockIndex::Params p;
    p.K1=K1; p.K2=K2; p.K3=K3;
    p.max_ef=max_ef;
    p.d_proj=dprj; p.per_block_r=pbr;
    p.graph_degree=deg; p.entry_per_cell=epc;
    p.batch_size=batch;
    p.search_mode = (hblock_v37_prr::HBlockIndex::Params::SearchMode)search_mode;
    p.eps_mode    = (hblock_v37_prr::HBlockIndex::Params::EpsilonMode)eps_mode;

    hblock_v37_prr::HBlockIndex idx(d_base, p);
