...
int main(int argc, char** argv)
{
    if (argc < 4) {
        fprintf(stderr,
            ...
            "         [n_c2_nbrs=4] [n_c1_nbrs=2] [top_p=4] [mini_km_iters=5]\n",
            argv[0]);
        return 1;
    }
    ...
    int top_p         = (argc > 19) ? atoi(argv[19]) : 4;
    int mini_km_iters = (argc > 20) ? atoi(argv[20]) : 5;
    ...
    printf("K1=%d K2=%d K3=%d  ck1=%d ck2=%d  k=%d  batch=%d\n"
           ...
           "  n_c2_nbrs=%d  n_c1_nbrs=%d  top_p=%d  mini_km_iters=%d\n",
           K1, K2, K3, ck1, ck2, k, batch_sz,
           d_proj, rerank_r, km_iters,
           graph_degree, graph_budget, entry_per_cell,
           n_c2_nbrs, n_c1_nbrs, top_p, mini_km_iters);

    hblock_v25::HBlockIndex::Params p;
    p.K1            = K1;
    p.K2            = K2;
    p.K3            = K3;
    p.ck1           = ck1;
    ...
    hblock_v25::HBlockIndex idx(d, p);
    ...
    // Oracle recall: exact scan of visited blocks (graph/entry quality diagnostic)
    ...
    printf("\n=== Results ===\n");
    printf("graph_degree=%d  graph_budget=%d  entry_per_cell=%d  top_p=%d\n",
           graph_degree, graph_budget, entry_per_cell, top_p);
