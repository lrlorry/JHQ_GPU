...
int main(int argc, char** argv)
{
    if (argc < 4) {
        fprintf(stderr,
            ...
            "         [k=10] [batch=1024] [d_proj_route=128] [d_proj_scan=64]\n"
            "         [rerank_r=192] [km_iters=30]\n",
            argv[0]);
        return 1;
    }
    ...
    int d_proj_route = (argc > 12) ? atoi(argv[12]) : 128;
    int d_proj_scan  = (argc > 13) ? atoi(argv[13]) : 64;
    int rerank_r     = (argc > 14) ? atoi(argv[14]) : 192;
    int km_iters     = (argc > 15) ? atoi(argv[15]) : 30;
    ...
    printf("K1=%d K2=%d K3=%d  ck=%d×%d×%d  k=%d  batch=%d\n",
           K1, K2, K3, ck1, ck2, ck3, k, batch_sz);
    printf("d_proj_route=%d  d_proj_scan=%d  rerank_r=%d  km_iters=%d\n",
           d_proj_route, d_proj_scan, rerank_r, km_iters);
    printf("v19: split routing/scan proj + empty-centroid fix + K_MAX=192 JL_TOP_P=16\n");

    hblock_v19::HBlockIndex::Params p;
    p.K1           = K1;  p.K2 = K2;  p.K3 = K3;
    p.ck1          = ck1; p.ck2 = ck2; p.ck3 = ck3;
    p.d_proj_route = d_proj_route;
    p.d_proj_scan  = d_proj_scan;
    p.rerank_r     = rerank_r;
    p.km_iters     = km_iters;
    p.batch_size   = batch_sz;

    hblock_v19::HBlockIndex idx(d, p);
    ...
    printf("\nRecall@%d = %.4f   QPS = %.0f   Latency = %.2f ms  (avg %d runs)\n",
