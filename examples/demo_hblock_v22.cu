...
int main(int argc, char** argv)
{
    if (argc < 4) {
        fprintf(stderr,
            ...
            "         [graph_degree=32] [graph_budget=32]\n",
            argv[0]);
        return 1;
    }
    ...
    printf("K1=%d K2=%d K3=%d  ck1=%d ck2=%d  k=%d  batch=%d"
           ...
           K1, K2, K3, ck1, ck2, k, batch_sz, d_proj, rerank_r, km_iters,
           graph_degree, graph_budget);

    hblock_v22::HBlockIndex::Params p;
    p.K1           = K1;
    p.K2           = K2;
    p.K3           = K3;
    p.ck1          = ck1;
    ...
    hblock_v22::HBlockIndex idx(d, p);
    ...
    printf("\n=== Results ===\n");
    printf("graph_degree=%d  graph_budget=%d\n", graph_degree, graph_budget);
    printf("Recall@%d : %.4f\n", k, rec);
