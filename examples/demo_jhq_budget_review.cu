// Review experiment: the frozen budget harness with per-draw observations.
// Search/training code is unchanged; this target links the same v57 library.
// Even-indexed queries calibrate; odd-indexed queries evaluate.
// ARM DRAW preserves alpha, held-out recall, arm batch time and calibration
// time so expected costs can be aggregated in time rather than reciprocal QPS.
// Existing demo_jhq_budget_arms and its historical logs remain unchanged.
...
int main(int argc, char** argv) {
    ...
    for (size_t i = 0; i < A.size(); ++i) {
        ...
        printf("ARM FIXED alpha=%-6.0f recall=%.9f qps=%.6f batch_ms=%.9f\n",
               A[i], Arec[i], Aqps[i], 1000.0 * nt / Aqps[i]);
    }
    ...
    for (int S2 : Slist) {
        ...
        double cm_sum = 0, em_sum = 0, batch_ms_sum = 0; int disagree = 0;
        ...
        for (int r = 0; r < REPS; ++r) {
            ...
            const double batch_ms = 1000.0 * nt / Aqps[b];
            batch_ms_sum += batch_ms;
            printf("ARM DRAW S=%d draw=%d alpha=%.0f recall=%.9f loss=%.9f "
                   "qps=%.6f batch_ms=%.9f cal_ms=%.9f enum_alpha=%.0f\n",
                   S2, r, A[b], Arec[b], ceil_rec - Arec[b], Aqps[b],
                   batch_ms, cm, A[eb]);
        }
        ...
        printf("ARM TIME S=%d reps=%d nq=%d mean_batch_ms=%.9f "
               "mean_cal_ms=%.9f effective_qps=%.6f\n", S2, REPS, nt,
               batch_ms_sum / REPS, cm_sum / REPS,
               1000.0 * nt / (batch_ms_sum / REPS));
    }
    printf("\n=== ARMS_OK ===\n");
    return 0;
}
