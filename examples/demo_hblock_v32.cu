...
int main(int argc, char** argv)
{
    ...
    int beam  = (argc >  4) ? atoi(argv[4])  : 32;
    ...
    printf("nb=%d  nq=%d  d=%d  d_gt=%d  beam=%d\n", nb, nq, d_base, d_gt, beam);
    ...
    // Sweep beam values at search time
    std::vector<int> beams = {8, 16, 32, 64, 128};
    printf("\n%-10s %-10s %-12s\n", "beam", "recall@10", "QPS");
    printf("%-10s %-10s %-12s\n", "----", "---------", "---");
    ...
    for (int b : beams) {
        // Update beam at search time by rebuilding params is not directly supported.
        // For sweep, we just run with the build-time beam and report.
        // A proper sweep would rebuild the index with different params.
        break;
    }

    // Single run with build-time beam
    auto t0 = Clock::now();
    const int REPS = 5;
    ...
    printf("beam=%-6d  recall@10=%.4f  QPS=%.0f  latency=%.2f ms\n",
           beam, recall, qps, ms);
