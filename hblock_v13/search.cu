...
{
    if (nq > ws.batch_cap)
        throw std::runtime_error("hblock_v13: nq > batch_cap");
...
// ── leaf_flat_kernel v13: transposed codes layout [blk][bpv][leaf_size] ───────
// Access pattern: all 128 threads read leaf_base[b * leaf_size + tid]
// → 128 consecutive bytes per iteration = 1 cache line = fully coalesced.
// v12: each thread jumped bpv=384 bytes → 128 cache misses per iteration.
__global__ void leaf_flat_kernel(
    ...
    const uint8_t* __restrict__ leaf_codes,   // [blk][bpv][leaf_size] transposed
    ...
    int d, int Kr, int Br, int bpv, int leaf_size, int top_p)
{
    ...
    // Transposed layout: leaf_codes[blk * bpv * leaf_size + b * leaf_size + tid]
    ...
    if (tid < n_vecs) {
        fd = 0.f;
        if (Br == 4) {
            // Each byte encodes 2 dimensions (low nibble = even dim, high = odd dim)
            for (int b = 0; b < bpv; ++b) {
