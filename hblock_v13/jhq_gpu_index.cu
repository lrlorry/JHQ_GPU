#include "hblock_v13/jhq_gpu_index.cuh"
#include "hblock_v13/encode.cuh"
...
void HBlockIndex::add(const float* h_x, int n) {
    ...
    // Transposed layout: [blk][bpv][leaf_size] for coalesced kernel access.
    // Old layout was [blk][leaf_size][bpv] (= [blk * leaf_size + pos][bpv]).
    ...
    for (int i = 0, j; i < n; i = j) {
        ...
        for (int vi = i; vi < j; ++vi) {
            ...
            // Store codes transposed: for each byte b, write to [blk][b][pos]
                          ...
                          float* h_dists, int* h_ids) const
{
    ...
    printf("  [v13] Route=%.1f ms  GPUSort=%.1f ms (%d pairs)"
